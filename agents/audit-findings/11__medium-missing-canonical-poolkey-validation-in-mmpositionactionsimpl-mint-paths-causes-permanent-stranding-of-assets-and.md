[Medium] Missing canonical PoolKey validation in MMPositionActionsImpl mint paths causes permanent stranding of assets and frozen withdrawals

# Description

New-position mint functions accept arbitrary PoolKey and can create LP positions in non-core pools without CoreHook/VTS registration. [Negative deltas are settled from MMPositionManager balances](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/PositionManagerImpl.sol#L137-L147), moving backing tokens into foreign pools and preventing later decreases/burns. This can freeze user withdrawals or force protocol top-ups.

The mint-new-position flows in MMPositionActionsImpl (_mintPosition, _mintFromDeltas, and _mintPositionInternal) accept a user-supplied PoolKey without enforcing that it is the canonical core LCC/LCC pool (with CoreHook). The low-level call in [PositionManagerImpl._modifySyntheticLiquidity then invokes poolManager.modifyLiquidity](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/PositionManagerImpl.sol#L273-L294) using that unvalidated key and follows with [MarketFactory.afterModifyLiquidity(key)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/MarketFactory.sol#L468-L481), which does not verify the key’s hook identity. When the PoolKey targets a non-core or vanilla pool, CoreHook is never executed and [VTSOrchestrator.processPosition is not called](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/VTSOrchestrator.sol#L560-L572), so the position is not registered in VTS. Despite this, negative deltas for the mint are settled from MMPositionManager’s own token balances, transferring assets from MMPositionManager to the foreign pool. The resulting LP position is owned by MMPositionManager but remains unknown to VTS, making subsequent decrease/burn operations impossible via protocol paths. This drains tokens that back user credits and [caps future withdrawals by MMPositionManager’s depleted balances](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L65-L79), effectively freezing user funds or requiring protocol top-ups to restore liquidity.

# Severity

**Impact Explanation:** [High] Backing tokens for user credits can be permanently stranded in a non-core LP owned by MMPositionManager with no protocol path to unwind, freezing withdrawals indefinitely or requiring protocol top-ups to restore liquidity.

**Likelihood Explanation:** [Low] The attack is griefing with no direct profit and depends on specific internal states (notably MMPositionManager holding substantial balances or protocol deltas). These conditions are plausible but not guaranteed and reflect unprofitable attacker behavior.
...
### Scenario 1.
An attacker identifies that MMPositionManager holds significant balances of an underlying token from prior operations (e.g., unwraps to address(this)). They craft a PoolKey for a vanilla Uniswap v4 pool on the same PoolManager and call the mint action with that key and a large liquidity value. [Negative deltas are paid from MMPositionManager’s balances](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/PositionManagerImpl.sol#L137-L147), moving assets into the foreign pool. CoreHook is not invoked, [VTS never registers the position](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/CoreHook.sol#L148-L162), and [future user withdrawals fail or are reduced because MMPositionManager no longer holds the backing tokens](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L65-L79).
#### Preconditions / Assumptions
- (a). MMPositionManager is configured as a bound endpoint in LiquidityHub for the market factory
- (b). MMPositionManager holds nontrivial balances of the targeted token (e.g., from prior unwraps to address(this) or settle flows with usePositionManagerBalance=true)
- (c). A vanilla (or permissive) Uniswap v4 pool for the targeted token exists on the same PoolManager (or can be permissionlessly created)
- (d). Attacker owns or is approved for a valid commitment NFT (tokenId)

### Scenario 2.
An attacker uses MINT_POSITION_FROM_DELTAS with payerIsUser=true and a foreign PoolKey to exploit existing protocol deltas at address(this). Because [CoreHook is not invoked for the foreign pool](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/VTSOrchestrator.sol#L560-L572), in-hook settlement does not occur, and negative deltas are still settled from MMPositionManager’s balances. Assets move into a foreign LP position unregistered in VTS, leading to frozen withdrawals for users expecting to TAKE their credited tokens.
#### Preconditions / Assumptions
- (a). MMPositionManager is configured as a bound endpoint in LiquidityHub for the market factory
- (b). There exist positive protocol deltas at address(this) for the targeted currencies or MMPositionManager holds corresponding token balances
- (c). A vanilla (or permissive) Uniswap v4 pool exists on the same PoolManager
- (d). Attacker owns or is approved for a valid commitment NFT (tokenId)
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/PositionManagerImpl.sol)
...
+        if (address(key.hooks) != MarketHandlerLib.getCoreHook(marketFactory)) revert Errors.InvalidMarket(key);
+        if (MarketHandlerLib.getProxyHook(marketFactory, key) == address(0)) revert Errors.InvalidMarket(key);
+
