[Medium] Unbounded backfill loop in HubRSC registration causes persistent settlement dispatch DoS for an LCC

# Description

[HubRSC._backfillUnderlyingQueueForLcc iterates over an unbounded per-LCC queue](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/reactive/src/HubRSC.sol#L596-L615) on first registration. If many distinct recipients were queued before registration, the loop can exceed gas limits, revert, and repeatedly prevent registration and subsequent liquidity dispatch for that LCC, causing stuck settlements.

On first-time underlying registration for an LCC, [HubRSC._registerLccUnderlying calls _backfillUnderlyingQueueForLcc(lcc, underlying)](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/reactive/src/HubRSC.sol#L587-L596). That function walks the entire per-LCC queue (queueDataByLcc[lcc]) [in a while loop without pagination, enqueuing each key into the shared-underlying queue](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/reactive/src/HubRSC.sol#L596-L615). If HubRSC has already ingested many SettlementQueuedReported events for that LCC ([creating many unique (lcc, recipient) keys](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/reactive/src/HubRSC.sol#L260-L279)) before seeing LiquidityHub’s LCCCreated/LiquidityAvailable on HubRSC, the loop may exceed the block gas limit and revert. Because [_handleLiquidityAvailable always calls _registerLccUnderlying before dispatch](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/reactive/src/HubRSC.sol#L386-L405), a revert prevents dispatch from starting. Since [MoreLiquidityAvailable retries are only emitted after a successful dispatch](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/reactive/src/HubRSC.sol#L528-L567), the system can get stuck for that LCC: hasUnderlyingForLcc remains false and each new liquidity event reattempts and reverts at registration, leaving all queued settlements for that LCC undispatched until operator intervention.

# Severity

**Impact Explanation:** [High] For the affected LCC, core settlement dispatch via HubRSC is completely unusable until admin intervention, causing recipients’ settlements to remain stuck indefinitely.

**Likelihood Explanation:** [Low] Requires uncommon operational conditions (late deployment/resync or significant cross-subscription delivery skew) and a large backlog of distinct recipients. Not attacker-driven due to HubCallback/Spoke gating.
...
### Scenario 1.
Late deployment/resync: HubRSC is deployed or resynced after significant protocol usage. It ingests many SettlementQueuedReported logs for an LCC (many unique recipients) before seeing LCCCreated/LiquidityAvailable. On first registration, _backfillUnderlyingQueueForLcc iterates over the large per-LCC queue, exceeds gas, reverts, and blocks registration and dispatch for that LCC, leaving all queued settlements stuck.
#### Preconditions / Assumptions
- (a). HubRSC is not yet subscribed or is resynced after LCC usage begins.
- (b). Many authorized recipients with real SettlementQueued events for the same LCC exist (large number of unique (lcc, recipient) keys).
- (c). HubRSC processes SettlementQueuedReported before it sees LCCCreated/LiquidityAvailable for that LCC.
- (d). Per-LCC queue size is large enough that iterating it in one transaction exceeds gas limits.

### Scenario 2.
Out-of-order delivery: Even with both subscriptions live, HubRSC receives many SettlementQueuedReported logs for an LCC before it receives LCCCreated/LiquidityAvailable. This builds a large per-LCC queue. When registration later runs, the unbounded loop exceeds gas, reverts, and repeatedly prevents registration and dispatch for that LCC, causing stuck settlements.
#### Preconditions / Assumptions
- (a). Both subscriptions are active, but delivery is significantly out-of-order (SettlementQueuedReported arrives well before LCCCreated/LiquidityAvailable).
- (b). Many authorized recipients have queued settlements for the same LCC, creating a large per-LCC queue.
- (c). Per-LCC queue size is large enough that the first registration backfill exceeds gas limits.
...
+        // Do not backfill historical per-LCC entries to avoid unbounded gas; historical entries are processed via per-LCC lane.
     }
