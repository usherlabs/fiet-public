[Medium] Unconditional DICE index checkpointing with per-event floors in VTSFeeLib/VTSCommitLib causes reduced coverage burn and smaller slashed pot

# Description

Frequent settlement during small coverage increments discards fractional DICE entitlements due to [per-event floor rounding](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L207) and [unconditional advancement of per-position coverage checkpoints](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L790) without remainder, reducing fee-burn materialisation and shrinking the slashed pot (harming CISE bonus recipients).

Pool-level DICE coverage index increments are floored per event ([VTSCommitLib.incrementCoverage uses FullMath.mulDiv to compute deltaIndex = floor(coveredAmount * Q128 / totalDeficitPrincipal)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L207)). At position level, VTSFeeLib._settleDICEForToken [unconditionally advances pa.coverageIndexLastX128 to the current pool index](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L790) and realises [cov = floor(deficitPrincipal * (indexNow - indexLast) / Q128)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L795). If cov == 0, no remainder is stored. Because [VTSOrchestrator.settlePositionGrowths is permissionless while unpaused](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L503-L512), a position owner (or any caller) can settle frequently after each small coverage increment, turning floor-of-sums into sum-of-floors at the position level and permanently discarding fractional entitlements that would have aggregated into a non-zero burn if settled less frequently. Additionally, even when cov > 0, _applyCoverageBurn -> _applyBurnBase -> _calculateFeesBurn [requires a positive outflow window (ofDelta > 0) and nonzero fees](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L279-L281); if these are zero at settlement time, the burn is skipped while the coverage checkpoint is still advanced, losing that event permanently. Net effect: reduced coverage-derived fee-burn, smaller slashed pot, and lower CISE bonuses to other participants.

# Severity

**Impact Explanation:** [Medium] The issue causes a direct, material reduction in fee-burn materialisation, shrinking the slashed pot and reducing CISE bonus payouts to other participants (loss of yield/fees) without breaking core invariants or stealing principal.

**Likelihood Explanation:** [Medium] Exploitation by self-interested MMs is realistic but depends on naturally small coverage increments, frequent settlement calls, and economic viability versus gas costs. These constraints are common but not guaranteed across all markets and times.
...
### Scenario 1.
Self-gaming by an MM: While the pool is unpaused and the MM’s position has a live cumulative deficit D > 0 and pool principal P > 0, coverage arrives in many small increments. The MM runs a bot to call [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L503-L512) frequently. For each small increment, pa.coverageIndexLastX128 is [advanced](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L790) and [cov = floor(D * deltaIndex / Q128)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L795) = 0, discarding fractional coverage. Over many increments, realised burn is strictly less than settling once later, shrinking the slashed pot and other participants’ CISE bonuses.
#### Preconditions / Assumptions
- (a). Pool is unpaused (permissionless settlement).
- (b). Attacker’s position has cumulativeDeficit D > 0.
- (c). Pool totalDeficitPrincipal P > 0.
- (d). Coverage increments are often small so that D * deltaIndex / Q128 < 1 per event.
- (e). Economic viability: saved fee-burn outweighs gas costs for frequent settlement.

### Scenario 2.
Timing settlement to discard coverage when outflow window is closed: Immediately after a coverage index step that would yield cov > 0 for a deficit position (D > 0, P > 0), but before any new outflows occur (ofDelta == 0) or when totalFees == 0, the attacker calls [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L503-L512). The coverage checkpoint is advanced, but _calculateFeesBurn [returns 0 due to outflow/fees gating](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L279-L281), so the event is never charged later, further reducing the pot and bonuses.
#### Preconditions / Assumptions
- (a). Pool is unpaused.
- (b). Victim position has cumulativeDeficit D > 0 and pool totalDeficitPrincipal P > 0.
- (c). A coverage increment occurred (indexNow > indexLast) such that cov > 0 would be computed.
- (d). No new outflow window since last snap (ofDelta == 0) and/or totalFees == 0 at settlement time.
- (e). Attacker can call settlePositionGrowths quickly after coverage increment and before outflows progress.

### Scenario 3.
Third-party griefing: A non-owner bot frequently calls [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L503-L512) across many positions during periods of small coverage increments. Each call advances per-position coverage checkpoints and discards fractional entitlements, keeping the pot smaller than under less adversarial settlement, thereby reducing CISE bonuses pool-wide.
#### Preconditions / Assumptions
- (a). Pool is unpaused.
- (b). Multiple positions have cumulativeDeficit D > 0 and pool principal P > 0.
- (c). Coverage arrives in small increments over time.
- (d). Attacker is willing to pay gas without direct profit (griefing motive).
...
+            // FIX-ME: DICE rounding-loss mitigation:
+            // - Implement per-position Q128 remainder carry: accumulate (deficitPrincipal * (indexNow - indexLast)) mod Q128
+            //   into a new pa.coverageBurnRemainderX128[tokenIndex], convert carry into whole units, and add to cov.
+            // - Bank cov into a new pa.pendingDICEBurnBase[tokenIndex] with an outflows floor (like residual), then consume
+            //   via _applyBurnBase; if not fully consumed (e.g., ofDelta == 0 or no fees), keep it banked for later.
+            // - Always advance coverageIndexLastX128 once remainder carry is implemented to avoid re-accounting.
...
+        // FIX-ME: Also apply any banked DICE-derived burn (new helper), symmetric to residual: _applyBankedDICEBurn(...)
 ...
 library VTSPositionLib {
     ...
     function _applyLiquidityMirrorTransition(
         ...
         if (initialLiquidity > 0 && nextLiquidity == 0) {
             ...
             pa.commitmentDeficitBps = 0;
+            // FIX-ME: On full deactivation also clear new DICE carry/banked fields to avoid stale episode carryover:
+            // - pa.coverageBurnRemainderX128.token0/1 = 0;
+            // - pa.pendingDICEBurnBase.token0/1 = 0;
+            // - pa.pendingDICEBurnOutflowsFloor.token0/1 = 0;
         }
