[Low] Missing min-immediate-out guard for exact-input swaps in ProxyHook allows deficit queuing and reduced immediate payout

# Description

In ProxyHook, exact-input swaps only enforce that the core AMM consumes the full input. If the CanonicalVault’s output-side underlying reserve is short, the hook [converts the shortfall into LCC and queues a settlement claim for the user (deficitRecipient)](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L255-L285). With empty hookData, the recipient typically [resolves via IMsgSender](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L473-L481), so many router-driven swaps allow this by default. sqrtPriceLimit only protects the core price path, not immediate underlying availability. Users can thus spend full input but receive reduced or zero immediate underlying, with the remainder as a delayed claim. This is an intentional design choice, not a funds-loss bug.

ProxyHook’s exact-output path [enforces immediate underlying availability via _assertExactOutputAvailable](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L370-L378) and re-checks that amountToSettle equals the requested output, reverting if not. For exact-input (params.amountSpecified < 0), _assertExactInputAfterCoreSwap [only checks that the core fill matches the specified input; if a deficitRecipient can be resolved (e.g., via IMsgSender when hookData is empty), it permits delivering less immediate underlying than the core amountOut.](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L388-L407) The function [_cancelProxySwapLccWithDeficit splits the output](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L255-L285): it cancels (burns) up to inMarketBalanceOf(outputUnderlying) and immediately settles that amount, then transfers the shortfall as LCC to the deficitRecipient and [queues it via LiquidityHub.queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/LiquidityHub.sol#L785-L802). [The BeforeSwapDelta returned to PoolManager is built from amountToSettle (the immediately-available underlying), not the full core amountOut.](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L312-L321) [MarketFactory sets ProxyHook as an EXEMPT issuer](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/MarketFactory.sol#L234), enabling the LCC transfer and queueing logic. sqrtPriceLimitX96 constrains only the core AMM price, not vault-side immediate liquidity, so exact-input swaps can complete with reduced immediate underlying out without reverting. This preserves protocol invariants but exposes users to reduced immediate settlement and MEV/griefing that can drain reserves just before a victim’s swap.

# Severity

**Impact Explanation:** [Low] No principal loss or invariant break occurs; shortfalls are converted to LCC and queued for later settlement. The impact is reduced immediate payout and UX/composability degradation rather than funds loss.

**Likelihood Explanation:** [Medium] The most representative case (partial immediate-out without an attacker) requires exact-input, IMsgSender-based routing, and intermittent reserve shortfalls—all uncommon but plausible conditions in real markets.
...
### Scenario 1.
MEV front-run drains the CanonicalVault’s output reserve to near-zero before a victim’s exact-input swap. The victim’s swap consumes full input in the core AMM but receives zero immediate underlying; the entire amountOut is transferred as LCC and queued to the user.
#### Preconditions / Assumptions
- (a). Market created via MarketFactory; ProxyHook is an issuer and BOUND_EXEMPT
- (b). [Router implements IMsgSender](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L473-L481); hookData is empty so deficitRecipient resolves to the end user
- (c). CanonicalVault’s output-side inMarketBalanceOf is low (near zero)
- (d). Victim uses exact-input swap with permissive or zero sqrtPriceLimitX96
- (e). Attacker can submit an exact-output swap first to drain the available reserve

### Scenario 2.
Without an attacker, the CanonicalVault has some but insufficient output reserve (e.g., 30). A victim performs an exact-input swap; ProxyHook settles only the 30 units immediately and queues the remainder as LCC for later settlement to the user.
#### Preconditions / Assumptions
- (a). [Router implements IMsgSender](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L473-L481); hookData is empty so deficitRecipient resolves to the end user
- (b). CanonicalVault has some but insufficient output-side inMarketBalanceOf
- (c). Victim uses exact-input swap; no attacker required

### Scenario 3.
An aggregator uses an IMsgSender-enabled router for an exact-input proxy swap with empty hookData and assumes it will receive full immediate-out to continue routing. Due to insufficient reserve, only a partial immediate-out is delivered; the remainder is queued, causing the route to fail or underperform.
#### Preconditions / Assumptions
- (a). Aggregator uses an IMsgSender-enabled router; hookData is empty (default locker resolution)
- (b). Aggregator assumes full immediate underlying for subsequent steps without enforcing a min-immediate-out at the proxy level
- (c). CanonicalVault’s output-side reserve is insufficient at execution time
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol)
...
+            // Always require full immediate underlying availability on exact-input swaps (no deficit path).
+            if (amountOut > maxOutputAvailable) {
