[Informational] Missing zero-amount guard in MarketVault._cancelLCCWithDeficit causes revert blocking all-deficit exact-input swaps

# Description

When vault availability is zero, MarketVault._cancelLCCWithDeficit [unconditionally calls LiquidityHub.cancel](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol#L331) with amount 0, which reverts in [LCC.burn](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/LCC.sol#L165-L168), preventing the intended all-deficit exact-input flow (transfer deficit LCC to recipient and queue settlement).

MarketVault._cancelLCCWithDeficit [splits a requested LCC cancellation into amountToCancel = min(amount, available) and deficitAmount = amount - available](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol#L319-L324). It then [unconditionally calls LiquidityHub.cancel(address(lccToken), address(this), amountToCancel)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol#L331). When available == 0 and a deficitRecipient is supplied, amountToCancel is 0. [LiquidityHub.cancel](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/LiquidityHub.sol#L702-L711) routes to [LCC.burn, which reverts on zero amounts](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/LCC.sol#L165-L168), causing the entire swap to revert [before transferring deficit LCC to the recipient and queueing via queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol#L336-L337). [ProxyHook’s exact-input path explicitly allows deficit handling when a recipient is resolved](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/ProxyHook.sol#L359-L362), so the all-deficit case should be supported. The result is a functional DoS of that specific swap path under zero-availability conditions.

# Severity

**Impact Explanation:** [Informational] Functional DoS of a specific exact-input-with-deficit swap path under zero-availability conditions; no principal loss, no invariant violation, and no permanent stuck funds.

**Likelihood Explanation:** [Medium] Zero availability for a given underlying is a plausible state; exact-input swaps with a resolvable recipient are normal; no special privileges are needed.
...
### Scenario 1.
All-deficit exact-input swap using locker resolution (IMsgSender): exact-input swap on the proxy pool with empty hookData resolves a locker recipient; vault availability for the output underlying is zero; ProxyHook allows deficit; _cancelLCCWithDeficit computes amountToCancel=0 and [calls LiquidityHub.cancel(0)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol#L331), which [reverts](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/LCC.sol#L165-L168).
#### Preconditions / Assumptions
- (a). Market created and initialized; ProxyHook registered as an issuer for both LCCs
- (b). Vault’s ERC-6909 claim token balance for the output underlying is exactly zero at swap time
- (c). Swapper routes through a smart account/router exposing IMsgSender.msgSender so locker recipient can be resolved
- (d). Core swap yields a positive amountOut

### Scenario 2.
All-deficit exact-input swap with explicitly provided recipient: exact-input swap with hookData encoding a valid non-exempt recipient; availability is zero; ProxyHook allows deficit; _cancelLCCWithDeficit calls [LiquidityHub.cancel(0)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol#L331) and reverts before transferring deficit LCC and queueing.
#### Preconditions / Assumptions
- (a). Market created and initialized; ProxyHook registered as an issuer for both LCCs
- (b). Vault’s ERC-6909 claim token balance for the output underlying is zero
- (c). hookData provides a valid non-exempt recipient address
- (d). Core swap yields a positive amountOut

### Scenario 3.
Depleted market integrator relying on deficit queueing: after prior operations deplete the vault’s output underlying claims to zero, an integrator performs an exact-input swap with a resolvable recipient; the same zero-amount cancel path reverts, blocking the intended all-deficit handling.
#### Preconditions / Assumptions
- (a). Market created and initialized; ProxyHook registered as an issuer for both LCCs
- (b). Vault’s ERC-6909 claim token balance for the output underlying is depleted to zero due to prior operations
- (c). Swapper supplies a resolvable deficit recipient (locker or explicit)
- (d). Core swap yields a positive amountOut
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/modules/MarketVault.sol)
...
+        if (amountToCancel > 0) {
+            // We only cancel the portion we can distribute via the swap mechanism. Skip when fully deficit.
