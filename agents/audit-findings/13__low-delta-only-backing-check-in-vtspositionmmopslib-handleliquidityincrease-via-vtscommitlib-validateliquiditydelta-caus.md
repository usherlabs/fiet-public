[Low] Delta-only backing check in VTSPositionMMOpsLib._handleLiquidityIncrease (via VTSCommitLib.validateLiquidityDelta) causes temporary cumulative over-issuance beyond backing

# Description

The MM add-liquidity path validates only the new liquidity delta against total (settled + signal) backing, allowing multiple small adds in a single transaction to cumulatively exceed backing before any checkpoint marks a commitment deficit. Issued LCC are deposited into the pool (not liquid to the attacker), so impact is a correctness/safety gap rather than immediate fund loss.

During MM add-liquidity, VTSPositionMMOpsLib._handleLiquidityIncrease calls [VTSCommitLib.validateLiquidityDelta](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSPositionMMOpsLib.sol#L404), which [computes issuedValue from only params.liquidityDelta](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSCommitLib.sol#L163) and [compares it to the full position’s settledValue plus the full commit’s signalValue](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSCommitLib.sol#L175). This enforces only newIssuedSlice <= settled + signal and does not account for the post-add total issuance. No commitment checkpoint is executed during add; the [insolvency freeze gate depends on commitmentDeficit](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSPositionLib.sol#L1117) set by [later checkpoints](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSCommitLib.sol#L494). An MM can therefore split a large add into many small adds in one transaction; each passes individually, but the final total issuance exceeds backing until a later checkpoint records deficits and freezes further changes. The LCC minted on add are immediately [used to settle PoolManager negative deltas](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/modules/PositionManagerImpl.sol#L295) (funding the LP position), reducing immediate theft risk but leaving an issuance-time invariant gap.

# Severity

**Impact Explanation:** [Low] No direct, immediate principal loss is demonstrated; LCC minted on add fund the pool rather than becoming spendable by the attacker. The main risk is a correctness/safety gap at issuance-time that is later remediated by checkpoint-derived commitment deficits which freeze further non-seizure changes.

**Likelihood Explanation:** [Low] Feasible only for an adversarial MM/advancer executing multiple adds in one transaction and with limited rational incentive (no immediate profit). Post-transaction, permissionless checkpoints promptly reveal and freeze deficits, limiting ongoing exploitation.
...
### Scenario 1.
Single transaction: an MM locker/advancer with a live commit (signal USD value S) performs multiple sequential add-liquidity calls on the same position, each small enough that [validateLiquidityDelta passes](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSCommitLib.sol#L175) (new slice <= settled + signal). After N increments, total post-add issuance exceeds backing. LCC minted per add are immediately used to fund the pool deposit. Only a later checkpoint records commitment deficits and freezes further non-seizure changes.
#### Preconditions / Assumptions
- (a). Attacker is the MM locker/advancer for a live, non-expired commitId with valid signal
- (b). Position starts with low/zero settledValue
- (c). Multiple add-liquidity operations are executed within a single transaction
- (d). No commitment checkpoint is interleaved during the same transaction

### Scenario 2.
Multiple positions under one commit: within a single transaction, the MM mints/increases several positions tied to the same commit and repeats small adds per position. Each per-delta check reuses the same commit’s full signal backing, so all adds pass. Aggregate issuance across positions exceeds commit backing until a later checkpoint marks deficits across those positions.
#### Preconditions / Assumptions
- (a). Attacker controls a live commitId with sufficient signal backing
- (b). Ability to create/increase multiple positions under the same commit within one transaction
- (c). Positions start with low/zero settledValue
- (d). No commitment checkpoint is interleaved during the same transaction

### Scenario 3.
In-hook settlement headroom: the MM enables [in-hook protocol credit settlement during add](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/VTSPositionMMOpsLib.sol#L104), increasing the position’s settledValue before validation. validateLiquidityDelta then sees larger (settled + signal) backing and accepts larger deltas. Repeating small adds in the same transaction leads to post-add total issuance exceeding backing, with deficits only recognized at a later checkpoint.
#### Preconditions / Assumptions
- (a). Attacker is the MM locker/advancer with a live commitId
- (b). Attacker holds positive underlying protocol credits usable for in-hook settlement
- (c). Multiple add-liquidity operations are executed within a single transaction
- (d). No commitment checkpoint is interleaved during the same transaction
...
+                    // Enforce total-issued-after-add by validating against post-add total liquidity
+                    liquidityDelta: int256(uint256(s.positions[p.positionId].liquidity))
                 }),
                 true
             );
         }
