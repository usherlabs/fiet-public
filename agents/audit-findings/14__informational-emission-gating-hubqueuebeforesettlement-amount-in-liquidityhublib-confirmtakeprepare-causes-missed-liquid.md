[Informational] Emission gating (hubQueueBeforeSettlement < amount) in LiquidityHubLib.confirmTakePrepare causes missed LiquidityAvailable and stalls automated settlement dispatch

# Description

[LiquidityHub.confirmTake](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LiquidityHub.sol#L956-L964) increments reserves and [settles the Hub’s own queue without reducing reserves](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/LiquidityHubLib.sol#L520-L528), but [LiquidityAvailable is suppressed when the Hub queue before settlement is ≥ the new amount](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/LiquidityHubLib.sol#L670-L671). [HubRSC only dispatches on liquidity events](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/reactive/src/HubRSC.sol#L400-L406), so external queued settlements can remain undispatched despite on-chain liquidity being available.

In LiquidityHubLib.confirmTakePrepare, [ctx.emitLiquidityAvailable is set to shouldEmit && (hubQueueBeforeSettlement < amount)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/LiquidityHubLib.sol#L670-L671). LiquidityHub.confirmTake then increases marketDerived reserve and [immediately processes the Hub’s own queue via the Hub-recipient branch](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LiquidityHub.sol#L956-L964), which burns Hub-held LCC and decrements queues but [does not decrement reserves](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/LiquidityHubLib.sol#L520-L528). When amount ≤ hubQueueBeforeSettlement, LiquidityAvailable is not emitted at all. Since HubRSC [relies solely on LiquidityAvailable/MoreLiquidityAvailable to trigger dispatch](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/reactive/src/HubRSC.sol#L400-L406) and never queries reserves, external queued settlements may not be dispatched even though the reserve is already available. Manual settlement via LiquidityHub.processSettlementFor remains possible, so this is a liveness/automation issue rather than a funds safety problem.

# Severity

**Impact Explanation:** [Medium] It breaks important non-core automation: significant but temporary availability loss of automated settlement dispatch; manual settlement remains possible and funds are not lost.

**Likelihood Explanation:** [Medium] Conditions are plausible in normal operation (Hub queue from wrapWith residuals, external queues present, and issuer confirming liquidity amounts not exceeding the Hub queue).
...
### Scenario 1.
Organic stall on a single LCC: External user has a queued settlement; the Hub also has its own queue. Issuer calls confirmTake with amount ≤ the Hub queue, which suppresses LiquidityAvailable. The Hub queue is reduced without consuming reserves, leaving reserves available for externals, but HubRSC never dispatches due to the missing event.
#### Preconditions / Assumptions
- (a). An LCC is initialized with underlying U
- (b). Victim has a non-zero queued settlement on the LCC
- (c). The Hub (address(this)) has a non-zero queued settlement on the same LCC
- (d). Issuer confirms liquidity with amount A such that A ≤ Hub queue before settlement

### Scenario 2.
Griefing via inflated Hub queue + small issuer chunks: An adversary repeatedly creates Hub-queued shortfall using wrapWith residuals, keeping the Hub queue high. Issuer delivers liquidity in small chunks, each ≤ the Hub queue, suppressing LiquidityAvailable repeatedly and stalling automated dispatch for external recipients despite available reserves.
#### Preconditions / Assumptions
- (a). An LCC exists and wrapWith residuals can create Hub-queued shortfall
- (b). Adversary can repeatedly inflate the Hub’s queue
- (c). Issuer delivers liquidity in small increments each ≤ the Hub’s current queue
- (d). External recipients have queued settlements on the LCC

### Scenario 3.
Shared-underlying stall across multiple LCCs: Several LCCs share the same underlying. External queues exist across these LCCs. Liquidity is confirmed on one LCC with amount ≤ its Hub queue, suppressing LiquidityAvailable. HubRSC does not scan the shared-underlying lane, so queued recipients across multiple LCCs miss dispatch despite available reserves.
#### Preconditions / Assumptions
- (a). Multiple LCCs share the same underlying U and are registered
- (b). External queued settlements exist across these LCCs
- (c). Issuer confirms liquidity on one LCC with amount ≤ its Hub queue before settlement
