[Informational] Per-position rounding-up without dust handling in VTSFeeLib CISE bonus allocation causes unfair fee distribution skew

# Description

[VTSFeeLib._queueBonusForToken allocates bonuses using rounding up](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L107) while [only decrementing the pool-wide CISE denominator by exact exposure](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L138-L141) and lacking any dust/carry mechanism. Attackers can fragment exposure across many positions and touch them early to capture extra rounding dust and a larger share of the pot than ideal pro‑rata, reducing later/aggregated LPs’ rewards. Total payouts remain capped by the accrued pot and materialised slashedPot, so this is a fairness issue, not a funds-safety break.

In CISE bonus allocation, each touch computes [bonus = ceil(potAvail * ciseExposure / totalExposure)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L107), then [immediately deducts this bonus from protocolFeeAccrued](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L116) and [queues a negative pending for the position](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L120). After allocation, [only the exact ciseExposure is removed from the pool-wide CISE denominator](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L138-L141), and there is no residual dust accumulator or second-pass redistribution. As a result, fragmenting the same economic exposure across many positions (each with ciseExposure ≥ 1) and touching them before others lets an attacker harvest at least 1 wei per claim from the pot and drain it sooner, skewing rewards away from later or aggregated claimants. However, every awarded bonus reduces the accounting pot exactly, and [negative pending payouts are strictly clamped to the actually funded slashedPot at materialisation](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L443-L446), preventing any overpayment beyond what was accrued and funded. This is an exploitable allocation/fairness bias rather than a principal or invariant violation.

# Severity

**Impact Explanation:** [Informational] The issue skews fee allocation (fairness loss) by enabling small rounding gains and first-touch priority; it cannot exceed the accrued pot and is clamped by slashedPot funding, so there is no principal loss or invariant violation. Per-claim gains are dust-level and bounded.

**Likelihood Explanation:** [Medium] Exploitation requires typical operating conditions (coverage events, fee sharing enabled) and the ability to create/touch multiple positions with ≥1 wei exposure. While feasible and scriptable, gas/capital and exposure requirements limit scale.
...
### Scenario 1.
Micro-fragmentation rounding harvest: The attacker creates many small positions that each accrue a minimal non-zero CISE exposure. After a coverage event [increases protocolFeeAccrued](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol#L368) and [the CISE window](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSCommitLib.sol#L181-L186), the attacker touches each position to claim a rounded-up bonus (≥ 1 wei) and deplete the pot before others claim, reducing later LPs’ allocations.
#### Preconditions / Assumptions
- (a). Fee sharing enabled (coverageFeeShare > 0)
- (b). protocolFeeAccrued > 0 and totalCISEExposureSinceLastMod > 0
- (c). Attacker controls many positions with ciseExposureSinceLastMod ≥ 1
- (d). Attacker can trigger touches (modifyLiquidity) for each position

### Scenario 2.
Cross-position self-exclusion bypass: The attacker uses one set of positions to contribute to the pot (raising their self-exclusion) and a different set with non-zero exposure but low self-exclusion to claim from the same pot immediately, including rounding gains, thereby capturing more than their fair aggregate share ahead of others.
#### Preconditions / Assumptions
- (a). Fee sharing enabled (coverageFeeShare > 0)
- (b). protocolFeeAccrued > 0 and totalCISEExposureSinceLastMod > 0
- (c). Two attacker position sets: A (contributed this epoch) and B (non-zero exposure, low self-exclusion)
- (d). Attacker can touch B positions before others

### Scenario 3.
Opportunistic first-to-touch after large coverage event: Following a large coverage increment that significantly increases the pot and [the CISE window](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSCommitLib.sol#L181-L186), the attacker quickly touches many of their positions to seize rounded-up shares and drain the pot before honest LPs touch, skewing distribution toward the attacker.
#### Preconditions / Assumptions
- (a). Fee sharing enabled (coverageFeeShare > 0)
- (b). A large recent coverage increment increased both the pot and the CISE window
- (c). Attacker has many positions with non-zero CISE exposure and can touch them promptly
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSFeeLib.sol)
...
+        // bonus = potAvail * ciseExposure / totalExposure (round down to avoid per-claim rounding bias; dust remains allocatable)
