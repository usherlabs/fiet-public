[Informational] Direction-insensitive reciprocal rounding in ProxyHook._calcCoreSqrtPriceLimit (flipped mapping) causes infinitesimally looser min bound than intended

# Description

When proxy/core directions are flipped, [ProxyHook._calcCoreSqrtPriceLimit](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L183-L191) always [floors the reciprocal of the proxy-side sqrtPriceLimitX96](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L189). For mapped core zeroForOne (min bound), the conservative mapping should be ceil; floor makes the min bound infinitesimally looser (and for oneForZero, slightly stricter), leading to dust-level deviation from the user’s intended slippage limit.

[ProxyHook._calcCoreSqrtPriceLimit transforms a proxy-side sqrtPriceLimitX96 into a core-side limit](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L302-L304). In flipped proxy/core mappings, it [reciprocates the limit with floor division](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L189) and [clamps to (MIN+1, MAX-1)](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L190-L191). Uniswap v4 semantics are direction-sensitive: zeroForOne uses a minimum-price bound and oneForZero a maximum-price bound. Correct reciprocal rounding should be ceil for min bounds and floor for max bounds. The current implementation always floors, which in the min-bound case allows the price to move by at most one Q64.96 sqrt-price unit below the intended minimum, producing an infinitesimally worse execution than requested; in the max-bound case, it can be over-strict and cause needless reverts. [Extremes (MIN+1/MAX-1)](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L186-L187) and [zero (direction-aware defaults)](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L183-L191) are handled exactly, further limiting impact. Practically, the effect is negligible and often bypassed by integrations that use zero/extreme limits or separate minOut checks.

# Severity

**Impact Explanation:** [Informational] The deviation is bounded by a single Q64.96 sqrt-price unit, resulting in dust-level differences with no realistic path to material loss or invariant violation.

**Likelihood Explanation:** [Low] Requires a flipped market, a tight non-extreme price limit used near the boundary, and reliance on the price limit for slippage control; many integrations use zero/extreme limits or separate minOut checks.
...
### Scenario 1.
Flipped proxy/core mapping with mapped core zeroForOne (min bound): user supplies a non-zero, non-extreme proxy sqrtPriceLimitX96; [_calcCoreSqrtPriceLimit floors the reciprocal](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L189), slightly loosening the min bound and allowing execution at a price infinitesimally below the intended minimum.
#### Preconditions / Assumptions
- (a). Proxy and core token ordering are flipped so flipped == true in _calcCoreSqrtPriceLimit
- (b). Mapped core direction is zeroForOne (min bound)
- (c). User provides a non-zero, non-extreme proxy-side sqrtPriceLimitX96 close to the boundary
- (d). Core pool can execute near the boundary

### Scenario 2.
Flipped proxy/core mapping with mapped core zeroForOne exact-output swap: user relies on the price limit as a min-price guard; [floor reciprocal](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/ProxyHook.sol#L189) instead of ceil permits an imperceptibly lower execution price, potentially requiring a dust-level extra input compared to a perfectly conservative mapping.
#### Preconditions / Assumptions
- (a). Proxy and core token ordering are flipped so flipped == true in _calcCoreSqrtPriceLimit
- (b). Mapped core direction is zeroForOne (min bound)
- (c). User performs an exact-output swap on the proxy
- (d). User provides a non-zero, non-extreme proxy-side sqrtPriceLimitX96 close to the boundary
- (e). No separate minOut cap beyond the price limit is enforced by the caller
...
+        // Direction-aware reciprocal: ceil for min-bound (zeroForOne), floor for max-bound (oneForZero).
+        uint256 q192 = uint256(1) << 192;
+        uint256 inverted = coreZeroForOne
+            ? (q192 + sqrtPriceLimitX96 - 1) / sqrtPriceLimitX96
+            : q192 / sqrtPriceLimitX96;
