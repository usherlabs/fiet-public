[Informational] Rounding mismatch in CISE accounting (VTSCommitLib.incrementCoverage and VTSFeeLib._settleCISEForToken) causes mild bonus allocation dilution

# Description

[Pool-side CISE denominator grows by the full coveredAmount](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L200-L236) while per-position exposures are floor-rounded and indices advance even on zero exposure, creating persistent 'ghost' denominator that slightly dilutes future bonus allocations.

On each coverage event, [VTSCommitLib.incrementCoverage adds coveredAmount to poolAccounting.totalCISEExposureSinceLastMod](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L200-L236) and [advances the pool coverage-per-settled index by floor(coveredAmount * Q128 / totalSettled)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L200-L236). Per-position exposure is realised in [VTSFeeLib._settleCISEForToken as floor(settled * deltaIndex / Q128)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L720-L740), and [ciseIndexLastX128 is advanced even when this rounds to zero](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L712-L723). The sum of per-position realised exposures for a step can be strictly less than the coveredAmount added to the pool’s denominator, leaving a persistent 'ghost' denominator. Bonus allocation in [VTSFeeLib._queueBonusForToken divides potAvail proportionally by totalCISEExposureSinceLastMod](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSFeeLib.sol#L39-L74), so the ghost denominator slightly dilutes allocations. This does not trigger fund loss, stuck funds, or invariant violations; it is a dust-level fairness/rounding nuance.

# Severity

**Impact Explanation:** [Informational] Effects are dust-level: slight dilution of proportional bonus allocations due to integer rounding; no principal loss, no stuck funds, and no invariant violations.

**Likelihood Explanation:** [Medium] Sum-of-floors rounding occurs naturally in normal operation (no attacker required), though attacker-driven micro-fragmentation is constrained and uneconomic.
...
### Scenario 1.
Micro-fragmentation: Many tiny coverage events (e.g., 1 wei) are induced; immediately after each, [settlePositionGrowths is called](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L500-L530) for many positions so per-position exposure increments round to zero while indices advance. The pool denominator grows by each 1-wei increment, creating ghost denominator that later slightly dilutes allocations.
#### Preconditions / Assumptions
- (a). Pool is unpaused
- (b). MarketFactory (trusted) forwards small liquidity usage to VTSOrchestrator.incrementCoverage
- (c). Attacker performs many minimal liquidity uses (or unwraps) to produce tiny coveredAmount steps
- (d). Anyone can call VTSOrchestrator.settlePositionGrowths to settle CISE immediately after each step

### Scenario 2.
Many positions, large coverage step: With N positions and a non-tiny coveredAmount C, the sum of floor-rounded per-position exposures can be less than C by up to < N wei, leaving a small ghost denominator that slightly dilutes future allocations.
#### Preconditions / Assumptions
- (a). Many positions have non-zero settled balances on the relevant token
- (b). A non-tiny coverage event occurs via VTSOrchestrator.incrementCoverage
- (c). Anyone may call settlePositionGrowths; no attacker action is required

### Scenario 3.
Timing-based dilution before first pot distribution: Multiple tiny coverage steps occur while the slashed pot is low/zero and are immediately settled to lock in zero per-position exposures; when the pot later becomes available, the existing ghost denominator mildly dilutes the first allocations.
#### Preconditions / Assumptions
- (a). Pool is unpaused
- (b). Multiple tiny coverage events occur before pot accrual
- (c). Public settlePositionGrowths is called to advance per-position indices with zero exposures
- (d). Later, fee burns or other events create potAvail
