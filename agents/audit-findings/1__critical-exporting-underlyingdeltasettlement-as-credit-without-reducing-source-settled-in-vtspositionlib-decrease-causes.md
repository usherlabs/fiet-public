[Critical] Exporting underlyingDeltaSettlement as credit without reducing source settled in VTSPositionLib decrease causes double realization of funds and vault drain

# Description

On MM decreases, [only the queued principal is removed](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1415-L1464) from a position’s live settled balance while the full [underlyingDeltaSettlement](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1560-L1580) is exported as owner-level credit. The same economic value can then be realized twice across positions over time, draining the market vault.

The PR alters the MM decrease accounting in VTSPositionLib so that [only the actually-queued principal (queuedDelta) is subtracted from the source position’s live settled amount](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1415-L1464), while the remainder ([underlyingDeltaSettlement = immediate settleable slice plus any unqueued shortfall remainder](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1560-L1580)) is [exported to the MMPositionManager as owner-level positive underlying credit via DynamicCurrencyDelta](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/DynamicCurrencyDelta.sol#L130-L209). Because that exported remainder is not removed from the source position’s settled, the same economic value is represented twice: (1) as live settled on the source position and (2) as redeployable owner-level credit. The attacker can [deposit these credits into another active position to increase its settled (via onMMSettle with negative requestedDelta, no token movement)](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/MMPositionActionsImpl.sol#L700-L736), then make that position inactive and withdraw from the vault ([inactive withdrawals are not RFS-gated](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1844-L1854)), and later also make the source position inactive and withdraw its still-intact settled amount. Withdrawals are [per-call clamped to vault liquidity](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1760-L1785), but over time (as the vault refills through normal activity) the attacker can realize both claims in full, causing material loss of principal from the vault.

# Severity

**Impact Explanation:** [High] Direct, material loss of principal from the market vault as the attacker realizes duplicated claims over time.

**Likelihood Explanation:** [High] Exploitation uses standard MM flows (decrease, increase/mint from deltas, burn to inactive, onMMSettle) without special privileges; reliance on vault refilling is typical for active markets.
...
### Scenario 1.
Attacker decreases Position A to create owner-level credits (underlyingDeltaSettlement) while only queuedDelta is removed from A.settled; then deposits those credits into Position C’s settled (no token movement), makes C inactive, and withdraws from the vault over time; later, makes A inactive and withdraws A’s still-high settled, realizing the same value twice.
#### Preconditions / Assumptions
- (a). Attacker controls an MMPositionManager and at least two positions (A and C) in the same market.
- (b). Position C is active initially (non-zero liquidity) so its commitment maxima allows increasing settled.
- (c). The vault has or will have liquidity over time (normal market operation).

### Scenario 2.
Attacker repeats the above pattern across multiple source and target positions, distributing credits into many targets, deactivating them, and withdrawing over time as the vault refills; afterwards, deactivates sources and withdraws their settled as well, amplifying the drain.
#### Preconditions / Assumptions
- (a). Attacker controls multiple source and target positions within the same market.
- (b). Targets can be activated, have settled increased using credits, then deactivated.
- (c). The vault refills over time through normal participant activity.

### Scenario 3.
Attacker immediately withdraws partially from an inactive target position (per-call clamped by current vault liquidity), then repeats withdrawals as liquidity refills to complete extraction; later deactivates the source and performs the same staged withdrawals from it.
#### Preconditions / Assumptions
- (a). At least one target position can be made inactive to allow withdrawals without RFS gating.
- (b). The vault may have limited liquidity immediately but refills over time.
- (c). The source position can later be made inactive for subsequent withdrawals.
...
+            BalanceDelta fullRequiredDelta = requiredSettlementDelta;
...
+            // Remove from live VTS settled the full required amount exported elsewhere (Hub-queued + credits)
...
+                LiquidityUtils.safeInt128ToUint256(fullRequiredDelta.amount0()),
+                LiquidityUtils.safeInt128ToUint256(fullRequiredDelta.amount1())
             );
...
## [Medium] Booking underlyingDeltaSettlement into DynamicCurrencyDelta in VTSPositionLib MM decrease causes end-of-batch revert (DoS)

### Description

The MM decrease path now records [underlyingDeltaSettlement](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1449) (settleable + unqueued remainder) into DynamicCurrencyDelta instead of only the immediately settleable slice, leaving leftover deltas that cannot be cleared in the same batch and causing [assertNonZeroDeltas()](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L58-L59) to revert.

In the MM decrease flow, the PR changes VTSPositionLib to [book requiredSettlementDelta as underlyingDeltaSettlement](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1449) = [required - retainedPrincipal](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1574-L1578) (i.e., settleable amount plus any unqueued shortfall). [Only the queued portion (retainedPrincipal) is removed from live settled immediately](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1440-L1446). The remainder (unavailable to the vault and not queued) is now tracked in DynamicCurrencyDelta for the MMPositionManager owner. During onMMSettle in the same batch, [withdrawals are clamped by actual vault availability](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1765-L1781) and [Phase 4 clears only the portion actually settled](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1816-L1830); any unqueued remainder remains as a positive delta. At batch end, PositionManagerEntrypoint._afterBatch invokes [vtsOrchestrator.assertNonZeroDeltas()](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L58-L59), which reverts because the deltas are nonzero. Prior code recorded only the settleable slice, allowing standard decrease+settle flows to clear deltas fully in-batch. No built-in utility clears the MMPositionManager-owned remainder in-batch ([TAKE targets the locker](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L74-L79), and netProtocolCredits only reduces negative debts), so normal operations can revert under plausible conditions.

### Severity

**Impact Explanation:** [Medium] This is a significant but temporary DoS of core MM operations (decrease+settle, seizure) due to end-of-batch revert; no funds are lost and the invariant is preserved by reverting.

**Likelihood Explanation:** [Medium] Requires plausible but not universal conditions: constrained vault availability and a minted principal smaller than shortfall, which commonly occur under real market conditions.
...
### Scenario 1.
Standard MM decrease followed by onMMSettle in the same batch reverts: the decrease creates a required settlement larger than the vault’s immediate availability, and the minted principal is smaller than the shortfall; the unqueued remainder is added to DynamicCurrencyDelta and cannot be cleared by the subsequent onMMSettle, so end-of-batch [assertNonZeroDeltas()](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L58-L59) reverts the entire batch.
#### Preconditions / Assumptions
- (a). Active MM position with significant settled balances so a decrease creates required settlement vs reduced commitment maxima
- (b). Market vault immediate availability is less than the requested withdrawal (dryModifyLiquidities returns less than requested)
- (c). Minted principal from the remove is smaller than the shortfall, creating an unqueued remainder

### Scenario 2.
Authorized seizure flow reverts: during a seizure-triggered decrease, the queued portion is limited by minted principal and the vault availability is insufficient, leaving an unqueued remainder booked to DynamicCurrencyDelta that onMMSettle cannot clear in-batch; the batch reverts at [assertNonZeroDeltas()](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L58-L59).
#### Preconditions / Assumptions
- (a). Position is seizable (grace elapsed and authorized to seize)
- (b). Market vault immediate availability is less than the requested withdrawal during seizure
- (c). Minted principal during the seizure decrease is smaller than the shortfall, creating an unqueued remainder

### Scenario 3.
One-sided remainder reverts the batch: only one token side has an unqueued remainder after decrease; even if the other side clears in-batch, the global nonzero-delta check still fails and the entire batch reverts.
#### Preconditions / Assumptions
- (a). Decrease produces an unqueued remainder on only one token side (the other side fully clears)
- (b). End-of-batch asserts require all deltas to be zero globally
...
-            // DynamicCurrencyDelta. The remainder (immediate settleable slice plus any unqueued shortfall) stays live.
-            requiredSettlementDelta = decreaseResult.underlyingDeltaSettlement;
+            // Book only the immediately-settleable portion into DynamicCurrencyDelta; any unqueued remainder is not
+            // recorded as a batch-scoped delta to preserve end-of-batch zero-delta invariant.
