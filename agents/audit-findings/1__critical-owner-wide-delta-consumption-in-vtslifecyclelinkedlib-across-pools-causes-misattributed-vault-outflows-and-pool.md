[Critical] Owner-wide delta consumption in VTSLifecycleLinkedLib across pools causes misattributed vault outflows and pool-level solvency breakage

# Description

The PR refactor lets MM withdrawals consume owner-wide underlying credits (DynamicCurrencyDelta) before touching the withdrawing position’s settled balance, while decreases immediately reduce the source position’s settled. Because owner credits are global per underlying (not pool- or position-scoped), credits created in one pool can be withdrawn from another pool’s vault without reducing the withdrawing pool’s settled, misattributing outflows and breaking per-pool solvency/accounting.

The new withdrawal path in VTSLifecycleLinkedLib consumes owner-wide underlying delta (DynamicCurrencyDelta) first and does not reduce the withdrawing position’s settled for the delta-backed slice. Simultaneously, the decrease path in VTSPositionLib now clamps the source position’s settled immediately for the full postDecreaseDelta and records the immediate portion as owner underlying credit. Owner delta is keyed only by (underlying currency, owner), so credits created in pool A can be used to withdraw from pool B’s vault. RFS checks only revert for active && rfsOpen; inactive positions bypass RFS, and active positions with rfsOpen=false can withdraw delta-backed amounts not capped by RFS lane availability. MarketVault.modifyLiquidities then executes the outflow from the target pool’s vault, but no per-pool settled reduction occurs for the delta-backed portion. This misattributes outflows, leaving pool B’s reserves reduced while its per-position and pool-level accounting (settled, CISE/DICE) remain overstated for that outflow.

# Severity

**Impact Explanation:** [High] Pool vaults can be directly drained (material loss of principal for pool participants) and per-pool accounting (settled/coverage) becomes misaligned with actual reserves, breaking core solvency/fairness expectations.

**Likelihood Explanation:** [High] Preconditions are common: shared-underlying pools, attacker-controlled positions, and available vault liquidity. No special external failures or trusted role misuse required; clear economic incentive exists.
...
### Scenario 1.
Cross-pool siphon via inactive position: Attacker decreases in pool A to create owner credit; then withdraws that credit through an inactive position in pool B. Pool B’s vault pays underlying while no B settled is reduced for the delta-backed portion, misattributing outflows and harming pool B participants.
#### Preconditions / Assumptions
- (a). Two pools share the same underlying currency (e.g., USDC)
- (b). Attacker controls MMPositionManager and positions in both pools
- (c). Pool A has a position that can be decreased to generate owner credit
- (d). Pool B has an inactive position (isActive == false)
- (e). Pool B’s MarketVault has available underlying liquidity to withdraw

### Scenario 2.
Bypass RFS withdraw capacity on active position: Attacker creates owner credit in pool A; in pool B, an active position with rfsOpen=false but no RFS withdrawable amount still withdraws delta-backed amounts (not capped by RFS), draining pool B’s vault without reducing its settled for the delta-backed slice.
#### Preconditions / Assumptions
- (a). Two pools share the same underlying currency
- (b). Attacker controls MMPositionManager and positions in both pools
- (c). Attacker has created positive owner underlying delta via a decrease in pool A
- (d). Pool B has an active position with rfsOpen == false and no RFS withdrawable for that lane
- (e). Pool B’s MarketVault has available liquidity

### Scenario 3.
Systematic cross-pool value migration: Attacker iteratively creates owner credits in source pools and withdraws them from target pools with deeper vault reserves using inactive or healthy positions, progressively draining target pools’ reserves while their settled/coverage accounting remains overstated.
#### Preconditions / Assumptions
- (a). Multiple pools share the same underlying currency
- (b). Attacker controls MMPositionManager and positions across these pools
- (c). Source pools allow decreases to create owner credit
- (d). Target pools have available vault liquidity and at least one inactive or healthy (rfsOpen == false) position
- (e). Owner delta is global per underlying, not pool-scoped
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol)
...
+            // Align pool-level accounting: reduce the withdrawing position's settled by the delta-backed slice
+            // so the paying pool's settled and vault reserves remain consistent.
+            VTSPositionLib._sUpdateSettlement(
+                s, positionId, tokenIndex, -deltaBackedWithdrawal.toInt256()
+            );
         }
