[Critical] Path-dependent commitmentMax tracking in VTSPositionLib._trackCommitment causes premature RFS closure and reduced settlement, enabling unauthorized withdrawals/removals and blocking seizure

# Description

[commitmentMax is updated by adding/subtracting per-delta rounded-up maxima](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSPositionLib.sol#L125-L162) instead of recomputing from live liquidity, making it path-dependent. This allows an MM to depress commitmentMax below the true maximum for still-live liquidity, causing RFS to appear closed and reducing required settlement, enabling removals/withdrawals and potentially blocking seizure.

[VTSPositionLib._trackCommitment](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSPositionLib.sol#L125-L162) maintains pa.commitmentMax by adding/subtracting [LiquidityUtils.calculateCommitmentMaxima](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/LiquidityUtils.sol#L74-L78) for each modifyLiquidity delta. That helper [uses Uniswap’s amount delta with roundUp=true](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/LiquidityUtils.sol#L74-L78), which is not additive: ceil(a) + ceil(b) can differ from ceil(a+b). Consequently, commitmentMax becomes path-dependent. With a narrow tick range where per-liquidity maxima are tiny, a position added in one large chunk (setting commitmentMax≈1) and then reduced in a single tiny chunk can subtract 1 due to rounding, collapsing commitmentMax to 0 while most liquidity remains. No code recomputes commitmentMax from live liquidity to correct this. [VTSPositionLib.getRFS relies on commitmentMax (c) to compute base settlement and clamp deficits](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSPositionLib.sol#L1766-L1795); if c is depressed to 0, required settlement becomes 0 and rfsOpen=false even when liquidity and deficits exist. Downstream, remove-liquidity and [MM withdrawals (onMMSettle) only revert when rfsOpen=true](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L446-L452), so with rfsOpen forced false they proceed, allowing the attacker to withdraw/queue underlying and reduce settled prematurely. Additionally, [MM operations mark the RFS checkpoint from current getRFS](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L300-L307); with c=0 this closes the checkpoint (openMask=0), which can prevent seizure via the normal RFS path. No other guard (e.g., commitmentDeficit freeze) corrects this unless a commitment deficit is written via checkpointWithCommitment, which the attacker can avoid by keeping a valid/backed commit.

# Severity

**Impact Explanation:** [High] Allows extraction of principal and premature reduction of live settled while RFS should be open, and can block seizure via normal RFS path. This undermines core settlement/seizure invariants and shifts deficit risk to other participants and coverage.

**Likelihood Explanation:** [High] Preconditions are under attacker control (range choice, chunked modifies); no special external constraints, oracles, or admin actions are required. The effect can be achieved in few steps (often a single tiny removal).
...
### Scenario 1.
Scenario 1: The attacker (MM) adds liquidity in one large chunk on a very narrow range (tiny per-liquidity maxima), setting commitmentMax≈1 per lane. After deficits accrue via swaps, the attacker removes a single tiny amount of liquidity; due to roundUp, this subtracts 1 from commitmentMax and clamps it to 0 while most liquidity remains. On the next decrease, [getRFS computes zero requirement (rfsOpen=false)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSPositionLib.sol#L1766-L1795) and the remove proceeds, exporting/withdrawing value that should have been settlement-gated.
#### Preconditions / Assumptions
- (a). Attacker controls an MM position (can modify liquidity and call MM settle paths)
- (b). Pool baseVTSRate for both tokens is non-zero
- (c). Very narrow tick range so per-liquidity maxima across full range are tiny
- (d). Commit signal remains valid/backed (no stored commitmentDeficit)
- (e). Some cumulativeDeficit has accrued via swaps and growth settlement
- (f). Uniswap v4 behaviors are canonical as assumed

### Scenario 2.
Scenario 2: The attacker (MM) first depresses commitmentMax to 0 using a tiny removal as above. They then call onMMSettle with a positive amountDelta to withdraw. Since getRFS returns rfsOpen=false when c=0, [withdrawals are allowed for an active non-seizing position](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L446-L452). The vault clamps amounts but live settled is still reduced and underlying paid out, bypassing intended RFS settlement gating.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1
- (b). The position holds non-zero live settled (pa.settled > 0)
- (c). Attacker has already depressed commitmentMax to 0 via a tiny removal

### Scenario 3.
Scenario 3: The attacker (MM) depresses commitmentMax to 0 and triggers an MM operation that [marks the RFS checkpoint from current getRFS](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L300-L307). With c=0, openMask=0 is written, closing the checkpoint. Later, a seizer attempts seizure via the normal RFS path (no commitmentDeficit present), but is blocked because the checkpoint is closed, preventing expected seizure despite deficits.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1 except on-settlement needs
- (b). No stored commitmentDeficit (normal RFS-grace seizure path applies)
- (c). Attacker can trigger an MM operation that updates the RFS checkpoint from current getRFS (e.g., modifyLiquidity or onMMSettle)
...
```diff
 ...
 library VTSPositionLib {
     ...
     function _touchExistingDecrease(
         ...
         _trackCommitment(s, positionId, params);
+        // Recompute commitment maxima from live post-modify liquidity to eliminate path-dependent rounding drift
+        {
+            Position memory pos = s.positions[positionId];
+            (uint256 rc0, uint256 rc1) =
+                LiquidityUtils.calculateCommitmentMaxima(pos.tickLower, pos.tickUpper, currentLiq);
+            s.positionAccounting[positionId].commitmentMax.token0 = rc0;
+            s.positionAccounting[positionId].commitmentMax.token1 = rc1;
+        }
