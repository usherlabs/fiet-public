[High] Permissionless settlement of inactive positions in VTSOrchestrator/VTSPositionLib causes skipped DICE slashes and underfunded fee pot

# Description

The PR enabled [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/VTSOrchestrator.sol#L488-L514) to run on inactive (zero-liquidity) positions. When executed, DICE coverage indices are checkpointed without being able to apply fee-burn slashes ([fee burn requires nonzero liquidity](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSFeeLib.sol#L236-L240)), permanently skipping main-coverage slashing for the inactive window and reducing protocol/LP fee distributions.

The PR changed [VTSOrchestrator.settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/VTSOrchestrator.sol#L488-L514) to accept any registered position (inactive included). When called for an inactive position, [VTSPositionLib._settleDICEForToken always checkpoints pa.coverageIndexLastX128 to the current pool coveragePerDeficitIndexX128](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L706-L715), then attempts a fee burn via VTSFeeLib. However, [VTSFeeLib._calculateFeesBurn returns zero whenever positionLiquidity == 0 (fees == 0 by construction)](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSFeeLib.sol#L271-L276), so no slashing is queued. This permanently skips the main DICE coverage window that accrued while the position was inactive. Later reactivation does not recover the skipped window, and [CoreHook._beforeAddLiquidity](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/CoreHook.sol#L93) automatically calls settlePositionGrowths prior to adding liquidity, reinforcing this behavior on reactivation. Meanwhile, coverage increments ([MarketFactory.useMarketLiquidity](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/MarketFactory.sol#L405-L409) → vtsOrchestrator.incrementCoverage → [VTSCommitLib.incrementCoverage](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSCommitLib.sol#L169-L176)) continue to increase the pool’s DICE index independent of a position’s active state. Net effect: positions can avoid main-coverage slashes for inactive periods with outstanding deficit principal, reducing protocolFeeAccrued/slashedPot and harming other participants’ fee/bonus distributions.

# Severity

**Impact Explanation:** [Medium] Skipped main-coverage slashes reduce protocolFeeAccrued/slashedPot and downstream fee/bonus distributions, a direct material loss of yield/fees rather than outright principal loss.

**Likelihood Explanation:** [High] No special constraints are required in common operations: MMs can be inactive during coverage windows, and the hook auto-calls settlement on reactivation. Public calls can also be used to force checkpointing. These are realistic, routine conditions.
...
### Scenario 1.
Scenario 1 (MM self-avoidance): An MM position with nonzero cumulativeDeficit fully removes liquidity (inactive). While inactive, coverage increments increase the pool’s coveragePerDeficitIndexX128. The MM (or anyone) then calls settlePositionGrowths on this inactive position (or later simply reactivates and relies on the hook). _settleDICEForToken checkpoints the main DICE index but, with zero liquidity, VTSFeeLib computes no fees and no slashing is queued. The historical main-coverage window is skipped. When the MM re-adds liquidity, the missed window cannot be charged retroactively.
#### Preconditions / Assumptions
- (a). Pool is unpaused (settlePositionGrowths is permissionless)
- (b). Position is registered and inactive (Uniswap position liquidity == 0)
- (c). Position has nonzero cumulativeDeficit (deficit principal > 0)
- (d). Pool totalDeficitPrincipal > 0 so incrementCoverage raises coveragePerDeficitIndexX128
- (e). Caller can invoke VTSOrchestrator.settlePositionGrowths or rely on later hook call

### Scenario 2.
Scenario 2 (Third-party griefing): A victim MM’s position is inactive with nonzero cumulativeDeficit. After the pool’s coveragePerDeficitIndexX128 increases, an attacker calls settlePositionGrowths on the victim’s position. The DICE index is advanced for the victim while feesBurn == 0 due to zero liquidity, so no slash is recorded. When the victim later reactivates, the historical main-coverage window is already skipped.
#### Preconditions / Assumptions
- (a). Pool is unpaused
- (b). Victim position is registered and inactive (liquidity == 0)
- (c). Victim has nonzero cumulativeDeficit (deficit principal > 0)
- (d). Coverage increment occurred since victim’s last checkpoint
- (e). Attacker can call VTSOrchestrator.settlePositionGrowths on the victim’s position

### Scenario 3.
Scenario 3 (Systemic auto-skip on reactivation): A position was inactive with nonzero cumulativeDeficit during a period when the pool’s main DICE coverage index increased. On reactivation, CoreHook._beforeAddLiquidity automatically calls settlePositionGrowths before liquidity is added, advancing the DICE index while VTSFeeLib yields no burn at zero liquidity. The past main-coverage window is skipped without any explicit attacker action.
#### Preconditions / Assumptions
- (a). Pool is unpaused
- (b). Position is registered and inactive during at least one coverage increment
- (c). Position has nonzero cumulativeDeficit (deficit principal > 0)
- (d). Reactivation path uses CoreHook._beforeAddLiquidity which auto-calls settlePositionGrowths prior to liquidity being added
...
+        // If position has zero live liquidity, defer main DICE settlement to avoid skipping fee burns.
+        // Do NOT advance coverageIndexLastX128 here; leave the window to be settled later when burns can be applied.
+        // Residual coverage above is already banked and safe to checkpoint.
+        if (liq == 0) {
+            return;
+        }
...
## [Medium] Allowing settlePositionGrowths on inactive positions in VTSOrchestrator causes misallocation of protocol fee pots

### Description

The PR enables [settlePositionGrowths to run for inactive positions](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/VTSOrchestrator.sol#L498-L518), which lets parked 'settled' amounts left after MM withdrawal shortfalls accrue CISE exposure and later be converted into [bonuses from protocolFeeAccrued/slashedPot](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSFeeLib.sol#L452-L493), misallocating fees away from active LPs.

The PR broadened [VTSOrchestrator.settlePositionGrowths to accept inactive positions](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/VTSOrchestrator.sol#L498-L518). In the MM decrease path, when vault liquidity is insufficient, [only the immediately available portion of the withdrawal is applied](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L1787-L1814) to the position's pa.settled, while the [shortfall is queued to the locker via LiquidityHub](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L1612-L1637). [Queue collection by the locker does not call back into VTS](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/LiquidityHub.sol#L951-L968) and therefore does not reduce pa.settled. With the PR change, calling settlePositionGrowths on the now-inactive position runs settled-indexed coverage settlement ([_settleSettledIndexedCoverageUsage](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L798-L813)), which accrues CISE exposure based on this parked pa.settled. On a subsequent modify-liquidity touch (e.g., a dust add to reactivate), VTSFeeLib.afterTouchPosition [allocates and materialises a bonus from protocolFeeAccrued/slashedPot](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSFeeLib.sol#L452-L493) using that exposure. This allows an MM to receive protocol-funded bonuses for amounts already paid out-of-band via the Hub queue, misallocating fee/coverage pots away from active, risk-bearing LPs.

### Severity

**Impact Explanation:** [Medium] This causes a direct, material misallocation of fee/coverage pots (protocolFeeAccrued/slashedPot) away from active LPs, reducing their rightful yield and coverage benefits.

**Likelihood Explanation:** [Medium] Exploitation requires timing liquidity removal to moments of reduced vault availability and harvesting when the pot has funds—plausible in live markets but not fully under the attacker’s control.
...
### Scenario 1.
Park then harvest: The attacker fully removes liquidity during a low vault-availability window so that only part of the withdrawal is immediately settleable. The shortfall is queued at LiquidityHub and collected by the locker; pa.settled remains parked. While inactive, settlePositionGrowths realises CISE exposure on the parked settled. A later dust add-liquidity touch materialises a bonus from protocolFeeAccrued/slashedPot.
#### Preconditions / Assumptions
- (a). The PR change enabling settlePositionGrowths on inactive positions is deployed
- (b). Attacker controls an MM commitment NFT (tokenId) and can renew signals (advancer control)
- (c). Vault dryModifyLiquidities returns less than requested during removal (low availability) to create a queued shortfall
- (d). Coverage increments occur over time (nonzero growth in coveragePerSettledIndexX128 and protocol pots)
- (e). ProtocolFeeAccrued/slashedPot has funds at harvest touch time
- (f). Attacker can re-touch (dust add-liquidity) to materialise bonuses

### Scenario 2.
Repeated harvest: The attacker repeats the above across multiple coverage windows, periodically calling settlePositionGrowths while inactive to bank additional CISE exposure. When the pot has funds, a dust add-liquidty touch materialises a larger composite bonus. The process can be repeated.
#### Preconditions / Assumptions
- (a). Same as Scenario 1
- (b). Repeated coverage increments over time
- (c). Attacker can repeatedly call settlePositionGrowths on the inactive position and later re-touch to harvest

### Scenario 3.
Two-lane harvest: The attacker parks pa.settled in both token lanes via a low-availability removal, collects both Hub queues, and accumulates CISE exposure for both tokens while inactive. A dust reactivation then materialises bonuses against both token-side pots, compounding the misallocation.
#### Preconditions / Assumptions
- (a). Same as Scenario 1
- (b). Attacker has nonzero pa.settled in both token lanes or creates it via growth/base settlement
- (c). Coverage increments occur on both token lanes
- (d). Attacker reactivates (dust add-liquidity) to materialise bonuses across both pots
...
-        _settleCISEForToken(s.positionAccounting[positionId], s.poolAccounting[poolId], 0);
-        _settleCISEForToken(s.positionAccounting[positionId], s.poolAccounting[poolId], 1);
+        // Only accrue CISE exposure for active positions. For inactive positions, checkpoint indices only
+        // so reactivation does not inherit historical exposure windows.
+        if (pos.isActive) {
+            _settleCISEForToken(s.positionAccounting[positionId], s.poolAccounting[poolId], 0);
+            _settleCISEForToken(s.positionAccounting[positionId], s.poolAccounting[poolId], 1);
+        } else {
+            PositionAccounting storage pa = s.positionAccounting[positionId];
+            PoolAccounting storage paPool = s.poolAccounting[poolId];
+            uint256 idx = paPool.coveragePerSettledIndexX128.get(0);
+            if (pa.ciseIndexLastX128.get(0) != idx) pa.ciseIndexLastX128.set(0, idx);
+            idx = paPool.coveragePerSettledIndexX128.get(1);
+            if (pa.ciseIndexLastX128.get(1) != idx) pa.ciseIndexLastX128.set(1, idx);
+        }
     }
