[Medium] Live-signal gate on non-seizing settlement in VTSOrchestrator.onMMSettle causes frozen inactive settled remnants and blocked decommit

# Description

[Non-seizing settlement requires a live signal](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/VTSOrchestrator.sol#L723), which prevents [withdrawing settled remnants from inactive positions](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L458-L467) once a commit is [expired or empty-reserve](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L120-L139). [Seizure is disallowed for inactive positions](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/MMPositionActionsImpl.sol#L325-L327) and [decommit is blocked while remnants exist](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/MMPositionManager.sol#L350-L352), leaving funds stuck unless the [advancer renews the signal](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/MMPositionManager.sol#L321).

VTSOrchestrator.onMMSettle enforces a live-signal requirement for non-seizing operations. When a commit becomes non-live (expired or with empty reserves), any ordinary settlement attempts revert. The protocol explicitly allows withdrawing settled remnants from inactive positions, but that path is the same non-seizing settlement and gets blocked by the live-signal gate. Seizure cannot be used because it is disallowed for inactive positions, and decommit is blocked while any inactive position holds nonzero settled amounts (inactiveRemnantCount > 0). There is no alternative owner-controlled path to reduce pa.settled or withdraw the remnants. Renewing the signal to restore the non-seizing path is advancer-gated, so a transferred NFT owner may be unable to recover funds, resulting in frozen settled value and blocked decommit.

# Severity

**Impact Explanation:** [High] Funds (settled remnants) can be indefinitely frozen without an owner-controlled workaround, and decommit is blocked until remnants are drained.

**Likelihood Explanation:** [Low] Exploitation relies on griefing by the advancer or an operator lapse (missed renew), and also on the presence of inactive remnants, making the combined conditions uncommon though plausible.
...
### Scenario 1.
Post-sale denial by malicious advancer: After selling the NFT, the advancer renews the signal to empty reserves. The new NFT owner tries to withdraw settled remnants from an inactive position via non-seizing settlement, which reverts due to the live-signal gate. Seizure is disallowed for inactive positions, and decommit is blocked by inactiveRemnantCount, freezing the funds.
#### Preconditions / Assumptions
- (a). At least one inactive position under the commit has nonzero pa.settled (inactive remnant)
- (b). The commitment NFT has been transferred to a new owner who is not the advancer
- (c). The advancer retains the ability to renew the signal
- (d). The commit is live prior to the advancer’s renewal to empty reserves

### Scenario 2.
Benign expiration before draining: An inactive position still holds settled remnants. The commit naturally expires (no renew). The NFT owner later attempts to withdraw via non-seizing settlement, which reverts because the commit is not live. Seizure is unavailable for inactive positions and decommit remains blocked, freezing the funds unless the advancer renews.
#### Preconditions / Assumptions
- (a). At least one inactive position under the commit has nonzero pa.settled (inactive remnant)
- (b). The commit is initially live
- (c). No one renews the signal before attempting to withdraw
- (d). Time passes until the commit expires

### Scenario 3.
Advancer rotation then non-live: The MM rotates the advancer to a party not controlled by the NFT owner. The commit then becomes non-live (expires or is renewed to empty reserves). The owner cannot renew and cannot use non-seizing settlement due to the live requirement. Seizure is disallowed for inactive positions and decommit is blocked by remnants, leaving funds stuck.
#### Preconditions / Assumptions
- (a). At least one inactive position under the commit has nonzero pa.settled (inactive remnant)
- (b). The advancer is rotated to an address not controlled by the NFT owner
- (c). The commit subsequently becomes non-live (expires or is renewed to empty reserves)
...
+        // Assert commit exists (do not require live yet; see non-seizing withdraw-only inactive exception below)
...
+        } else {
+            // Non-seizing: require live signal unless this is a withdraw-only settle on an inactive position.
+            if (!(amountDelta.amount0() >= 0 && amountDelta.amount1() >= 0 && !s.positions[positionId].isActive)) {
+                _assertSignalValid(commitId, true);
+            }
         }
