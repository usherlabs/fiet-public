[Medium] Permissionless growth settlement with per-call clamping in VTSOrchestrator/VTSPositionLib (unpaused) causes timing-dependent RFS reopen and yield loss

# Description

A public [settlePositionGrowths function](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/VTSOrchestrator.sol#L401-L600) allows third parties to crystallize growth at adversarial times. Because settlement applies deficits first and then [clamps inflow to commitmentMax](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionLib.sol#L201-L260) without carrying excess forward, splitting settlement into multiple calls can reduce the final settled balance versus a single aggregated settle at the victim’s action time, re-opening RFS or increasing deposit requirements.

When unpaused, anyone can call [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/VTSOrchestrator.sol#L401-L600) for any registered position. Internally, per-call settlement [applies deficit growth first](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionLib.sol#L601-L620) (reducing settled or increasing cumulativeDeficit) and then applies inflow, [clamping the new settled amount to the position’s commitmentMax](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionLib.sol#L201-L260). Any inflow beyond the clamp is not carried forward. The growth snapshots are updated each call, so previously clamped inflow is not available later. Owner and MM flows also settle just before gate checks, but only for the period since the last settle ([CoreHook beforeRemoveLiquidity calls](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/CoreHook.sol#L1-L120)). An adversary can settle early (when inflow exists and headroom is small) to clamp away inflow, then ensure a small deficit occurs before the victim’s action. The victim’s settle then applies the new deficit without being able to reclaim the earlier inflow, leaving a lower settled balance than a single aggregated settle would produce. This can re-open RFS and cause remove-liquidity or MM withdrawals to revert ([remove-liquidity RFS check/revert](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionLib.sol#L801-L1000), [MM withdrawal RFS check/revert](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L401-L600)), or force larger deposits to close RFS, and results in direct loss of yield/fees compared to the aggregated baseline.

# Severity

**Impact Explanation:** [Medium] Per-call clamping permanently discards inflow above headroom (direct, material loss of yield/fees) and can re-open RFS at execution time, causing significant but temporary DoS (reverts) for remove-liquidity and MM withdrawals.

**Likelihood Explanation:** [Medium] Exploitation requires mempool timing/front-running and often a small swap to create a post-early-settle deficit. These are realistic in active DeFi and can be incentivized (e.g., blocking competitor operations or forcing larger deposits).
...
### Scenario 1.
Front-run remove-liquidity to force revert and cause yield loss: the attacker settles early to clamp inflow at commitmentMax, induces a small post-settlement deficit, and the victim’s pre-modify settle applies the deficit, re-opening RFS; remove-liquidity reverts and earlier inflow above headroom is permanently lost.
#### Preconditions / Assumptions
- (a). VTS and the target pool are unpaused
- (b). A registered, active position with known PositionId
- (c). The position’s settled amount on a lane is close to commitmentMax (small headroom)
- (d). Since the last snapshot, net inflow accrued on that lane with little/no deficit
- (e). The victim’s remove-liquidity action is pending (visible in mempool or predictable)
- (f). Attacker can front-run and optionally execute a small reverse-direction swap to introduce a post-early-settle deficit before the victim’s transaction

### Scenario 2.
Block MM withdrawal (onMMSettle) and cause yield loss: the attacker settles early to clamp inflow and introduces a small post-settlement deficit; onMMSettle’s pre-check settle applies the deficit, RFS is open, and withdrawal reverts; the clamped-away inflow remains lost compared to a single aggregated settle.
#### Preconditions / Assumptions
- (a). VTS and the target pool are unpaused
- (b). A registered, active position with known PositionId
- (c). The position’s settled amount on a lane is close to commitmentMax (small headroom)
- (d). Since the last snapshot, net inflow accrued on that lane with little/no deficit
- (e). The victim is about to execute onMMSettle with a withdrawal (positive lane amount)
- (f). Attacker can front-run and optionally execute a small reverse-direction swap to introduce a post-early-settle deficit before the victim’s transaction

### Scenario 3.
Increase immediate deposit required (economic disadvantage): the attacker settles early to clamp inflow and ensures a small post-settlement deficit; the victim’s deposit/settlement call now sees a larger RFS delta (since settled is lower), forcing a larger deposit than would be needed with a single aggregated settle, and the earlier clamped inflow represents lost yield.
#### Preconditions / Assumptions
- (a). VTS and the target pool are unpaused
- (b). A registered, active position with known PositionId
- (c). The position’s settled amount on a lane is close to commitmentMax (small headroom)
- (d). Since the last snapshot, net inflow accrued on that lane
- (e). The victim intends to deposit/close RFS via settlement
- (f). Attacker can front-run and optionally execute a small reverse-direction swap to introduce a post-early-settle deficit before the victim’s transaction
...
-            }
+            // Always restrict settlement to the canonical CoreHook for the pool (unpaused or paused).
+            // This prevents arbitrary third parties from crystallising growth at adversarial times.
+            Pool memory pool = s.pools[poolId];
+            IMarketFactory factory =
+                liquidityHub.getFactory(Currency.unwrap(pool.currency0), Currency.unwrap(pool.currency1));
+            MarketHandlerLib.assertCoreHook(factory, _msgSender());
...
-            settlePositionGrowths(positionId);
+            // no-op: public settlement restricted to CoreHook; compute checkpoint from current stored accounting
         }
     }
