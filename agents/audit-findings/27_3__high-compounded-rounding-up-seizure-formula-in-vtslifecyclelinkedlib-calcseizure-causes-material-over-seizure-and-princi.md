[High] Compounded rounding-up seizure formula in VTSLifecycleLinkedLib._calcSeizure causes material over-seizure and principal loss

# Description

The seizure sizing formula rounds up multiple times per lane, applies per-token base floors, sums lanes against full liquidity, and can [snap to full close](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L678-L686). This can heavily overestimate seizedLiquidityUnits for dust cures, and that value is used to actually [remove liquidity and route principal to the seizer](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/MMPositionActionsImpl.sol#L334-L353).

In [VTSLifecycleLinkedLib._calcSeizure](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L646-L663), per-lane [exposureBps](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/LiquidityUtils.sol#L82-L87) = ceil(rfs_pre/commitment*10000) and [settleOfRfsBps](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/LiquidityUtils.sol#L94-L100) = ceil(s/rfs_pre*10000) are computed, then multiplied and rounded up again via [LiquidityUtils.seizedUnitsFromBps](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/LiquidityUtils.sol#L102-L110) to produce ceil(liquidity * fracBps/10000). Each lane is computed against the full position liquidity and then summed. Additionally, exposureBps is [floored to baseVTSRate per token](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L646-L663) when configured, and the result can [snap to full seizure if residual liquidity < minResidualUnits](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L678-L686). Any non-zero RFS lane and any non-zero deposit on that lane produce at least 1 bp of per-lane seizure after compounded rounding, which can be very large relative to the exact fractional benchmark L*s/C for dust repairs. seizedLiquidityUnits is then used by [MMPositionActionsImpl._seizePosition → _decreaseInternal](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/MMPositionActionsImpl.sol#L334-L353) to perform a Uniswap v4 decrease. In [VTSPositionMMOpsLib._handleSeizureLiquidityDecrease](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionMMOpsLib.sol#L646-L667), the decrease’s principal is routed so that only the burn slice covers 'excess settled', and the retained principal (principal − burn) is queued to the seizer. Seizure authorization (CheckpointLibrary.isSeizable) only gates timing/eligibility and does not mitigate the sizing bias once allowed.

# Severity

**Impact Explanation:** [High] Excessive seizedLiquidityUnits are used to perform real Uniswap v4 liquidity decreases and route the resulting LCC principal to the seizer (minus the burn for 'excess settled'), causing direct, material loss of principal for the victim.

**Likelihood Explanation:** [Medium] Exploitation requires the position to be seizable (grace elapsed or deficit-bypass) and to have small RFS or modest cure share; these are realistic states in operation, but not universally present at all times.
...
### Scenario 1.
Dual-lane dust RFS on a large position: With L=1,000,000 and c0=c1=1e12, r0_pre=r1_pre=1, and deposits s0=s1=1, exposureBps=1 bp and phi=10000 bp per lane yield fracBps=1 bp; seized units per lane = ceil(1,000,000/10000)=100, totaling 200 units. The Uniswap v4 decrease mints principal; with little/no 'excess settled', most or all principal is queued to the seizer despite dust deposits.
#### Preconditions / Assumptions
- (a). Victim has an active MM position with large liquidity (e.g., L=1,000,000) and very large commitment maxima (c0=c1≈1e12).
- (b). Pre-seizure RFS on both lanes is dust-sized but positive (r0_pre=r1_pre>0).
- (c). CheckpointLibrary.isSeizable passes (grace elapsed or deficit bypass).
- (d). baseVTSRate is 0 or low; minResidualUnits=0 (not required but simplifies).
- (e). Position’s 'excess settled' under a small decrease is negligible.
- (f). Underlying tokens are standard ERC20; attacker has sufficient allowances for deposits.
- (g). Attacker is not the commit NFT owner or approved operator (per _seizePosition check).

### Scenario 2.
BaseVTSRate floor amplifies seizure: With baseVTSRate=2000 bps per token and curing a modest share (phi=500 bps, i.e., 5% of RFS), each lane seizes about 1% of L (fracBps=100 bps), summing to ~2% of L. The resulting principal from the decrease is largely queued to the seizer when 'excess settled' is small.
#### Preconditions / Assumptions
- (a). Victim position is seizable (CheckpointLibrary.isSeizable passes).
- (b). baseVTSRate per token is configured (e.g., 2000 bps).
- (c). RFS per lane is small relative to commitment so exposureBps is floored to baseVTSRate.
- (d). Attacker cures a modest fraction of RFS (e.g., phi≈5%).
- (e). Position’s 'excess settled' created by the small decrease is limited.
- (f). Underlying tokens are standard ERC20; attacker has deposit allowances.
- (g). Attacker is not the commit NFT owner or approved operator.

### Scenario 3.
Small-L with minResidualUnits snap-to-full: With L=3, minResidualUnits=2, and dust dual-lane RFS and deposits, each lane seizes ceil(3/10000)=1 unit due to rounding; total=2, residual=1<2 triggers snap to full (3 units). The entire position is seized; any non-zero principal from removing 3 units is queued to the seizer.
#### Preconditions / Assumptions
- (a). Victim position has small liquidity (e.g., L=3).
- (b). minResidualUnits is set (e.g., 2).
- (c). Dual-lane dust RFS is open and the position is seizable.
- (d). baseVTSRate=0 (not required but illustrates minimal per-lane effect).
- (e). Underlying tokens are standard ERC20; attacker has deposit allowances.
- (f). Attacker is not the commit NFT owner or approved operator.
...
+            if (r0pre > 0 && s0 > 0) {
+                uint256 u0a = c0 > 0 ? FullMath.mulDivRoundingUp(liq, s0, c0) : 0;
+                uint256 u0b = cfg.token0.baseVTSRate > 0 ? FullMath.mulDivRoundingUp(FullMath.mulDiv(liq, cfg.token0.baseVTSRate, LiquidityUtils.BPS_DENOMINATOR), s0, r0pre) : 0;
+                total += u0a > u0b ? u0a : u0b;
             }
-            if (r1pre > 0) {
...
+            if (r1pre > 0 && s1 > 0) {
+                uint256 u1a = c1 > 0 ? FullMath.mulDivRoundingUp(liq, s1, c1) : 0;
+                uint256 u1b = cfg.token1.baseVTSRate > 0 ? FullMath.mulDivRoundingUp(FullMath.mulDiv(liq, cfg.token1.baseVTSRate, LiquidityUtils.BPS_DENOMINATOR), s1, r1pre) : 0;
+                total += u1a > u1b ? u1a : u1b;
             }
         }
