[High] Missing per-owner latest-commit enforcement and aggregate VRL backing in VTS commit/validation causes reserve double counting and principal loss

# Description

Commits for the same market-maker owner remain concurrently live and [per-position backing checks are commit-local](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSCommitLib.sol#L181-L188), allowing the same VRL reserve set to be counted [multiple times across positions](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionLib.sol#L662-L684) and overlapping commits. This enables over-issuance and reserve drain, resulting in material principal loss for other users.

New commits are [created and stored](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSCommitLib.sol#L280-L287) without invalidating older commits for the same owner. [Commit liveness checks](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L50-L159) only require that a commit exists, has a non-zero owner, is unexpired, and has non-empty reserves; they do not enforce that a commit must be the latest (highest nonce) for that owner. Backing checks at add-liquidity time are [per-position and compare the position’s issued value only against that single commit’s signal value plus the position’s own settled amounts](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSCommitLib.sol#L181-L188). There is no [aggregation across positions](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSPositionLib.sol#L662-L684) or across multiple commits of the same owner. As a result, the same VRL reserve set can be re-applied for each position, and even across overlapping commits, causing systemic over-allocation of reserves. Attackers can open multiple positions that each pass local backing checks, then decrease and [unwrap to extract underlying](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/LiquidityHub.sol#L596-L619) or accumulate settlement queues serviced by CanonicalVault/Hub, leading to direct, material loss of principal for other users.

# Severity

**Impact Explanation:** [High] The flaw enables direct, material loss of principal by allowing over-allocation and extraction of underlying reserves (or outsized settlement queues) beyond true off-chain VRL backing, harming other users’ ability to redeem or settle.

**Likelihood Explanation:** [Medium] Exploitation depends on plausible operational conditions (e.g., available reserves, VRL expiry overlap, and market configuration such as baseVTSRate). No cryptographic breaks or trusted-role misuse are required, but practical extraction is constrained by configuration and reserve availability.
...
### Scenario 1.
Single commit, multiple positions: The attacker commits once (valid VRL reserves V USD), mints multiple positions under the same commit where each position’s issued value is individually ≤ V, then decreases and unwraps LCC to underlying. Because backing checks are per-position and commit-local, the same reserves are counted multiple times, allowing outsized reserve extraction and queue accumulation that harms other users.
#### Preconditions / Assumptions
- (a). Attacker is a market-maker using MMPositionManager and has a valid VRL-backed commit with sufficient expiry and reserves of value V USD.
- (b). Market configuration has low or near-zero baseVTSRate so RFS remains closed after adds.
- (c). Hub/CanonicalVault hold non-trivial reserves to fund withdrawals or settlement.
- (d). Oracle prices are accurate; Uniswap v4 semantics are canonical; trusted roles behave as assumed.

### Scenario 2.
Two overlapping commits for the same owner: The attacker creates commit C1 (V USD) and, before it expires, creates commit C2 with a higher VRL nonce but similar reserves. Both commits remain valid concurrently. The attacker opens positions under C1 and then under C2, each time comparing issued value only to the respective commit’s signal value. Aggregate exposure across C1 and C2 can greatly exceed actual reserves; subsequent decreases and unwraps drain underlying and push others into queues.
#### Preconditions / Assumptions
- (a). Two VRL commits for the same owner overlap in validity (older commit not expired when newer commit is created).
- (b). Per-position add-liquidity checks use commit-local signal value; there is no owner-global latest-only enforcement.
- (c). Sufficient Hub/CanonicalVault reserves exist to support withdrawals or settlement.
- (d). Oracle prices are accurate; Uniswap v4 semantics are canonical; trusted roles behave as assumed.

### Scenario 3.
Many positions under a single commit with base settlement funded by the market vault: The attacker creates one commit (V USD), repeatedly opens positions sized so each passes local backing checks. Base settlement for each add is sourced from MarketVault reserves (when configured), gradually consuming available liquidity. Later, the attacker decreases positions and unwraps to extract underlying or accumulate queue claims that CanonicalVault will try to service, reducing liquidity available to others.
#### Preconditions / Assumptions
- (a). A single valid commit exists with reserves V USD.
- (b). MarketVault has sufficient reserves to fund base settlement for multiple adds.
- (c). Per-position backing checks are commit-local; there is no commit-wide/owner-wide VRL budget aggregation.
- (d). Oracle prices are accurate; Uniswap v4 semantics are canonical; trusted roles behave as assumed.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol)
...
```diff
 ...
 library VTSLifecycleLinkedLib {
     ...
     {
+        // TODO(security): For requireLiveSignal=true, also enforce a latest-by-nonce rule:
+        // `commit.vrlNonce == signalManager.mmNonce(commit.mmState.owner)`. Either pass `signalManager` in context or
+        // route this check through the orchestrator where `signalManager` is available.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/e8ef35fa5e0602b949585d8caf092ac0ba34595f/contracts/evm/src/types/Commit.sol)
...
+    // TODO(security): Add `uint256 vrlNonce;` and `uint256 consumedUsd;` to (1) enforce latest-by-nonce liveness for
+    // requireLiveSignal flows and (2) aggregate and cap per-commit reserve consumption across all positions.
 }
```
...
+        // TODO(security): When requireLiveSignal is true, also require that
+        // `s.commits[commitId].vrlNonce == signalManager.mmNonce(s.commits[commitId].mmState.owner)` so only the
+        // latest (by VRL nonce) commit is treated as live.
...
+        // TODO(security): Persist `LiquiditySignal.nonce` into `s.commits[commitId].vrlNonce` so latest-by-nonce checks
+        // can treat older commits as non-live in all requireLiveSignal flows.
         LiquiditySignal memory signal = abi.decode(liquiditySignal, (LiquiditySignal));
...
+        // TODO(security): On renew, update `s.commits[commitId].vrlNonce` from `LiquiditySignal.nonce` and preserve
+        // latest-only semantics; do not rotate `authorisedRelayer`.
         LiquiditySignal memory signal = abi.decode(liquiditySignal, (LiquiditySignal));
         Commit storage commit = s.commits[commitId];
         // Invariants:
     ...
     function _checkpointWithCommitment(
         ...
         }
         {
             ...
             Commit storage commit = s.commits[commitId];
+            // TODO(security): Also treat commits as having zero backing when they are NOT latest-by-nonce for the owner
+            // (commit.vrlNonce != signalManager.mmNonce(commit.mmState.owner)), even if not expired, for all
+            // commitment-backed checkpoint semantics.
