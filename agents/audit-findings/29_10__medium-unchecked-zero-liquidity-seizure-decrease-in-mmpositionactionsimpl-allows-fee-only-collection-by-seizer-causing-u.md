[Medium] Unchecked zero-liquidity seizure decrease in MMPositionActionsImpl allows fee-only collection by seizer causing unauthorized loss of accrued LCC fees

# Description

During seizure, MMPositionActionsImpl._seizePosition unconditionally calls a decrease with seizedLiquidityUnits even when it is zero. This triggers a zero-liquidity modify that can still realize accrued LCC fees to the locker (the seizer), enabling fee theft without seizing liquidity.

In the seizure flow, MMPositionActionsImpl._seizePosition first invokes settlement to size the seizure, then immediately calls _decreaseInternal with the returned seizedLiquidityUnits. There is no guard for seizedLiquidityUnits == 0. As a result, _decreaseInternal proceeds to PositionManagerImpl._modifySyntheticLiquidity with liquidityDelta = 0. The implementation and comments explicitly acknowledge that zero-liquidity modifies can still return positive callerDelta when fees are accrued. The subsequent settlement logic settles positive LCC deltas from PoolManager to the MM router and credits the locker (msgSender) via balance-to-delta sync. In a seizing context, the locker is the seizer, not the victim position owner. Because a zero-liquidity modify does not stage planned cancels, no LCC is forwarded to custody (qCommitted == 0), and the entire fee receipt remains as the seizer's positive delta, which the seizer can TAKE within the same batch to extract the tokens and still satisfy the batch's NonzeroDeltaCount == 0 invariant. This allows a third-party seizer to harvest a victim’s accrued LCC fees whenever the position is seizable, even if the accepted settlement for seizure was clamped to zero and no liquidity is actually seized.

# Severity

**Impact Explanation:** [Medium] The attack causes a direct, material loss of the victim’s fee yield (accrued LCC fees) without affecting principal or core invariants.

**Likelihood Explanation:** [Medium] Exploitation requires the position to be seizable and to have accrued fees—conditions outside the attacker’s full control but plausible and recurrent in practice. No privileged roles or user mistakes are required.
...
### Scenario 1.
Single-lane zero-liquidity seizure fee harvest: The attacker initiates SEIZE_POSITION on a victim’s seizable position, proposing settlement only on a closed lane so settlement clamps to zero and seizedLiquidityUnits == 0. The subsequent zero-liquidity decrease collects accrued LCC fees to the attacker’s credit, which the attacker TAKES in the same batch.
#### Preconditions / Assumptions
- (a). Victim position is active and seizable (grace elapsed or deficit-bypass conditions met).
- (b). There are accrued LCC fees on the position (from prior swaps).
- (c). Attacker can submit a seizing batch via the bound router (normal seizure authorization).
- (d). Attacker proposes settlement on a lane with no open RFS demand so accepted settlement is clamped to zero.

### Scenario 2.
Repeated fee harvesting over time: Each time the victim’s position becomes seizable and fees accrue, the attacker repeats the zero-liquidity seizure path to drain the newly accrued LCC fees without seizing any liquidity.
#### Preconditions / Assumptions
- (a). Victim position periodically becomes seizable (due to lifecycle/operations).
- (b). Fees accrue between attacker runs (active market).
- (c). Attacker can monitor and repeatedly execute the zero-liquidity seizure path.

### Scenario 3.
Dual-lane opportunism: The attacker selects whichever lane currently has no RFS demand to force settlement to zero and seizedLiquidityUnits == 0, then uses the zero-liquidity modify to siphon accrued LCC fees and TAKE them out.
#### Preconditions / Assumptions
- (a). Victim position is seizable and at least one lane has no RFS demand at the moment of attack.
- (b). There are accrued LCC fees to collect.
- (c). Attacker proposes settlement on the lane with no RFS demand to keep settlement at zero.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/MMPositionActionsImpl.sol)
...
+        // No liquidity to seize: skip zero-liquidity modify to prevent fee-only collection in seizing context.
+        if (seizedLiquidityUnits == 0) {
+            // Clear seizing context early to avoid ambient authority for subsequent actions in this batch.
