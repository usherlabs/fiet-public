[High] Multi-stage upward rounding in seizure sizing in VTSLifecycleLinkedLib._calcSeizure causes over-seizure and attacker value capture

# Description

Seizure sizing rounds up at multiple intermediate steps per token lane, guaranteeing at least 1 bp of liquidity is seized for any strictly positive deposit. Repeating micro-deposits lets an attacker drain a seizable position and capture most LCC from paired decreases.

In VTSLifecycleLinkedLib._calcSeizure, the per-lane seizure fraction is computed via LiquidityUtils.exposureBps(rfs_pre, commitment) and LiquidityUtils.settleOfRfsBps(s, rfs_pre), both using rounding-up. Their product is again rounded up, and the conversion to seized liquidity units is rounded up once more. Per-lane seized units are then summed. For any s > 0 on a lane with rfs_pre > 0, this guarantees at least ceil(liquidity/10,000) units are seized on that lane in that transaction, regardless of the true proportional fraction. Because rfsPre is the pre-deposit snapshot, splitting a cure into many tiny deposits preserves the per-tx minimum. MMPositionActionsImpl._seizePosition applies these seized units via _decreaseInternal, and VTSPositionMMOpsLib._handleSeizureLiquidityDecrease then routes the minted LCC from the decrease to the attacker (locker) except for a burn slice capped by the lane’s excess settled, which is typically small or zero for micro-decreases. Summation across lanes doubles the per-tx seizure when both lanes deposit. The minResidualUnits snap-to-full can finalize full seizure when the remainder falls below the configured threshold.

# Severity

**Impact Explanation:** [High] Direct, material loss of principal liquidity units from the victim’s position with most LCC from decreases routed to the attacker; loss of future yield as positions are drained.

**Likelihood Explanation:** [Medium] Exploitation requires the position to be seizable, a meaningful precondition outside the attacker’s control but plausible; once seizable, the path is straightforward and economically incentivized.
...
### Scenario 1.
Single-lane micro-deposit repeated drain: The attacker repeatedly seizes a victim’s seizable position by depositing 1 wei on one open RFS lane. Each tx seizes ≈1 bp of liquidity and, due to minimal excess settled, most of the minted LCC from the decrease is queued to the attacker.
#### Preconditions / Assumptions
- (a). Victim position is seizable (ordinary RFS path with elapsed grace or commitment-deficit bypass).
- (b). Position has substantial liquidity (so 1 bp is material).
- (c). Standard ERC20 behavior and canonical Uniswap v4 semantics.
- (d). Attacker can call MMPositionManager seizure (third-party; not NFT owner/approved).

### Scenario 2.
Two-lane micro-deposits: The attacker deposits 1 wei on both lanes per tx. Each tx seizes ≈2 bps (≈1 bp per lane) and queues most of the minted LCC from both lanes to the attacker.
#### Preconditions / Assumptions
- (a). Both token lanes are RFS-open and the position is seizable.
- (b). Position has substantial liquidity.
- (c). Standard ERC20 behavior and canonical Uniswap v4 semantics.
- (d). Attacker can call MMPositionManager seizure (third-party; not NFT owner/approved).

### Scenario 3.
Snap-to-full at tail: After many micro-seizures leave a small remainder, one more micro-deposit triggers the minResidualUnits check, snapping the remainder to a full seize in a single tx.
#### Preconditions / Assumptions
- (a). Position is seizable and near-drained after prior micro-seizures.
- (b). MarketVTSConfiguration.minResidualUnits > 0 (defaults to 1 when 0).
- (c). Standard ERC20 behavior and canonical Uniswap v4 semantics.
- (d). Attacker can call MMPositionManager seizure (third-party; not NFT owner/approved).
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/LiquidityUtils.sol)
...
+        uint256 fracBps = FullMath.mulDiv(exposureBps_, settleOfRfsBps_, BPS_DENOMINATOR);
+        return FullMath.mulDiv(liquidityUnits, fracBps, BPS_DENOMINATOR);
     }
