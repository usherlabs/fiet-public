[Medium] Lack of commit-bucket custody reconciliation when Hub settleQueue decreases causes permanent stuck LCC in MMQueueCustodian

# Description

Commit-bucket (tokenId > 0) LCC forwarded to MMQueueCustodian can become permanently stuck if the beneficiary’s Hub queue is later reduced or zeroed, because [LiquidityHub.settleFromCustodian caps releases by the live settleQueue](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/LiquidityHubLib.sol#L700-L706) and [MMPositionManager only reconciles the utility bucket (tokenId == 0)](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/MMPositionManager.sol#L600-L606), leaving no path to release commit-bucket “excess” custody.

During MM decreases, a beneficiary’s Hub queue is increased and matching commit-bucket custody is [recorded in MMQueueCustodian for tokenId > 0](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/MMPositionActionsImpl.sol#L120-L128). Later, if the beneficiary reduces their Hub queue (e.g., by transferring LCC from a non-protocol address to a protocol endpoint, which [annuls queued claims](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/LCC.sol#L224-L232)), [LiquidityHub.settleFromCustodian strictly caps release by the live settleQueue](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/LiquidityHubLib.sol#L700-L706). When the queue is zero (or reduced below custody), settleFromCustodian returns 0 and no release occurs. MMPositionManager [explicitly reconciles only the utility bucket (tokenId == 0)](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/MMPositionManager.sol#L600-L606) by returning any excess custody above the live queue to the beneficiary; there is no analogous reconciliation for commit buckets. In realistic end states (no open positions, no positive deltas, only direct-supply LCC available, and [non-protocol→non-protocol LCC transfers blocked](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/LCC.sol#L196-L200)), the beneficiary cannot re-open a Hub queue ([direct-supply unwraps cannot create shortfall/queue](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/LiquidityHubLib.sol#L408-L424)), leaving the commit-bucket LCC permanently locked in the custodian without a practical on-chain recovery path.

# Severity

**Impact Explanation:** [High] Funds (commit-bucket LCC) can be frozen/blocked indefinitely with no practical on-chain workaround in many realistic end states.

**Likelihood Explanation:** [Low] The worst-case permanent lock requires compounded, user-driven conditions (creating commit custody, later self-annulling the queue via substantial non-protocol→protocol transfers, and lacking positions/deltas or other means to re-open a queue). These constraints are plausible but not ubiquitous.
...
### Scenario 1.
Self-queue paydown: After MM decreases created a Hub queue and commit custody (tokenId > 0), the beneficiary later wraps underlying to mint LCC and transfers it from their EOA (non-protocol) to a protocol endpoint. This [triggers queue annulment](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/LCC.sol#L224-L232), potentially zeroing settleQueue. A subsequent COLLECT_AVAILABLE_LIQUIDITY (tokenId > 0) settles 0 due to queue==0, leaving the commit-bucket custody permanently stuck.
#### Preconditions / Assumptions
- (a). Commit-bucket custody (tokenId > 0) exists from prior MM decreases (qCommitted).
- (b). Beneficiary later holds ERC20 LCC in their EOA (e.g., newly wrapped) and transfers it to a protocol endpoint, triggering annulSettlementBeforeTransfer.
- (c). Resulting settleQueue[lcc][beneficiary] is reduced to 0 or below custody.
- (d). Beneficiary has no open positions or positive deltas to re-open a Hub queue.
- (e). Beneficiary holds only direct-supply LCC; non-protocol→non-protocol transfers are blocked, preventing sourcing market-derived LCC from peers.

### Scenario 2.
Seizure-flow custody stuck: A seizure decrease creates commit-bucket custody and a Hub queue for the seizer. Later, the seizer reduces/zeros their queue via a non-protocol→protocol LCC transfer. COLLECT_AVAILABLE_LIQUIDITY then settles 0 due to queue==0, permanently locking the seized-principal custody.
#### Preconditions / Assumptions
- (a). Seizure decrease created commit-bucket custody and a Hub queue for the seizer.
- (b). Seizer subsequently reduces/zeros their Hub queue via a non-protocol→protocol LCC transfer.
- (c). No open positions or positive deltas remain to re-open the queue.
- (d). Only direct-supply LCC available; cannot create shortfall via unwrap; non-protocol→non-protocol LCC transfers blocked.

### Scenario 3.
Utility-unwrap bootstrap fails: With commit-bucket custody and a near-zero queue, the beneficiary tries to create fresh queue via UNWRAP_LCC. Holding only direct-supply LCC, [unwrap fully redeems against directSupply and does not queue shortfall](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/LiquidityHubLib.sol#L408-L424). COLLECT_AVAILABLE_LIQUIDITY (tokenId > 0) still settles 0, leaving custody stuck.
#### Preconditions / Assumptions
- (a). Commit-bucket custody (tokenId > 0) exists and live Hub queue is near zero or zero.
- (b). Beneficiary has no positions or positive deltas.
- (c). Beneficiary only holds direct-supply LCC; unwrap fully redeems against directSupply and does not create queue.
- (d). Non-protocol→non-protocol LCC transfers are blocked, preventing acquiring market-derived LCC to induce shortfall.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/MMPositionManager.sol)
...
+        // Reconcile any excess custodied LCC over the live Hub queue for all buckets (utility and commit).
...
+            queueCustodian.release(tokenId, lcc, locker, custodied - hubQueued);
         }
