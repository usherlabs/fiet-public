[Medium] Permissionless growth settlement with unconditional snapshot advancement in VTSPositionLib causes under-attribution of deficits and premature withdrawals/reduced seizure

# Description

When [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L709-L712) is called permissionlessly, VTSPositionLib [advances per-position growth snapshots](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L561-L568) even when the computed per-position growth floors to zero, permanently discarding sub-unit growth. This under-attributes swap-driven deficits, reducing RFS and seizure sizing, enabling earlier withdrawals or smaller seizures than warranted.

In VTSPositionLib._deltaAndCheckpointGrowth, per-position growth is computed as [add = floor((insideNow - lastSnap) * liquidity / Q128)](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L575-L579). The function then [unconditionally updates pa.deficitGrowthInsideLast and pa.inflowGrowthInsideLast to insideNow](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L561-L568), even when add == 0. Because VTSOrchestrator.settlePositionGrowths is permissionless while unpaused, any caller can repeatedly force settlement after tiny swaps so that add floors to zero each time while [snapshots advance](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L561-L568), permanently discarding fractional growth. For deficits, _settlePositionDeficitGrowth [only increases pa.cumulativeDeficit (and pool totalDeficitPrincipal) when add > 0](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L632-L635). Discarded intervals therefore understate cumulativeDeficit. getRFS uses cumulativeDeficit (capped by commitment) and base VTS to compute required settlement; under-attribution depresses RFS, potentially allowing active, non-seizing withdrawals that should be gated. During seizure, rfsPreForSeizure (taken immediately after growth settlement) drives exposure and seized units; depressing rfsPre reduces seizure severity (subject to baseVTSRate and minResidualUnits floors). Withdrawals remain bounded by IMarketVault.dryModifyLiquidities, so this is a material economic mis-accounting rather than an unbounded immediate drain, but it enables premature withdrawals or smaller seizures versus true exposure.

# Severity

**Impact Explanation:** [Medium] The bug causes significant economic mis-accounting: under-attributed swap-driven deficits depress RFS and reduce seizure sizing, enabling earlier withdrawals or smaller seizures than warranted. However, withdrawals remain bounded by vault availability (dryModifyLiquidities), so it is not an immediate unbounded principal drain.

**Likelihood Explanation:** [Medium] Exploitation requires repeated small swaps and repeated settlement calls while the position is in-range. This is feasible and rational for an MM (clear incentive), though not trivial and carries swap costs; thus medium likelihood.
...
### Scenario 1.
Active-position withdrawals allowed prematurely: An attacker performs many tiny swaps while the victim’s position is in-range and calls [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L709-L712) after each. Each interval’s per-position growth floors to zero, but [snapshots advance](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L561-L568). Later, the victim calls onMMSettle to withdraw. Because cumulativeDeficit remains understated and base is already met, getRFS indicates rfsOpen=false. VTSLifecycleLinkedLib._executeWithdrawals proceeds (active, non-seizing, rfsOpen=false) and the vault pays out any available liquidity (bounded by dryModifyLiquidities), reducing pa.settled.
#### Preconditions / Assumptions
- (a). Pool and position are unpaused (permissionless settlePositionGrowths is callable).
- (b). Victim’s position is registered, active, and in-range during attacker’s swaps.
- (c). Attacker can perform small swaps and repeatedly call settlePositionGrowths.
- (d). Victim’s effective settled meets base settlement; no large pre-existing cumulative deficits that would keep RFS open regardless.

### Scenario 2.
Amplified under-attribution via liquidity fragmentation: The MM splits liquidity into many small positions and (by themselves or via third parties) repeats the tiny-swap + [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L709-L712) pattern on each. Because flooring is applied per position per interval, much more growth is discarded in aggregate. Later withdrawals across these positions see depressed RFS and are allowed earlier/more than warranted by true exposure (still bounded by vault availability), shifting principal shortfall to the system.
#### Preconditions / Assumptions
- (a). Same as Scenario 1, and the MM splits liquidity across many small positions in ranges covering current tick.
- (b). Attacker/MM repeatedly executes small swaps and settles growth per position after each small interval.

### Scenario 3.
Reduced seizure severity: Just before a seizer calls onMMSettle with isSeizing=true, the MM (or aligned party) executes many tiny swaps and repeatedly [settles growth](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol#L709-L712). rfsPreForSeizure (captured immediately after growth settlement) is reduced, so _calcSeizure computes fewer seized units (subject to baseVTSRate and minResidualUnits). The MM loses less liquidity than warranted by actual swap-driven outflows.
#### Preconditions / Assumptions
- (a). Pool and position are unpaused; target position is near or subject to seizure (grace expired).
- (b). MM (or aligned party) can execute small swaps and call settlePositionGrowths repeatedly immediately before seizer’s transaction.
- (c). Seizure paths and floors (baseVTSRate, minResidualUnits) configured as in deployment.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/f5235186da277999275f3a0a6ae3cadd91ddf1e8/contracts/evm/src/libraries/VTSPositionLib.sol)
...
+
+        // Advance snapshots per lane only when rebasing on zero-liquidity or when at least 1 raw unit is attributed.
+        if (!p.isInflow) {
+            if (p.liquidity == 0 || add0 > 0) pa.deficitGrowthInsideLast.token0 = inside0;
+            if (p.liquidity == 0 || add1 > 0) pa.deficitGrowthInsideLast.token1 = inside1;
+        } else {
+            if (p.liquidity == 0 || add0 > 0) pa.inflowGrowthInsideLast.token0 = inside0;
+            if (p.liquidity == 0 || add1 > 0) pa.inflowGrowthInsideLast.token1 = inside1;
+        }
     }
