[Critical] Setter-style underlying-delta overwrite in VTSPositionLib MM decrease handling causes principal loss

# Description

A PR change to VTSPositionLib._processMMOperations for MM decreases [now clamps the full exported settlement out of pa.settled immediately](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1279-L1285) and then records only the per-operation underlying claim using [DynamicCurrencyDelta.accountUnderlyingSettlementDelta](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1293-L1295) (a setter). Because all MM positions share the same owner (MMPositionManager), a later operation in the same batch can overwrite an earlier owner-level underlying credit. The earlier position’s pa.settled has already been reduced, so the overwritten portion is lost and not withdrawn or queued.

For MM decreases, the new code path calls [_applySettlementClampFromExcess with the full postDecreaseDelta (the entire exported requirement)](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1279-L1285), reducing pa.settled immediately. It then [sets requiredSettlementDelta to the operation’s underlyingDeltaSettlement](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1286-L1288) and calls [DynamicCurrencyDelta.accountUnderlyingSettlementDelta(owner, requiredSettlementDelta, ...)](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1293-L1295). [This function sets (not adds to) the owner’s underlying deltas](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/DynamicCurrencyDelta.sol#L134-L159), so multiple operations in the same PoolManager unlock for the same owner/currency overwrite each other. When a later decrease overwrites an earlier owner-level credit, and only the final credit is withdrawn at batch end, the earlier exported value is lost because it was already removed from pa.settled and is no longer represented as an owner-level delta or in a queue. The [end-of-batch NonzeroDeltaCount check passes](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L58-L63) after withdrawing the final (overwriting) credit, masking the loss. This behavior was introduced by the PR: [previously only the queued shortfall was clamped immediately, and the immediate settleable slice remained represented in pa.settled until settlement](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1150-L1156), avoiding this overwrite-driven loss.

# Severity

**Impact Explanation:** [High] Direct, material loss of principal settlement value for the MM: earlier immediate claims are removed from pa.settled and then overwritten at the owner-level, making them unclaimable.

**Likelihood Explanation:** [High] No special constraints: batching multiple MM operations in one unlock on the same market is common and intended; overwrite occurs purely due to normal sequencing.
...
### Scenario 1.
Two MM decreases (positions A then B) in one batch on the same market/currency: Decrease A exports +100; [pa[A].settled is reduced by 100](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1279-L1285) and [owner underlying delta is set to +100](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1293-L1295). Decrease B exports +20; pa[B].settled is reduced by 20 and owner underlying delta is set to +20, overwriting +100. A single end-of-batch settle [withdraws only 20](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L58-L63); the earlier 80 from A is permanently lost.
#### Preconditions / Assumptions
- (a). Two or more MM operations in the same PoolManager unlock (batch)
- (b). Both positions owned by the same MMPositionManager (shared Position.owner)
- (c). Same underlying currency lane (same pool/market)
- (d). Decreases permitted (RFS closed for the decreased positions)

### Scenario 2.
An MM decrease followed by an MM increase in the same batch on the same market/currency: The decrease exports an immediate underlying claim and reduces pa.settled by that amount; [owner underlying delta is set to the claim](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1293-L1295). The subsequent increase [sets a new owner-level target](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/DynamicCurrencyDelta.sol#L134-L159) (possibly smaller or negative), overwriting the earlier credit. End-of-batch settle withdraws only the final overwritten credit; the earlier decrease’s immediate claim is lost.
#### Preconditions / Assumptions
- (a). An MM decrease followed by an MM increase in the same PoolManager unlock (batch)
- (b). Same MMPositionManager as owner (shared deltas)
- (c). Same underlying currency lane (same pool/market)
- (d). Decreases permitted for the decreased position

### Scenario 3.
Mixed decreases with partial vault availability (some principal shortfall queued): A decrease splits export into queued shortfall (safely tracked) and an immediate underlying slice, then [reduces pa.settled by the full export](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1279-L1285) and [sets owner underlying delta to the immediate slice](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1293-L1295). A later decrease overwrites the owner-level delta. End-of-batch settle withdraws only the final credit; earlier immediate slice(s) are lost while queued portions remain safe.
#### Preconditions / Assumptions
- (a). Two or more MM decreases in the same PoolManager unlock (batch)
- (b). Limited vault availability causing principal shortfall queue plus an immediate underlying slice
- (c). Same MMPositionManager as owner (shared deltas)
- (d). Same underlying currency lane (same pool/market)
- (e). Decreases permitted for the decreased positions
...
+            // Split model: Underlying settlement deltas on MMPM represent market liquidity claims (settle-only).
+            // Accumulate per-batch to prevent overwrite across multiple ops: target = current + this op's delta.
+            BalanceDelta current = DynamicCurrencyDelta.getUnderlyingDeltaPair(
+                p.owner, p.poolKey.currency0, p.poolKey.currency1
+            );
...
+                p.owner,
+                toBalanceDelta(
+                    current.amount0() + requiredSettlementDelta.amount0(),
+                    current.amount1() + requiredSettlementDelta.amount1()
+                ),
