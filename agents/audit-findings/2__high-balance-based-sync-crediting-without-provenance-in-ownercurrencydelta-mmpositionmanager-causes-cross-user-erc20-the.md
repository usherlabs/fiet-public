[High] Balance-based SYNC crediting without provenance in OwnerCurrencyDelta/MMPositionManager causes cross-user ERC20 theft

# Description

[OwnerCurrencyDelta.syncBalanceAsCredit](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/OwnerCurrencyDelta.sol#L181-L201) credits a locker up to the full ERC20 balance held by MMPositionManager with no provenance check. MMPositionManager exposes [SYNC](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionManager.sol#L610-L617) and [TAKE](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L83-L98) to any locker, and [ERC20 self-take to address(this)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L83-L98) leaves tokens on the contract. As a result, any later locker can SYNC a token and TAKE it out, stealing prior users’ residual ERC20 or using it to reduce their own debt.

[OwnerCurrencyDelta.syncBalanceAsCredit reads currency.balanceOf(owner) and adjusts the target locker’s delta to match the owner’s full ERC20 balance (or uses it to reduce negative delta)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/OwnerCurrencyDelta.sol#L181-L201) without tying that balance to the current locker or action. MMPositionManager exposes [SYNC (which calls vtsOrchestrator.sync → OwnerCurrencyDelta.syncBalanceAsCredit with owner=address(this), target=locker)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionManager.sol#L610-L617) and [TAKE](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L83-L98) (which debits the locker’s delta and transfers ERC20 out, capped by the contract’s actual balance). [PositionManagerEntrypoint._take explicitly allows ERC20 self-take to address(this)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L83-L98), leaving real tokens stranded on the contract while [end-of-batch checks only enforce that transient deltas are zero](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/modules/VTSCurrencyDelta.sol#L81-L89). Several internal flows also [route ERC20 through MMPositionManager and invoke _syncBalanceAsCredit](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionActionsImpl.sol#L432-L454) (e.g., settlement with usePositionManagerBalance=true, [UNWRAP_LCC/WRAP_NATIVE to address(this)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionManager.sol#L476-L484)). Because SYNC credits based on the entire contract balance with no provenance and TAKE can then withdraw that amount, any locker can appropriate prior users’ ERC20 left on MMPositionManager or erase their own debts using those funds.

# Severity

**Impact Explanation:** [High] Direct, material loss of principal funds: attackers can withdraw ERC20 tokens left on MMPositionManager by other users or use them to reduce their own negative deltas.

**Likelihood Explanation:** [Medium] Exploitation requires that residual ERC20 balances exist on MMPositionManager. This state is uncommon but plausible and arises from permitted flows (ERC20 self-take to address(this), unwraps/settlements to the contract). Once present, exploitation is straightforward and requires no special privileges.
...
### Scenario 1.
Scenario 1: A prior locker leaves token T on MMPositionManager (e.g., TAKE(T, to=address(this))). Later, an attacker calls SYNC(T) to credit themselves up to the full T balance on the contract and then TAKE(T, attackerEOA, 0) to withdraw all T.
#### Preconditions / Assumptions
- (a). MMPositionManager currently holds a non-zero ERC20 balance T (e.g., from a prior locker’s ERC20 self-take to address(this) or other flows).
- (b). Any EOA can act as a locker and invoke SYNC/TAKE via modifyLiquidities.
- (c). ERC20 tokens behave as standard per scope (no hooks/rebase/FoT).

### Scenario 2.
Scenario 2: MMPositionManager holds residual underlying ERC20 U. The attacker UNWRAP_LCC to address(this), which delivers additional U to MMPositionManager and [triggers _syncBalanceAsCredit(U)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionManager.sol#L476-L484), crediting the attacker up to the full U balance (old residual + new unwrap). The attacker then TAKE(U, attackerEOA, 0) to withdraw all U.
#### Preconditions / Assumptions
- (a). MMPositionManager currently holds a non-zero residual balance of underlying ERC20 U.
- (b). Attacker has LCC to unwrap (either via positive deltas or token balance).
- (c). UNWRAP_LCC is called with recipient = address(this) so underlying U is delivered to MMPositionManager and _syncBalanceAsCredit(U) is invoked.
- (d). ERC20 tokens behave as standard per scope.

### Scenario 3.
Scenario 3: MMPositionManager holds residual WETH. The attacker calls WRAP_NATIVE to deposit WETH into MMPositionManager; the code then [calls _syncBalanceAsCredit(WETH)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionManager.sol#L631-L635), crediting the attacker up to the full WETH balance (old residual + new wrap). The attacker TAKE(WETH, attackerEOA, 0) to withdraw all WETH.
#### Preconditions / Assumptions
- (a). MMPositionManager holds a non-zero residual WETH balance.
- (b). Attacker can provide ETH to WRAP_NATIVE, which deposits WETH into MMPositionManager and invokes _syncBalanceAsCredit(WETH).
- (c). ERC20 tokens behave as standard per scope.
...
+        // ERC20 balance-based SYNC is unsupported to prevent attribution-less credits.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/modules/PositionManagerEntrypoint.sol)
...
+    /// @dev `TAKE` to `address(this)` is disallowed: it would debit the locker's delta without moving tokens,
+    ///      stranding balance on MMPM.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/MMPositionActionsImpl.sol)
