[High] Unescrowed residual fee-backing accumulation in VTSFeeLib during zero-liquidity deactivation causes unbacked pot claim minting

# Description

New residual fee-backing captured on full deactivation is stored and later burned without escrow, allowing fees already withdrawn by the LP to be reused to mint protocol fee pot claims.

This PR introduces episode-scoped residual fee-backing via [PositionAccounting.pendingResidualFeeBacking](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/types/VTS.sol#L167-L172) and the functions [_captureResidualFeeBackingOnDeactivation](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L540-L599) and [_applyBankedResidualBurn](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L590-L599) in VTSFeeLib. When a position with a pending residual-burn episode deactivates to zero liquidity, the code [adds the current fee growth to pendingResidualFeeBacking and advances feeGrowthInsideLast](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L540-L599), but does not escrow or hold back the corresponding fees that the LP receives from modifyLiquidity as feesAccrued. Later, when outflow windows allow, [_applyBankedResidualBurn includes pendingResidualFeeBacking in totalFees for fee-burn](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L246-L286), which [increases protocolFeeAccrued](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L365-L378) and is materialized into slashed pot claims via pendingFeeAdj and [CoreHook.settleHookDeltasToPot](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/CoreHook.sol#L196-L233). This reuses fee episodes already withdrawn by the LP to mint spendable ERC6909 pot claims, breaking accounting invariants and enabling misallocation in CISE-based distributions. [Episode clearing only occurs when pendingResidualBurnBase is fully consumed](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L590-L599), so banked fee-backing persists across reactivations within the same episode.

# Severity

**Impact Explanation:** [High] ERC6909 pot claims are minted from fee-burn computed over banked residual fee-backing that reflects fees already paid to the LP, fitting the 'incorrectly minted or created' funds criterion and breaking core accounting invariants; this enables material misallocation in pot-funded distributions.

**Likelihood Explanation:** [Medium] Exploitation requires an unresolved residual episode, timing around outflow windows, and possibly bearing trading fees to accrue notable fee growth. These are uncommon but realistic conditions that do not rely on trusted role misuse or broken integrations.
...
### Scenario 1.
Single-cycle: An LP fully removes to zero during an unresolved residual episode. The remove banks fee growth into pendingResidualFeeBacking while the same fees are paid out as feesAccrued. Later, when outflow windows allow, the banked fees are included in totalFees for fee-burn and mint protocolFeeAccrued/slashed pot claims, creating unbacked claims.
#### Preconditions / Assumptions
- (a). MarketVTSConfiguration.coverageFeeShare > 0 (fee sharing enabled)
- (b). Position has a pending residual-burn episode (pendingResidualBurnBase.tokenX > 0)
- (c). Outflow window progress (ofDelta) initially insufficient to consume the episode immediately

### Scenario 2.
Repeated toggles: The LP reactivates, accrues fees, and repeatedly removes to zero while the same residual episode remains unresolved. Each deactivation adds to pendingResidualFeeBacking (also paid out as feesAccrued). When the episode is finally consumed, the cumulative banked fees are burned into protocolFeeAccrued, inflating pot value from already-withdrawn fees.
#### Preconditions / Assumptions
- (a). Same as Scenario 1
- (b). Residual episode remains unresolved across multiple remove-to-zero/reactivation cycles
- (c). Sufficient swap volume (organic or induced) to accrue fees between cycles

### Scenario 3.
Collusive extraction: The attacker uses position P to inflate pot claims via banked residual fee-backing and uses a second position Q with high CISE exposure to receive bonuses funded by the inflated protocolFeeAccrued. This drains the pot and redistributes value based on unbacked claims.
#### Preconditions / Assumptions
- (a). Same as Scenario 1
- (b). Attacker controls a second position capable of accruing significant CISE exposure
- (c). Coverage allocation and afterTouchPosition calls occur to allocate bonuses from protocolFeeAccrued
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol)
...
+        c.feesBurn = FullMath.mulDiv(c.consumedFreshFees, c.bps, LiquidityUtils.BPS_DENOMINATOR);
...
+        if (consumedBurnBase == 0) return 0;
...
## [Medium] Deferred residual fee backing realized as generic slash in VTSFeeLib across zero-liquidity causes principal-like loss on later MM modifies

### Description

When a position fully deactivates, historical fee growth is only recorded in storage ([pendingResidualFeeBacking](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/types/VTS.sol#L171-L177)) without escrow. Later, during residual-burn application, this bank is converted into a positive [pendingFeeAdj (slash)](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L369-L377) and materialized as a hook delta. In MM flows, feeAdj is applied as part of fees in a way that can reduce LCC issuance on deposits or increase cancellation on withdrawals, effectively charging future modifications for earlier fees already paid out.

The PR adds episode-scoped residual fee backing across zero-liquidity intervals. On full deactivation, [VTSFeeLib._captureResidualFeeBackingOnDeactivation](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L559-L566) books fee-growth into [PositionAccounting.pendingResidualFeeBacking](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/types/VTS.sol#L171-L177) and advances feeGrowthInsideLast, but does not escrow fee tokens; the LP still receives feesAccrued on that remove. Later, [VTSFeeLib._applyBankedResidualBurn](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L572-L585) consumes the stored bank (and any fresh fees) to [compute feesBurn](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L292-L299), credits protocol fee accounting, and [sets pa.pendingFeeAdj += feesBurn](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L369-L377). VTSFeeLib.afterTouchPosition materializes pendingFeeAdj as a positive hook delta (slashed pot funding). In [VTSPositionLib._processMMOperations](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1369-L1378), feeAdj is folded into principalDelta via accruedFeesAfterAdj = feesAccrued - feeAdj and principalDelta = p.callerDelta - accruedFeesAfterAdj, which is not bounded by current feesAccrued. As a result, if feesAccrued at that modify is small or zero, the realized slash effectively reduces LCC issuance on add-liquidity or increases cancellation/queueing on remove-liquidity, charging future modify calls for historical fees that were already paid at deactivation.

### Severity

**Impact Explanation:** [Medium] The realized amount is a legitimate coverage fee share but is collected later in a way that affects MM principal-like LCC accounting (reduced issuance or increased cancellation). It does not break core invariants or constitute theft but causes a material, misattributed fee realization for the position owner.

**Likelihood Explanation:** [Medium] Requires a residual DICE episode, full deactivation while residual exists, and a later modify to trigger materialization. These are realistic for active market makers but not universal.
...
### Scenario 1.
Victim fully removes liquidity and collects all fees; pendingResidualFeeBacking is recorded. On a later add-liquidity, residual-burn consumes the bank and generates a positive feeAdj. With near-zero feesAccrued on that call, principalDelta is adjusted by feeAdj, causing fewer LCC to be minted than the underlying deposit would normally imply.
#### Preconditions / Assumptions
- (a). Position has a non-zero pending residual DICE burn base prior to full deactivation
- (b). Meaningful fee growth accrued on the opposite (fee) token lane
- (c). coverageFeeShare (bps) > 0
- (d). Position is fully deactivated to zero liquidity to trigger residual fee capture
- (e). A later add-liquidity modify occurs with low per-call feesAccrued

### Scenario 2.
Victim fully removes liquidity and collects all fees; later reactivates, and then removes liquidity again. The bank is consumed and realized as feeAdj at touch time, and on the subsequent remove, principalDelta includes feeAdj, leading to more LCC being canceled/queued than would occur with pure principal-only accounting.
#### Preconditions / Assumptions
- (a). Position has a non-zero pending residual DICE burn base prior to full deactivation
- (b). Historical fee growth exists on the fee token lane before deactivation
- (c). coverageFeeShare (bps) > 0
- (d). Position is reactivated after deactivation and later a remove-liquidity modify occurs

### Scenario 3.
Victim performs a small reactivation add-liquidity after a prior zero-liquidity interval with banked residual fees. Despite minimal feesAccrued on the add, the bank is realized as a large feeAdj, reducing minted LCC disproportionately relative to the small deposit.
#### Preconditions / Assumptions
- (a). Position has a non-zero pending residual DICE burn base
- (b). Deactivation previously captured historical fee growth into pendingResidualFeeBacking
- (c). coverageFeeShare (bps) > 0
- (d). A small add-liquidity modify is executed with minimal per-call feesAccrued
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol)
...
+        // Do not mix feeAdj into principal: reconstruct principal from callerDelta minus feesAccrued only.
+        BalanceDelta accruedFeesAfterAdj = p.feesAccrued;
