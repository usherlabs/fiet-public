[High] Generalized custody–queue reconciliation with zero-amount collect in MMPositionManager causes settlement-gate bypass of commit-bucket custody

# Description

[Extending reconciliation to all tokenIds](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/MMPositionManager.sol#L565-L576) allows a locker to annul their global Hub queue via a user→MMPM roundtrip and then call collect with maxAmount=0 to release commit-bucket custodied LCC without burning/settlement, bypassing intended reserve/settlement gating and ordering.

The PR changes MMPositionManager to reconcile custodied LCC against the beneficiary’s global LiquidityHub.settleQueue for all buckets (including commit buckets) via [_reconcileCustodyWithHubQueue(lcc, tokenId, beneficiary)](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/MMPositionManager.sol#L565-L576), called before [_collectAvailableLiquidity](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/MMPositionManager.sol#L602-L606). If custodied > hubQueued, the excess (custodied − hubQueued) is immediately released from the custodian to the beneficiary. Because [LiquidityHub.settleFromCustodian(lcc, custodian, tokenId, recipient, maxAmount) returns early when maxAmount==0](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/libraries/LiquidityHubLib.sol#L678-L684) (and thus does not invoke processSettlementFor to burn/settle), a locker can perform a user→MMPM transfer to trigger [LiquidityHub.annulSettlementBeforeTransfer](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/libraries/LiquidityHubLib.sol#L721-L731) (reducing their live queue), then SYNC and [TAKE](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L88-L96) to retrieve the same tokens (no net LCC cost), and finally call COLLECT_AVAILABLE_LIQUIDITY with maxAmount=0 to have reconciliation release commit-bucket custody as liquid LCC without any settlement/burning. Pre-PR, only the utility bucket reconciled this way; the PR newly enables this path for commit buckets, breaking intended reserve- and settlement-gated ordering for commit-bucket custody release.

# Severity

**Impact Explanation:** [Medium] This breaks important protocol functionality by bypassing intended reserve- and settlement-gated ordering for commit-bucket custody release, allowing reconciliation-only tokenization of custodied LCC for the locker. It does not directly mint new tokens or pay out underlying, so it is not a direct principal loss.

**Likelihood Explanation:** [High] All steps are under the attacker’s control using ordinary user-facing flows (user→MMPM transfer, SYNC, TAKE, and collect with maxAmount=0). No rare conditions, trusted roles, or special timing are required, and the incentive is clear.
...
### Scenario 1.
Single-commit drain: The locker transfers LCC from their EOA to MMPM (non-protocol→protocol), triggering [annulSettlementBeforeTransfer](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/LCC.sol#L227-L231) and reducing their LiquidityHub.settleQueue. They then SYNC and [TAKE](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L88-L96) to pull the same LCC back, resulting in a near-zero live queue at no net token cost. Calling COLLECT_AVAILABLE_LIQUIDITY(lcc, tokenId, 0) causes [_reconcileCustodyWithHubQueue](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/MMPositionManager.sol#L565-L576) to release (custodied − hubQueued) ≈ custodied from the commit-bucket custodian to the locker, while settleFromCustodian short-circuits due to maxAmount=0 (no burn/settlement).
#### Preconditions / Assumptions
- (a). Attacker is the locker (beneficiary) able to call MMPositionManager actions
- (b). There exists nonzero commit-bucket custody in MMQueueCustodian for (tokenId, lcc, locker)
- (c). User→MMPM LCC transfers are allowed (protocol-bound endpoint semantics)
- (d). SYNC and TAKE are callable, establishing and consuming locker delta from MMPM-held balance
- (e). COLLECT_AVAILABLE_LIQUIDITY accepts maxAmount=0
- (f). LCC and LiquidityHub behave per in-scope assumptions (no weird ERC20 behavior)

### Scenario 2.
Multi-commit aggregate drain: The locker repeats the free queue-annul loop once for the aggregate queue across multiple commit tokenIds. Then, for each tokenId, they call COLLECT_AVAILABLE_LIQUIDITY with maxAmount=0. Each call reconciles and releases the full per-bucket custodied LCC to the locker, bypassing reserve-backed settlement across all their commitments.
#### Preconditions / Assumptions
- (a). Same as Scenario 1, but multiple commit tokenIds each have nonzero custody
- (b). Aggregate queue can be reduced via a single or repeated user→MMPM annul loop
- (c). Locker can iterate COLLECT_AVAILABLE_LIQUIDITY(lcc, tokenId, 0) over multiple tokenIds

### Scenario 3.
Zero-reserve environment: Even when LiquidityHub has zero market-derived reserve for the LCC’s underlying, the locker performs the queue-annul loop to bring their live queue to zero, then collects with maxAmount=0. Reconciliation still releases commit-bucket custody as liquid LCC, despite no reserves, because processSettlementFor is never invoked.
#### Preconditions / Assumptions
- (a). Same as Scenario 1
- (b). LiquidityHub has zero market-derived underlying reserve for the given LCC
- (c). Locker still can perform the annul loop to drop their queue and then collect with maxAmount=0
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/MMPositionManager.sol)
...
+        // Reconcile only for utility bucket (tokenId == 0). Commit buckets must release custody
+        // only via Hub-led settlement paths (settleFromCustodian -> processSettlementFor).
