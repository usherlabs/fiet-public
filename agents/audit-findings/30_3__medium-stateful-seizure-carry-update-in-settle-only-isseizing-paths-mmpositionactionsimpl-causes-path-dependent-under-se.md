[Medium] Stateful seizure-carry update in settle-only isSeizing paths (MMPositionActionsImpl) causes path-dependent under-seizure (no liquidity removal)

# Description

When a batch is in ambient seizing context, settle-only deposit paths call onMMSettle(isSeizing=true), which advances and persists seizure carry while ignoring the returned seizedLiquidityUnits. Liquidity is not decreased, creating path-dependent under-seizure and potential subsidy if protocol credits are used.

The PR introduced stateful seizure sizing with Q128 carry persistence (SeizureCarryQ128Lib) and began storing per-lane seizure-carry remainders in PositionAccounting.seizureLiquidityCarry. VTSLifecycleLinkedLib._calcSeizure now computes whole seized units plus a Q128 remainder and immediately writes the remainder to storage. In MMPositionActionsImpl, only the SEIZE_POSITION path consumes seizedLiquidityUnits and reduces liquidity; it also reverts when seizure sizing yields 0 to block zero-liquidity modifies. However, other settlement paths that can run under ambient seizing (set by _seizePosition and cleared only at end-of-batch) ignore the seizedLiquidityUnits they receive from onMMSettle and perform no liquidity decrease. Specifically: (1) the deposit leg settling protocol credits (_settleProtocolCreditsFromDeltas), (2) the deposit leg settling locker credits via _settle(...), and (3) raw SETTLE_POSITION deposits. In each of these, onMMSettle(isSeizing=true) advances seizure carry in storage, and if a unit threshold is crossed, returns seizedLiquidityUnits>0, but MMPositionActionsImpl discards it and does not call _decreaseInternal. This allows a same-batch settle-only deposit to "spend" carry (and even cross whole units) without actually removing liquidity. Future seizures then start from a reduced carry baseline and under-seize relative to the cumulative cures applied. If protocol (address(this)) credits are used, this also subsidizes the position by consuming shared produced credits without the intended unit-removal penalty.

# Severity

**Impact Explanation:** [Medium] Core seizure enforcement becomes path-dependent: carry can be advanced and whole seized units can be produced without any liquidity removal on settle-only paths, leading to persistent under-seizure. When protocol credits fund the cure, this also subsidizes the position. This is a meaningful economic/policy correctness violation but not an immediate principal theft or freeze.

**Likelihood Explanation:** [Medium] Requires sequencing actions within one batch (set seizing context, then a settle-only deposit) and availability of credits. These are plausible in real operation and automation, but not unconstrained or always-on.
...
### Scenario 1.
Seizer sets ambient seizing by calling SEIZE_POSITION (a proper seizure decrease occurs), then in the same batch calls SETTLE_POSITION_FROM_DELTAS with payerIsUser=true and shouldTake=false to settle protocol credits. onMMSettle(isSeizing=true, fromDeltas=true) advances seizure carry and may return seizedLiquidityUnits>0, but MMPositionActionsImpl ignores it, so no liquidity is removed while protocol credits fund a cure.
#### Preconditions / Assumptions
- (a). Live MM position with liquidity > 0 and at least one open RFS lane
- (b). Ambient seizing context set earlier in the same batch by a successful SEIZE_POSITION
- (c). Protocol (address(this)) underlying credits available for at least one lane
- (d). Deposit size and/or existing carry remainder sufficient to produce seizedLiquidityUnits > 0 in sizing

### Scenario 2.
Seizer sets ambient seizing by calling SEIZE_POSITION, then in the same batch settles from their own locker credits using SETTLE_POSITION_FROM_DELTAS with payerIsUser=false and shouldTake=false, which calls _settle(...). onMMSettle(isSeizing=true) advances seizure carry and may return seizedLiquidityUnits>0, which is ignored, leaving liquidity unchanged while locker credits fund a cure.
#### Preconditions / Assumptions
- (a). Live MM position with liquidity > 0 and at least one open RFS lane
- (b). Ambient seizing context set earlier in the same batch by a successful SEIZE_POSITION
- (c). Locker (msgSender) has or creates positive underlying credits in the batch
- (d). Deposit size and/or existing carry remainder sufficient to produce seizedLiquidityUnits > 0 in sizing

### Scenario 3.
Seizer sets ambient seizing by calling SEIZE_POSITION, then performs a raw deposit-only SETTLE_POSITION (negative lanes). onMMSettle(isSeizing=true) advances seizure carry and may produce seizedLiquidityUnits>0, which MMPositionActionsImpl ignores, so no liquidity decrease happens.
#### Preconditions / Assumptions
- (a). Live MM position with liquidity > 0 and at least one open RFS lane
- (b). Ambient seizing context set earlier in the same batch by a successful SEIZE_POSITION
- (c). Deposit executed directly via SETTLE_POSITION with negative lanes
- (d). Deposit size and/or existing carry remainder sufficient to produce seizedLiquidityUnits > 0 in sizing
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/MMPositionActionsImpl.sol)
...
+                (, PositionId posId) = getPosition(tokenId, positionIndex);
+                if (_isSeizing(posId)) {
...
+            // Disallow settle-only deposits under ambient seizing context; must use SEIZE_POSITION which couples sizing with decrease.
+            {
