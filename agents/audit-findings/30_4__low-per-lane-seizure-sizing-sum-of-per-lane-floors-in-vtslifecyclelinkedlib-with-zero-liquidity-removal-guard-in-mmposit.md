[Low] Per-lane seizure sizing (sum of per-lane floors) in VTSLifecycleLinkedLib with zero-liquidity-removal guard in MMPositionActionsImpl causes seizure DoS and under-collection

# Description

Seizure sizing floors each token lane independently and sums them, while the protocol seizes a single scalar liquidity amount. In small-liquidity, base-tranche-binding cases this yields zero seized units even when the aggregate exact entitlement is ≥1, triggering a revert that blocks seizure. It also causes under-collection versus flooring once on the aggregate. Per-lane fractional carries do not mitigate this because they are cleared on modify.

The PR introduces per-lane Q128 carries (TokenPairSeizureCarryQ128) and lane-specific sizing via SeizureCarryQ128Lib.accumulateLane. VTSLifecycleLinkedLib._calcSeizure computes seizedLiquidityUnits as the sum of already-floored lane results: seized = seized0 + seized1. MMPositionActionsImpl._seizePosition reverts with Errors.SeizureWithoutLiquidityRemoval if seizedLiquidityUnits == 0. Because the protocol removes a single scalar liquidity amount, summing per-lane floors undercounts versus flooring once on the aggregate. In small-liquidity positions where both lanes are base-tranche binding, each lane can floor to 0 while the aggregate exact claim across both lanes is ≥1, returning seized=0 and causing a revert that blocks seizure. Per-lane fractional remainders cannot combine across lanes and are cleared by VTSPositionLib._trackCommitment on the subsequent decrease, so fractional value is lost rather than recovered later. finalizeSeizureTotal only up-rounds near full closure and does not prevent the zero-total case for typical configurations.

# Severity

**Impact Explanation:** [Low] Primarily an availability/liveness issue (seizure path DoS) with no direct funds loss or invariant break; under-collection is a dust-level rounding effect without a cheap path to materialize significant losses.

**Likelihood Explanation:** [Low] Requires specific conditions (both lanes base-binding and small liquidity) that are plausible but not default; MMs can engineer them, but multiple preconditions reduce overall likelihood.
...
### Scenario 1.
Spurious revert (DoS of seizure): With small position liquidity and both lanes base-tranche binding, fully curing both lanes yields per-lane floors of 0 while the aggregate exact entitlement is ≥1. _calcSeizure returns 0, and _seizePosition reverts with SeizureWithoutLiquidityRemoval, blocking seizure.
#### Preconditions / Assumptions
- (a). Both token lanes are base-tranche binding during seizure sizing
- (b). Small position liquidity L such that floor(L * baseBps / B) = 0 per lane (e.g., baseVTSRate=1%, L<100)
- (c). Seizure is authorized (grace/insolvency gating satisfied)
- (d). Guarantor deposits fully cure each lane within per-lane clamps

### Scenario 2.
Under-collection relative to aggregate floor: Curing both lanes where one lane’s floor is 1 and the other’s is 0, while the aggregate exact would floor to 2, results in only 1 unit seized. The subsequent decrease clears per-lane carries, so the remaining fractional value on the other lane is lost.
#### Preconditions / Assumptions
- (a). Both token lanes are base-tranche binding during seizure sizing
- (b). Liquidity L such that one lane’s floor yields 1 and the other yields 0 while the aggregate exact would be ≥2
- (c). Seizure is authorized and deposits respect per-lane clamps

### Scenario 3.
MM evasion via many small-liquidity positions: An MM splits liquidity across many positions with small L under base-tranche binding on both lanes. Each seizure attempt returns 0 seized units and reverts, preventing seizure across the fleet unless parameters or liquidity change.
#### Preconditions / Assumptions
- (a). MM can split liquidity into many small-L positions
- (b). Both lanes base-tranche binding per position during seizure sizing
- (c). Seizure is authorized for these positions
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol)
...
+        // FIXME(seizure-agg): Current logic adds already-floored per-lane results (floor(a)+floor(b)).
+        // To implement floor on the aggregate and avoid zero-liquidity seizure when aggregate exact ≥ 1:
+        // - compute each lane's (prodWhole, rem->fracQ) using the same rational,
+        // - sum whole units and sum Q128 fractions (+ stored carries), floor extraWhole once,
+        // - persist a single leftover carry on an open lane; then apply _finalizeSeizureTotal.
         uint256 total =
...
File: `contracts/evm/src/libraries/SeizureCarryQ128Lib.sol`

[Source](https://github.com/usherlabs/fiet-protocol/blob/ad08079143460861ffd09663c34782c7c967ddbc/contracts/evm/src/libraries/SeizureCarryQ128Lib.sol)
...
+    // TODO(seizure-agg): Consider exposing a two-lane aggregation helper that returns combined whole units
+    // and a single combined carry to support floor-on-aggregate sizing across both lanes.
...
```diff
 ...
 library VTSPositionLib {
     ...
     function _trackCommitment(VTSStorage storage s, PositionId positionId, uint128 liveLiquidity) internal {
         PositionAccounting storage pa = s.positionAccounting[positionId];
+        // NOTE(seizure-agg): Clearing per-lane seizure carries on every modify means cross-lane
+        // fractional capture must occur within the same seizure-sizing step before the decrease.
