[Critical] Permissionless settlement of custodian-owned queues in LiquidityHub/MMPositionManager causes stranded underlying and commit lifecycle DoS

# Description

After moving MM settlement queues to per-recipient custodians and removing the coordinated settle-from-custodian path, permissionless [LiquidityHub settlements](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L920-L939) can zero the custodian’s queue and burn its LCC before MMPositionManager collects, leaving underlying stuck in the custodian and the custodian’s beneficiary ledger non-decremented. Subsequent collects short-circuit, and commit buckets remain non-empty, blocking decommit and NFT transfer.

This PR routed MM liquidity-decrease queues to a per-recipient MMQueueCustodian and removed the previous LiquidityHub settleFromCustodian flow. [Collection now requires a fresh viable Hub settlement](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L550-L579) (min over hub queue, custodian market-derived balance, beneficiary entitlement, reserve, and maxAmount) and only then forwards underlying via custodian.[collectUnderlyingToBeneficiary](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMQueueCustodian.sol#L120-L150). [LiquidityHub.processSettlementFor remains permissionless](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L920-L939): any caller can settle settleQueue(lcc, custodian) when reserves and custodian balance allow. That settlement burns the custodian’s market-derived LCC and transfers underlying into the custodian but does not decrement the custodian’s beneficiary-scoped ledger (_queuedLcc). When the beneficiary later calls MMPositionManager to collect, amount evaluates to 0 (hub queue and/or custodian LCC balance are zero), so the function returns without paying out and without decrementing the entitlement ledger. Underlying remains stranded on the custodian while entitlements stay outstanding, and for commit buckets this keeps [isBucketEmpty](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMQueueCustodian.sol#L120-L150) false, blocking [decommit](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L340-L359) and NFT transfer indefinitely unless new matching queue is created. No fallback exists to forward already-received underlying when amount==0.

# Severity

**Impact Explanation:** [High] Underlying can be indefinitely stranded on custodians while custodian entitlements remain non-zero, and commit buckets remain non-empty, blocking decommit and NFT transfer with no built-in workaround once no further matching queues can be created.

**Likelihood Explanation:** [High] No special constraints: LiquidityHub.processSettlementFor is intentionally permissionless and typical automation will call it as reserves arrive, making the benign-automation scenario realistic.
...
### Scenario 1.
Commit-bucket DoS via preemptive settlement: An MM decreases liquidity; VTS queues principal to the per-recipient MMQueueCustodian; MMPM forwards LCC into the custodian and records beneficiary entitlement. A third party then calls [LiquidityHub.processSettlementFor(lcc, custodian)](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L920-L939) when reserves are available, burning the custodian’s LCC and paying underlying to the custodian. Later, the locker calls COLLECT_AVAILABLE_LIQUIDITY, which short-circuits with amount==0 and never calls [collectUnderlyingToBeneficiary](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMQueueCustodian.sol#L120-L150). Underlying remains stuck on the custodian and the commit bucket stays non-empty, blocking decommit/NFT transfer.
#### Preconditions / Assumptions
- (a). A commitment NFT was minted; a per-recipient MMQueueCustodian exists for the NFT recipient.
- (b). A liquidity decrease queues principal to the custodian ([hook queueRecipient](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/types/Position.sol#L50-L66)) and MMPM forwards LCC to the custodian, recording beneficiary entitlement.
- (c). Reserves for the LCC’s underlying become available at the Hub.
- (d). [processSettlementFor is permissionless and callable by any address](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L920-L939).

### Scenario 2.
Utility-bucket freeze after UNWRAP_LCC: The locker unwraps LCC via MMPM, which routes through the custodian ([unwrapLccViaHub](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMQueueCustodian.sol#L80-L118)) and records queued shortfall for the utility bucket. A third party settles settleQueue(lcc, custodian) permissionlessly before the user collects. Subsequent collect sees amount==0 and does not forward underlying; the user’s utility-bucket entitlement remains unpaid with underlying stuck in the custodian.
#### Preconditions / Assumptions
- (a). A queue custodian exists for the locker.
- (b). UNWRAP_LCC is executed via MMPM, routing through the custodian; queued shortfall is recorded for the utility bucket.
- (c). Reserves become available at the Hub.
- (d). [processSettlementFor is permissionless and callable by any address](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L920-L939).

### Scenario 3.
Benign automation DoS: A settlement bot permissionlessly calls [LiquidityHub.processSettlementFor](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L920-L939) for any outstanding custodian queues as reserves arrive. Many MM queues are settled to custodians ahead of their collects, zeroing hub queues and custodian LCC balances while leaving custodian entitlements non-zero. Later collects consistently short-circuit with amount==0, causing widespread inability to collect queued shortfalls and, for commit buckets, blocking decommit/NFT transfers.
#### Preconditions / Assumptions
- (a). Monitoring/automation is active that calls LiquidityHub.processSettlementFor on outstanding queues when reserves arrive.
- (b). MM queues are owned by custodians and entitlements are recorded in custodian._queuedLcc per beneficiary.
- (c). No fallback exists in MMPM to forward previously received underlying from the custodian when amount==0.
...
-        if (amount == 0) return;
+        if (amount == 0) {
+            // Fallback: if Hub was pre-settled (hubQ/holderBal zero) but custodian already holds underlying,
+            // pay out from currently held underlying up to the beneficiary's entitlement.
+            if (entitled > 0) {
+                address underlying = ILCC(lcc).underlying();
+                uint256 uBal = underlying == address(0)
+                    ? custAddr.balance
+                    : IERC20(underlying).balanceOf(custAddr);
+                uint256 pay = Math.min(entitled, uBal);
+                if (pay > 0) custodian.collectUnderlyingToBeneficiary(bucket, lcc, locker, pay);
+            }
+            return;
+        }
