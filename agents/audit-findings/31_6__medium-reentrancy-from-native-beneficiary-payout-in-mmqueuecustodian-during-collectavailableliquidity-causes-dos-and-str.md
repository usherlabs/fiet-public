[Medium] Reentrancy from native beneficiary payout in MMQueueCustodian during collectAvailableLiquidity causes DoS and stranded funds on custodian

# Description

Splitting MM collection into a Hub settlement followed by a raw native payout to the beneficiary enables reentrancy into [LiquidityHub.processSettlementFor](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L931-L939). A beneficiary contract can settle extra amounts for the custodian mid-collect, reducing Hub queue/holder balances without decrementing custodian per-beneficiary ledgers. This desynchronizes state, strands ETH on the custodian, and can block other beneficiaries’ collects and commitment lifecycle operations.

The PR replaced an atomic Hub-side helper with a two-step flow: [MMPositionManager first calls LiquidityHub.processSettlementFor(lcc, custodian, amount)](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L560-L569), then [MMQueueCustodian.collectUnderlyingToBeneficiary forwards native ETH to the beneficiary via a raw call](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMQueueCustodian.sol#L133-L137). Because the raw native call to the beneficiary occurs after the Hub call completes, a contract beneficiary’s receive() can reenter LiquidityHub and [call processSettlementFor(lcc, custodian, y)](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L931-L939) again in the same transaction. The extra settlement [reduces the custodian’s Hub queue](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/libraries/LiquidityHubLib.sol#L538-L545) and market-derived LCC balance and [transfers additional ETH to the custodian](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/libraries/LiquidityHubLib.sol#L574-L581), but the custodian’s per-beneficiary custody (_queuedLcc/_bucketQueuedTotal) is only decremented for the original amount. Subsequent collects [recompute amount = min(hubQ, entitled, holderBal, reserve)](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L560-L569), which can become zero due to the reentrant drain, causing MMPM to skip payout and leaving custodied slices outstanding. As a result, ETH accumulates on the custodian while beneficiaries cannot collect, and [commitment decommit/transfer can be blocked by non-empty custody buckets](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L350-L358).

# Severity

**Impact Explanation:** [Medium] The issue can cause significant availability loss: other beneficiaries’ collects return zero while ETH sits on the custodian, and custody buckets remain non-empty, blocking decommit/transfer. The duration depends on future Hub reserves and is not guaranteed to exceed a week across deployments.

**Likelihood Explanation:** [Medium] Requires realistic but not universal conditions: a contract beneficiary with receive(), non-zero residual hub queue/holder balances, and sufficient reserves. The behavior is primarily a sabotage/grief vector rather than profit-seeking, but plausible in adversarial settings.
...
### Scenario 1.
Two beneficiaries share a custodian on a native-underlying LCC. Attacker B1 (a contract) calls collect, the manager settles Hub for the custodian, and the custodian forwards ETH to B1. In B1’s receive(), B1 reenters LiquidityHub.processSettlementFor to settle additional y for the custodian. Hub queue and custodian LCC balances are reduced by x1+y, but the custodian’s ledger is only decremented by x1. Later, victim B2’s collect sees amount=0 and returns early; B2’s custody remains, and the commitment bucket stays non-empty, blocking decommit/transfer.
#### Preconditions / Assumptions
- (a). Native-underlying LCC market (underlying == address(0))
- (b). MMQueueCustodian deployed for the commitment owner (recipient-keyed)
- (c). Attacker beneficiary is a contract with a receive() function
- (d). Hub settleQueue[lcc][custodian] > attacker’s entitlement and other beneficiaries’ entitlements exist
- (e). Custodian’s market-derived LCC balance and Hub market-derived reserve are sufficient
- (f). LiquidityHub.processSettlementFor is permissionless

### Scenario 2.
A custodian serving many beneficiaries has large outstanding queues. The attacker repeatedly performs collect for its own slices, reentering during each native payout to settle extra amounts to the custodian. This systematically drains shared hub queue/holder balances. Other beneficiaries’ collects then see amount=0 and do not pay out, leaving their custody intact and preventing lifecycle operations.
#### Preconditions / Assumptions
- (a). Native-underlying LCC market (underlying == address(0))
- (b). Custodian serves multiple beneficiaries/buckets with positive queued custody
- (c). Attacker beneficiary is a contract with a receive() function
- (d). Hub queue and custodian holder balances remain positive across multiple collects
- (e). LiquidityHub.processSettlementFor is permissionless

### Scenario 3.
Utility bucket 0 (unwrap shortfalls): An attacker beneficiary collects from bucket 0 and reenters during payout to settle extra to the custodian. Other utility claimants subsequently see amount=0 and cannot collect, while ETH sits on the custodian until new Hub settlements occur.
#### Preconditions / Assumptions
- (a). Native-underlying LCC market (underlying == address(0))
- (b). Utility bucket 0 holds queued custody for multiple beneficiaries
- (c). Attacker beneficiary is a contract with a receive() function
- (d). Hub queue and custodian holder balances are sufficient to settle more than one slice
- (e). LiquidityHub.processSettlementFor is permissionless
...
+        // Detect and revert on any extra settlement that occurred during beneficiary payout (reentrancy hardening).
+        uint256 hubQAfter = liquidityHub.settleQueue(lcc, custAddr);
+        (, uint256 holderBalAfter) = ILCC(lcc).balancesOf(custAddr);
+        if ((hubQ > hubQAfter && hubQ - hubQAfter > amount) || (holderBal > holderBalAfter && holderBal - holderBalAfter > amount)) {
+            revert Errors.InvariantViolated("extra settlement during payout");
+        }
     }
