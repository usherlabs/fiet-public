[Medium] Shared recipient-keyed custodian with stale per-bucket ledger in MMPositionManager._collectAvailableLiquidity causes cross-bucket settlement starvation

# Description

Switching to a [single custodian per recipient](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L92-L100) collapses multiple commitments and utility into one shared Hub queue/balance. [_collectAvailableLiquidity settles this shared queue](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L565) but only debits one local bucket’s ledger. Because [processSettlementFor is permissionless](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L931-L940) and unaware of bucket ledgers, local slices can become stale and later be collected against newly created shared queue for another bucket, starving the rightful bucket and blocking its transfer/decommit.

The PR [routes all commitment and utility buckets for a recipient to one MMQueueCustodian](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L92-L100), creating a single LiquidityHub.settleQueue(lcc, custodian) and a single custodian-held market-derived LCC balance. MMQueueCustodian still tracks [per-bucket, per-beneficiary queued slices](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMQueueCustodian.sol#L38-L44). In MMPositionManager._collectAvailableLiquidity, the [amount to collect is computed as the minimum of the shared Hub queue, the caller’s local per-bucket entitlement, the custodian’s market-derived LCC balance, the market-derived reserve, and maxAmount](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L556-L563). The function then [calls LiquidityHub.processSettlementFor(lcc, custodian, amount)](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L565), which [burns the custodian’s LCC and transfers underlying](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/libraries/LiquidityHubLib.sol#L530-L551), and finally [custodian.collectUnderlyingToBeneficiary(bucket, lcc, locker, amount)](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/MMPositionManager.sol#L566), which debits only that bucket’s local ledger. Because [LiquidityHub.processSettlementFor is permissionless](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol#L931-L940) and unaware of per-bucket ledgers, third parties (including automation) can settle the shared Hub queue early, leaving local bucket ledgers stale. Later, when a different bucket on the same custodian creates fresh queue, a caller on the stale bucket can collect first, consuming the newly created shared queue. The rightful bucket’s beneficiary is then starved (amount=0) and its bucket remains non-empty, preventing transfer/decommit (CommitCustodyNotDrained). This is a priority inversion/liveness bug, not direct theft: the starved bucket’s local entitlement remains but must wait for further liquidity.

# Severity

**Impact Explanation:** [Medium] Significant availability loss/lifecycle DoS: newly created settlement capacity for one bucket can be consumed by a stale local slice on another bucket due to shared Hub queue/balance, leaving the rightful bucket unable to collect and its NFT non-transferable/non-decommittable until more liquidity arrives. This is a priority inversion/liveness issue rather than direct principal loss.

**Likelihood Explanation:** [Medium] Requires a stale per-bucket ledger (realistic due to permissionless/automated processSettlementFor) and timing where a different bucket on the same custodian creates queue before the rightful beneficiary collects. These are notable but realistic conditions in normal operation; users can partially mitigate by batching collect with decrease.
...
### Scenario 1.
Two commitments A and B share owner O (same custodian). A previously recorded a per-bucket entitlement QA for beneficiary LA. A third party calls LiquidityHub.processSettlementFor(lcc, custodian, x), 0 < x ≤ QA, reducing the shared Hub queue and custodian LCC but not A’s local ledger. Later, B decreases/unwraps creating fresh queue QB on the same custodian. LA calls COLLECT_AVAILABLE_LIQUIDITY for A; the function settles up to QA using the shared Hub queue (which now includes QB) and pays underlying to LA, while only A’s local ledger is debited. B’s local ledger still shows QB but hubQ is now reduced/empty, so B cannot collect and its NFT remains non-transferable/non-decommittable.
#### Preconditions / Assumptions
- (a). Commitments A and B share the same owner O, so custodianFor[O] is used for both (one shared Hub queue and LCC balance).
- (b). A has a recorded local bucket entitlement QA for beneficiary LA in MMQueueCustodian.
- (c). A third party (e.g., reactive automation) has called LiquidityHub.processSettlementFor(lcc, custodian, x), reducing the shared Hub queue and custodian LCC without touching A’s local ledger.
- (d). Later, B performs a decrease/unwrap that creates fresh Hub queue QB for the same custodian.
- (e). Market-derived reserve is available to permit settlement.

### Scenario 2.
A user U has utility bucket 0 entitlement Q0 on custodianFor[U]. A third party settles y ≤ Q0 on the custodian via LiquidityHub.processSettlementFor, making Q0 stale. U then decreases/unwraps a commitment C (same custodian), creating fresh queue QC. U calls COLLECT_AVAILABLE_LIQUIDITY for tokenId=0; the function consumes shared hub queue (including QC) and pays to U under bucket 0, leaving C’s ledger QC outstanding but unable to collect now, blocking transfer/decommit of C.
#### Preconditions / Assumptions
- (a). User U’s utility bucket 0 has a recorded entitlement Q0 on custodianFor[U].
- (b). A third party has settled y ≤ Q0 on the custodian via LiquidityHub.processSettlementFor, making Q0 stale vs shared queue.
- (c). U operates a commitment C (same custodian) to create fresh queue QC.
- (d). Market-derived reserve is available to permit settlement.

### Scenario 3.
Owner O holds commitments A and B (same custodian). A was seized earlier; A’s per-bucket entitlement QA is recorded for the seizer S. A third party pre-settles x ≤ QA on the custodian. Later O decreases B, creating queue QB. S calls COLLECT_AVAILABLE_LIQUIDITY for A and consumes shared hub queue (which includes QB), receiving underlying while only A’s ledger is reduced. B’s ledger remains QB but cannot collect yet, so B’s NFT remains non-transferable/non-decommittable until new liquidity arrives.
#### Preconditions / Assumptions
- (a). Owner O has two commitments A and B sharing the same custodian (custodianFor[O]).
- (b). Commitment A was seized; MMQueueCustodian recorded QA for beneficiary S (the seizer).
- (c). A third party settled x ≤ QA on the custodian via LiquidityHub.processSettlementFor, making A’s local ledger stale.
- (d). Owner O later decreases B, creating fresh queue QB on the same custodian.
- (e). Market-derived reserve is available to permit settlement.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/84f5c03842082bd9984dbfd2174305fbd7ce4aaa/contracts/evm/src/LiquidityHub.sol)
...
+import {IMMQueueCustodian} from "./interfaces/IMMQueueCustodian.sol";
 ...
 contract LiquidityHub is BoundRegistry, Ownable, ReentrancyGuardTransient {
     ...
     function processSettlementFor(address lcc, address recipient, uint256 maxAmount)
         ...
         nonReentrant
     {
+        // For MM custodians (detected via positionManager()), restrict settlement to their bound PositionManager
+        if (recipient.code.length > 0) {
+            try IMMQueueCustodian(recipient).positionManager() returns (address pm) {
+                if (pm != address(0) && msg.sender != pm) revert Errors.NotApproved(msg.sender);
+            } catch {}
+        }
