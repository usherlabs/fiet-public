[Medium] Missing max-input slippage guard in MMPositionActionsImpl plain add-liquidity paths causes unbounded LCC spend on price moves

# Description

The plain add-liquidity actions (MINT_POSITION and INCREASE_LIQUIDITY) do not enforce per-leg max-input limits, allowing a fixed-liquidity add to consume more LCC than intended if the pool price moves before inclusion and the manager holds staged LCC balances.

In MMPositionActionsImpl, the plain add-liquidity paths _mintPosition/_increase call _mintPositionInternal/_increaseInternal, which execute modifyLiquidity and [compute principalDelta](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L660) but never compare the required inputs against user maxima, because these actions do not accept amount0Max/amount1Max. In contrast, the *_FROM_DELTAS variants [enforce maxima via _validateMaxIn](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L700) after executing the same internal add path. During a plain add, PositionManagerImpl._settleNegativeDeltas [settles negative caller deltas by transferring LCC from the MMPositionManager’s balances to the PoolManager](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/PositionManagerImpl.sol#L140-L153) with no cap. If the MMPositionManager (MMPM) holds LCC tokens (e.g., from earlier operations or preloads) with zero owner delta, a front-run price move can increase the token requirement for the same fixed liquidity and the transaction will still succeed without reverting, spending more of that leg than intended. The [end-of-batch NonzeroDeltaCount invariant](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L60-L66) does not prevent this when tokens are staged with zero delta, so the batch can complete cleanly despite worse execution.

# Severity

**Impact Explanation:** [High] A front-run price move can make the same fixed-liquidity add spend materially more LCC from the manager’s balances, resulting in direct principal loss to the operator due to the absence of max-input enforcement.

**Likelihood Explanation:** [Low] Exploitation requires uncommon operator postures (keeping LCC on the manager with zero deltas across batches or preloading off-protocol) and a feasible sandwich with sufficient price impact at execution time.
...
### Scenario 1.
Operator previously decreased liquidity leaving surplus immediate non-fee LCC on the MMPositionManager; in that batch they used [TAKE(lcc, to=self) to clear their positive delta while keeping the LCC tokens on the manager](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L92-L99). In a later batch, they submit a plain add (MINT_POSITION or INCREASE_LIQUIDITY) with fixed liquidity. An attacker sandwiches the transaction to move price so more of one LCC leg is required. PositionManagerImpl._settleNegativeDeltas [pays the negative deltas from the manager’s LCC balances](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/PositionManagerImpl.sol#L140-L153), no max-in is enforced, and the add succeeds while overspending LCC.
#### Preconditions / Assumptions
- (a). A live LCC/LCC pool with CoreHook and VTSOrchestrator configured.
- (b). Operator has an active commitment and position.
- (c). In an earlier batch, a decrease minted LCC to the MMPositionManager; only the queued slice was forwarded to custodian, leaving surplus immediate non-fee LCC on the manager.
- (d). The operator used TAKE(lcc, to=self) to clear their positive delta in that batch, leaving LCC tokens on the manager with zero owner delta.
- (e). Later, the operator uses a plain add-liquidity action (MINT_POSITION or INCREASE_LIQUIDITY) with a fixed liquidity amount (no maxima).
- (f). An attacker can execute a sandwich to shift price before inclusion.

### Scenario 2.
Operator preloads the MMPositionManager with LCC via direct ERC20 transfer (allowed because the manager is protocol-bound). Later, they submit a plain add (MINT_POSITION or INCREASE_LIQUIDITY) with fixed liquidity. An attacker sandwiches to move price, increasing the LCC required. The manager’s balances [fund the negative deltas](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/PositionManagerImpl.sol#L140-L153) without any max-input cap, so the add succeeds and consumes more LCC than expected.
#### Preconditions / Assumptions
- (a). A live LCC/LCC pool with CoreHook and VTSOrchestrator configured.
- (b). Operator preloads MMPositionManager with LCC via direct ERC20 transfer (manager is protocol-bound).
- (c). Operator uses a plain add-liquidity action (MINT_POSITION or INCREASE_LIQUIDITY) with a fixed liquidity amount (no maxima).
- (d). An attacker can execute a sandwich to shift price before inclusion.
...
+            // SECURITY: Plain add-liquidity path has no amount0Max/amount1Max slippage guard. Prefer *_FROM_DELTAS variants,
+            // or extend this action to accept maxima and validate via _validateMaxIn(principalDelta, ...).
...
+            // SECURITY: Plain add-liquidity path lacks max-input guards; consider guarded params or use *_FROM_DELTAS.
...
+        // SECURITY: If adding guarded params to this path, enforce slippage via
+        // _validateMaxIn(principalDelta, amount0Max, amount1Max) after computing principalDelta.
     }
     ...
     function _mintPositionInternal(
         ...
         (BalanceDelta liquidityDelta, BalanceDelta feesAccrued,) =
             _modifySyntheticLiquidity(poolKey, params, tokenId, hookData);
         principalDelta = liquidityDelta - feesAccrued;
+        // SECURITY: If extending plain mint to be guarded, validate maxima here:
+        // _validateMaxIn(principalDelta, amount0Max, amount1Max).
     }
 }
```
