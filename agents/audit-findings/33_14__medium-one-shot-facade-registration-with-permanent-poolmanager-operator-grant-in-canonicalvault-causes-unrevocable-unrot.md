[Medium] One-shot facade registration with permanent PoolManager operator grant in CanonicalVault causes unrevocable, unrotatable facade and potential ongoing abuse

# Description

[CanonicalVault.registerMarket](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/CanonicalVault.sol#L110) grants the market’s facade a permanent ERC‑6909 operator approval on PoolManager claims and stores the facade immutably, while MarketFactory and [BoundRegistry](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/BoundRegistry.sol#L62-L69) provide no rotation/demotion paths and LiquidityHub does not expose issuer revocation. A compromised or buggy facade cannot be quarantined or rotated for a live market, forcing migration and enabling ongoing abuse of privileged flows.

During market creation, CanonicalVault.registerMarket assigns the facade (ProxyHook) and [calls IERC6909Claims(address(poolManager)).setOperator(facade, true)](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/CanonicalVault.sol#L110), granting owner-wide operator rights over CanonicalVault’s PoolManager claims. CanonicalVault offers no revoke/rotate function for this grant or the stored facade. MarketFactory has no method to rotate the proxy hook for an existing market; [isMarketFacade just checks the originally stored mapping](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MarketFactory.sol#L534). [BoundRegistry makes EXEMPT/DEX bound levels immutable once set](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/BoundRegistry.sol#L62-L69), preventing later demotion of the facade. LiquidityHub includes the facade in [initial issuers](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MarketFactory.sol#L311-L313) with no public revocation function. Together, these decisions permanently entrench the facade’s privileges; if it is later found buggy or compromised, there is no on-chain way to revoke or rotate it for the live market. Although ERC‑6909 operator status alone does not trivially enable theft (v4 pot/claim semantics constrain arbitrary withdrawals), the facade’s privileged flows (e.g., deficit routing via [queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/CanonicalVault.sol#L234)) can be abused by a compromised implementation. The only practical mitigation is to create a new market and migrate users, causing downtime and fragmentation while leaving the old, compromised facade privileged indefinitely.

# Severity

**Impact Explanation:** [Medium] Inability to rotate or revoke a privileged facade for a live market leads to significant operational disruption (forced migration, downtime) and enables ongoing abuse if the facade is compromised (principal losses via misrouted deficits). While operator status alone does not guarantee theft, the combined privileges create material availability and safety risks.

**Likelihood Explanation:** [Medium] It is plausible in real deployments to discover serious bugs in a complex hook (Scenario 1). Direct principal loss scenarios (Scenario 2) require an additional compromise or bug, which lowers their likelihood, but the overall issue (no rotation/quarantine) remains a realistic operational risk.
...
### Scenario 1.
Emergency rotation is needed after a critical ProxyHook bug is discovered, but CanonicalVault, MarketFactory, BoundRegistry, and LiquidityHub provide no on-chain path to revoke, demote, or rotate the facade. Operators must create a new market and force users to migrate, causing downtime and liquidity fragmentation.
#### Preconditions / Assumptions
- (a). A critical bug is discovered in the deployed ProxyHook (facade) for a live market.
- (b). No on-chain rotation/revocation exists for the facade in CanonicalVault or MarketFactory.
- (c). BoundRegistry EXEMPT level is immutable and cannot be demoted.
- (d). LiquidityHub issuer rights for the facade cannot be revoked.
- (e). Users/LPs need to continue operations or exit positions.

### Scenario 2.
A compromised or buggy ProxyHook misroutes deficit settlements to an attacker-controlled recipient during proxy swaps. Because the facade’s issuer, EXEMPT bound level, and CanonicalVault access cannot be revoked or rotated, the attacker can continue siphoning deficits over time; users suffer direct principal losses on impacted swaps.
#### Preconditions / Assumptions
- (a). The ProxyHook code is compromised or contains an exploitable bug enabling misdirection of deficitRecipient.
- (b). Users continue to trade against the proxy pool (invoking ProxyHook callbacks).
- (c). No on-chain rotation/revocation/demotion of the compromised facade is possible (CanonicalVault/MarketFactory/BoundRegistry/LiquidityHub constraints).
- (d). Uniswap v4 PoolManager behaves canonically (no reliance on PoolManager misbehavior).

### Scenario 3.
The owner-wide ERC‑6909 operator grant to the facade for CanonicalVault’s claims cannot be revoked. While not a direct theft vector by itself due to v4 claim/pot constraints, it amplifies risk and blast radius if the compromised facade’s code can manipulate claim operations across currencies.
#### Preconditions / Assumptions
- (a). Same as Scenario 2 (compromised facade).
- (b). The ERC‑6909 operator grant on PoolManager claims is owner-wide for CanonicalVault and cannot be revoked in CanonicalVault.
- (c). Potential claim manipulation depends on the compromised facade’s code paths and v4 semantics.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/CanonicalVault.sol)
...
+        // SECURITY: Hard-binding to factory's current facade prevents emergency rotation from CanonicalVault.
+        // Consider adding a factory-only rotation function in this contract to update `cfg.facade`, and
+        // (for legacy deployments) revoke/grant PoolManager operator accordingly. Then gate solely on `cfg.facade`.
...
+        // SECURITY: Avoid granting permanent ERC6909 operator to the facade.
+        // Option A (preferred): remove this grant and ensure all PoolManager claim ops for CanonicalVault
+        // are executed by CanonicalVault itself (ProxyHook should call CanonicalVault wrappers).
+        // Option B: add an onlyFactory rotation function to update `markets[marketId].facade`, clear
+        // `facadeToMarket[old]`, and revoke/grant operator for old/new facades to enable emergency rotation.
