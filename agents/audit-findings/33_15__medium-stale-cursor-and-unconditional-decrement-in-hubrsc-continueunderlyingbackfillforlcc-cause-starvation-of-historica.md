[Medium] Stale cursor and unconditional decrement in HubRSC._continueUnderlyingBackfillForLcc cause starvation of historical recipients in shared-underlying dispatch

# Description

The bounded backfill in HubRSC can prematurely mark completion due to a stale resume cursor and unconditional decrementing of a snapshot counter, leaving some still-live historical keys unmirrored to the shared-underlying queue and causing automated settlement starvation when shared-underlying routing is preferred.

In HubRSC._continueUnderlyingBackfillForLcc, the backfill progress is tracked by [underlyingBackfillRemainingByLcc[lcc]](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L613) (a snapshot of per-LCC queue size at registration) and a resume cursor [underlyingBackfillCursorByLcc[lcc]](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L764) saved as [the "next" key to visit](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L750-L752). [Each loop iteration decrements the remaining counter](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L758-L759) even if no unique, still-live key was mirrored (e.g., when the cursor points to a deleted key or a key already mirrored). If the saved "next" key is later removed, [LinkedQueue.nextOrHead(key) returns the head](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/libs/LinkedQueue.sol#L73-L75), causing re-scans of already-mirrored prefix keys that still consume the remaining counter. As a result, remaining can reach zero without ever visiting some later still-live historical keys. [The LCC is then dropped from the backfill worklist](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L728-L733), and since [_dispatchLiquidity prefers the shared-underlying lane whenever it is non-empty](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L428), the missed keys remain invisible to automated dispatch until the lane drains or they receive new SettlementQueued events post-registration. This is a liveness/fairness issue, not a hard freeze: the protocol exposes a permissionless settlement path (LiquidityHub.processSettlementFor) that can still settle recipients directly when reserves are available.

# Severity

**Impact Explanation:** [Medium] Automated settlement fairness/liveness is impaired for some recipients; however, funds are not frozen because anyone can permissionlessly call LiquidityHub.processSettlementFor to settle when reserves are available.

**Likelihood Explanation:** [Medium] Requires a combination of plausible conditions (large backlog vs. budget, intermittent cursor deletions, and a typically non-empty shared-underlying lane). No trusted-role misuse or exotic states are required.
...
### Scenario 1.
Stale cursor plus small backfill budgets: An LCC with a large historical per-LCC backlog registers its underlying. Initial mirroring copies only a small prefix. Before the next pass, the saved resume cursor key is pruned (e.g., processed/annulled). The next pass starts from a deleted key, wraps to head, and re-scans already-mirrored entries while still decrementing the remaining counter. After enough such passes, backfill is marked complete without ever mirroring later still-live keys, which remain invisible to shared-underlying dispatch as long as that lane stays non-empty.
#### Preconditions / Assumptions
- (a). Per-LCC historical backlog size exceeds backfill budget (maxDispatchItems)
- (b). Underlying is registered for the LCC (hasUnderlyingForLcc true)
- (c). Saved resume cursor keys can be pruned between passes (e.g., due to processing/annulments)
- (d). Shared-underlying queue for the underlying is often non-empty (routing prefers it)
- (e). No fresh SettlementQueued increases for the missed keys after registration

### Scenario 2.
Busy shared-underlying lane suppresses fallback: After the same premature completion, multiple LCCs on the same underlying keep the shared-underlying queue non-empty. Routing always prefers the underlying lane, so the unmirrored historical keys are never considered by automated dispatch for an extended period, as per-LCC fallback does not trigger while the underlying lane remains populated.
#### Preconditions / Assumptions
- (a). Per-LCC historical backlog size exceeds backfill budget
- (b). Stale-cursor deletions occur between backfill passes
- (c). Shared-underlying queue remains non-empty due to ongoing activity from this or other LCCs on the same underlying
- (d). No fresh SettlementQueued increases for the missed keys after registration

### Scenario 3.
Operator annulments accelerate remaining burn: Following registration, the operator legitimately annuls a block of early keys that intersect the saved resume cursors. Subsequent backfill passes frequently start from deleted cursors, repeatedly re-scanning the head prefix and consuming the remaining counter without reaching tail keys. Backfill completes prematurely and the missed keys stay invisible to automated dispatch while the underlying lane is non-empty.
#### Preconditions / Assumptions
- (a). Per-LCC historical backlog size exceeds backfill budget
- (b). Legitimate operator annulments intersect future saved resume cursors (causing deletions)
- (c). Shared-underlying queue is frequently non-empty
- (d). No fresh SettlementQueued increases for the missed keys after registration
...
+                // Only count progress when we actually mirror a new key to the underlying lane
+                bool wasPresent = queueDataByUnderlying[underlying].inQueue[key];
...
## [Medium] Tail-append backfill and bounded scan in HubRSC shared-underlying dispatch causes unbounded delay of older settlements

### Description

HubRSC [mirrors historical per-LCC queue entries to the shared-underlying lane in chunks](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L608-L621) and [appends them at the tail](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/libs/LinkedQueue.sol#L24-L32) while [immediately switching dispatch to the shared lane](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L427-L433). With small per-event liquidity and continuous new arrivals (also tail-appended), the bounded scan often advances the cursor by only a few items before [liquidity is exhausted](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L446-L471). [Zero-batch retry does not trigger in this common case](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L509-L523). This can cause unbounded delays and practical starvation of older, pre-registration settlements.

HubRSC’s underlying registration (_registerLccUnderlying) seeds only up to maxDispatchItems of a per-LCC backlog into the shared-underlying queue via [_initializeUnderlyingBackfill](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L608-L621) and defers the rest to _continueUnderlyingBackfill, which [appends later mirrored keys to the tail](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L753-L756). New post-registration arrivals for the same underlying are also appended to the shared-underlying tail via [_enqueueUnderlyingKey](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L625-L628). Once the shared-underlying queue is non-empty, [_dispatchLiquidity chooses it over the per-LCC queue](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L427-L433). The dispatcher scans up to a cap while remainingLiquidity > 0 and [stops as soon as liquidity is consumed](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L446-L471), often after the first dispatchable key. The queue cursor then advances only by the number of scanned entries. Zero-batch retry (_handleZeroBatchRetry) only applies when batchCount == 0; [it does nothing when at least one item was dispatched](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/reactive/src/HubRSC.sol#L509-L523). Under sustained low-liquidity/high-arrival conditions, many new tail-inserted keys can consistently land between the moving cursor and older backfilled keys in the ring order, so the distance to those older keys does not reliably shrink. This yields unbounded delays and effective starvation for some older settlements, particularly those surfaced late by backfill. Settlement safety is not compromised (no principal loss), but liveness/fairness and timely payouts are materially degraded.

### Severity

**Impact Explanation:** [Medium] This causes significant but temporary availability loss/DoS of timely settlement via the auto-dispatch path for some recipients. Funds are not lost and permissionless self-settlement remains possible, preventing a high-impact classification.

**Likelihood Explanation:** [Medium] Conditions such as low-liquidity windows and continuous arrivals are plausible in real deployments. An attacker can economically justify earlier payouts; Scenario 2 arises naturally without an attacker. Constraints exist but are not rare or exceptional.
...
### Scenario 1.
Adversarial flooding front-runs scarce liquidity: After an LCC’s underlying is registered, only the first chunk of older per-LCC recipients is mirrored to the shared-underlying queue; the rest will be appended later by backfill. An attacker repeatedly creates new queued settlements at fresh EOAs (each a unique key) so each liquidity event finds an attacker-controlled key early, consumes the small available amount, and advances the cursor by only a few steps. Meanwhile, many newly appended keys (including late backfill) accumulate between the cursor and older victims. Older pre-registration recipients experience unbounded delay in being dispatched.
#### Preconditions / Assumptions
- (a). An LCC A shares underlying U
- (b). Before underlying registration, many recipients queued settlements for A (per-LCC backlog exists)
- (c). Per-event available liquidity is small for extended periods
- (d). Attacker holds sufficient LCC and can create new SettlementQueued entries across fresh EOAs
- (e). Backfill appends historical keys to the shared-underlying tail over time

### Scenario 2.
Natural multi-LCC contention on a shared underlying: Many LCCs share the same underlying and each has a historical per-LCC backlog. The per-event backfill budget rotates across all LCCs, so any one LCC’s older backlog is mirrored slowly and appended at the tail behind ongoing new arrivals from all LCCs. Shared-lane dispatch frequently consumes small liquidity early in the window and advances the cursor minimally, delaying visibility and then dispatch of older entries without any malicious actor.
#### Preconditions / Assumptions
- (a). Multiple LCCs share the same underlying U
- (b). Each LCC has a historical per-LCC backlog before registration
- (c). Per-event available liquidity is modest
- (d). Backfill budget per event is limited and rotates across LCCs
- (e). No malicious actor is required

### Scenario 3.
Suppressing zero-batch progression: The attacker maintains many keys near the current cursor so each scan finds at least one dispatchable key that fully absorbs the small available amount, making batchCount > 0 and preventing zero-batch retry. The cursor advances by only a few scanned entries per event while continuous tail appends keep older backfilled keys effectively out of reach for long periods.
#### Preconditions / Assumptions
- (a). Same as Scenario 1 (shared underlying, pre-registration backlog, small per-event liquidity)
- (b). Attacker can keep at least one attacker-controlled key near the cursor with enough unreserved amount to absorb all small available per event
- (c). Attacker continues appending new keys between events to maintain pressure
...
```diff
 ...
 contract HubRSC is AbstractReactive {
     ...
     function _dispatchLiquidity(address lcc, uint256 available) internal {
         ...
         // historical backlog may still exist only in the per-LCC queue.
-        bool useSharedUnderlying = hasUnderlyingForLcc[lcc] && queueDataByUnderlying[underlying].size > 0;
+        bool useSharedUnderlying = hasUnderlyingForLcc[lcc] && queueDataByUnderlying[underlying].size > 0 && underlyingBackfillRemainingByLcc[lcc] == 0;
         address dispatchLane = useSharedUnderlying ? underlying : lcc;
         _clearInactiveZeroBatchRetryCredits(lcc, underlying, useSharedUnderlying);
...
+                if (state.remainingLiquidity == 0) { state.scanned++; continue; }
                 uint256 settleAmount =
                     dispatchable <= state.remainingLiquidity ? dispatchable : state.remainingLiquidity;
