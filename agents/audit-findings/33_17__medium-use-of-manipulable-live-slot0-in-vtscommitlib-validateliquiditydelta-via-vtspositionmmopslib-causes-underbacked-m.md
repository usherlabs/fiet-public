[Medium] Use of manipulable live slot0 in VTSCommitLib.validateLiquidityDelta (via VTSPositionMMOpsLib) causes underbacked MM position admission

# Description

The MM add-liquidity admission check computes issued USD value from the live Uniswap v4 slot0 and validates against signal + settled. Because slot0 can be manipulated just before add-liquidity, an attacker can pre-swap to a favorable price, pass admission with insufficient signal, get LCC issued, and leave an underbacked position until a later commitment-backed checkpoint.

On MM liquidity increases, VTSPositionMMOpsLib._handleLiquidityIncrease reads the current pool price (slot0) and calls VTSCommitLib.validateLiquidityDelta to ensure issuedUsd <= settledUsd + signalUsd. validateLiquidityDelta computes issuedUsd by deriving effective token amounts with LiquidityUtils.calculateEffectiveTokenAmounts at the provided live slot0 and valuing them using oracle USD prices. There is no TWAP or worst-case-over-range guard. Adding liquidity does not move price, so an attacker can pre-swap to a favorable point inside/near the position’s range within the same transaction, making the instantaneous issuedUsd look small enough to pass. LCC is then issued. After arbitrage restores price, the same position’s issuedUsd at oracle prices exceeds what admission allowed, leaving the position underbacked until a later commitment-backed checkpoint recognizes a deficit. Immediate reserve drain is prevented by RFS/remove gating and Hub reserve accounting, but the admission-time solvency assurance is weakened and systemic underbacking risk increases.

# Severity

**Impact Explanation:** [Medium] No immediate principal theft or reserve drain occurs due to RFS/remove gating and Hub reserve accounting, but the admission-time solvency assurance is weakened. Underbacked positions can be admitted, increasing systemic risk (higher likelihood and severity of later RFS/deficit/seizure events).

**Likelihood Explanation:** [Medium] Exploitation requires capital to move price and transaction ordering control (e.g., MEV), which are significant but realistic constraints in DeFi. No reliance on out-of-scope integrations or trusted-role misuse.
...
### Scenario 1.
Single-tx admission bypass for a new position: The attacker pre-swaps the core pool to a favorable price and immediately adds liquidity. validateLiquidityDelta uses the manipulated slot0 and passes; LCC is issued. After price reverts, the position is underbacked until a later commitment-backed checkpoint.
#### Preconditions / Assumptions
- (a). Attacker is a market maker with a valid commit signal that would be insufficient under honest pricing for the intended add-liquidity.
- (b). Attacker can move Uniswap v4 spot within the same transaction (sufficient capital and transaction ordering/MEV control).
- (c). OracleHelper prices are accurate and independent of AMM spot.
- (d). Protocol admission uses live slot0 without TWAP or worst-case valuation; RFS and Hub reserve protections are in place.

### Scenario 2.
Batched expansions while pinning spot: Within one block/bundle, the attacker maintains a favorable slot0 and performs multiple add-liquidity operations. Each admission passes at the manipulated spot; later reversion reveals aggregate underbacked exposure across positions.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1.
- (b). Attacker can maintain the favorable spot across multiple add-liquidity calls in one block/bundle (e.g., private relay/MEV).

### Scenario 3.
Top-up of an existing position: The attacker pre-swaps to a favorable spot and adds liquidity to an existing active position. Admission passes under spot-based validation; after reversion the enlarged position becomes underbacked until checkpoint.
#### Preconditions / Assumptions
- (a). OracleHelper prices are accurate and independent of AMM spot.
- (b). An existing active MM position with some settled backing and limited remaining signal.
- (c). Attacker can pre-swap to a favorable slot0 within the transaction (capital and ordering control).
- (d). Admission uses live slot0; no TWAP or worst-case range check.
...
-        );
+        // Admission hardening: value issuance against range commitment maxima (manipulation-resistant),
+        // not the instantaneous spot composition.
+        {
+            uint128 L = SafeCast.toUint128(uint256(params.liquidityDelta));
+            (uint256 c0, uint256 c1) =
+                LiquidityUtils.calculateCommitmentMaxima(params.tickLower, params.tickUpper, L);
+            issuedValue = OracleUtils.lccPairValue(
+                oracleHelper,
