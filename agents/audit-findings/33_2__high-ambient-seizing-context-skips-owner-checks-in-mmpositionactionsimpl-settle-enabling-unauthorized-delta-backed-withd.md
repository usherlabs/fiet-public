[High] Ambient seizing context skips owner checks in MMPositionActionsImpl._settle, enabling unauthorized delta-backed withdrawals and vault principal loss

# Description

While a batch-scoped [ambient seizing flag is set by SEIZE_POSITION](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L345-L349), [MMPositionActionsImpl._settle skips NFT owner/approval checks](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L525-L526) and [only blocks deposit-only settles](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L530), [VTS allows delta-backed withdrawals during seizing](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L470-L487) and the [router forwards withdrawn underlying to the caller EOA](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L483-L486), enabling a third-party seizer to extract vault-funded value without NFT ownership or approval.

SEIZE_POSITION sets a [transient seizing flag for a specific position](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L345-L349) that [persists until the batch ends](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L62-L63). During this period, [MMPositionActionsImpl._settle does not enforce NFT owner/approval for that position](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L525-L526) and [only disallows deposit-only settles outside the primary seize settle](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L530). The companion guard in _settleFromDeltas [bans only the deposit cell (payerIsUser=true && shouldTake=false) under ambient seizing](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L788-L806), leaving withdrawal lanes permitted. In VTS, when isSeizing=true, [withdrawal planning permits only delta-backed withdrawals (settled-backed withdrawals are disallowed)](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L470-L487), drawing from router-wide OwnerCurrencyDelta (not position-scoped). MMPositionActionsImpl._processSettlementTransfers then [forwards any positive deltas (withdrawals) directly to msgSender() when usePositionManagerBalance=false](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/MMPositionActionsImpl.sol#L483-L486). As a result, a third-party seizer can, in the same batch, call a withdraw settle (SETTLE_POSITION or SETTLE_POSITION_FROM_DELTAS with shouldTake=true) on the seized position and receive underlying to their EOA without owner approval, provided router-level underlying credits and vault claims exist. This produces unauthorized principal loss from the market's vault.

# Severity

**Impact Explanation:** [High] Unauthorized withdrawals route underlying (principal) from the market’s vault/claims to the attacker EOA without NFT owner/approval, constituting direct, material loss of principal funds.

**Likelihood Explanation:** [Medium] Exploitation requires a seizable position and sufficient vault claims; both are realistic and commonly satisfied in normal protocol operation, though not guaranteed at every instant.
...
### Scenario 1.
After calling SEIZE_POSITION to set ambient seizing and executing the seizing decrease, the attacker immediately calls SETTLE_POSITION_FROM_DELTAS with payerIsUser=true and shouldTake=true on the same position; the withdrawal consumes the router’s newly created underlying credits from the seizing decrease, and MMPositionActionsImpl forwards the underlying directly to the attacker EOA because usePositionManagerBalance=false.
#### Preconditions / Assumptions
- (a). The position is seizable (grace period elapsed and seizability checks pass).
- (b). CanonicalVault has sufficient PoolManager claims to fund the immediate settleable slice.
- (c). The attacker can execute multiple actions in the same batch (single unlock session).
- (d). The seizing decrease produces router-level positive underlying credits (OwnerCurrencyDelta) for the same pair.

### Scenario 2.
The attacker first calls SEIZE_POSITION on any seizable position in the same market to set ambient seizing, then calls a withdrawal settle (SETTLE_POSITION or SETTLE_POSITION_FROM_DELTAS with shouldTake=true) to drain pre-existing router-level positive underlying credits for that pair, receiving underlying directly to their EOA.
#### Preconditions / Assumptions
- (a). At least one position in the market is seizable to set ambient seizing.
- (b). The router (MMPositionManager) already holds pre-existing positive OwnerCurrencyDelta for the same underlying pair.
- (c). CanonicalVault has sufficient PoolManager claims at execution time.

### Scenario 3.
The attacker calls SEIZE_POSITION and then SETTLE_POSITION with positive withdrawal amounts on the same position; under ambient seizing, _settle skips owner checks and VTS allows delta-backed withdrawal, so CanonicalVault pays out (subject to claims) and the router forwards the underlying to the attacker EOA.
#### Preconditions / Assumptions
- (a). A seizable position exists and is seized in the batch.
- (b). Router-level positive underlying credits exist (e.g., from the immediately preceding seizing decrease).
- (c). CanonicalVault has sufficient PoolManager claims to service the requested withdrawal.
...
+                revert Errors.SeizureSettleOnlyDepositDisallowed();
+            }
+            bool inPrimarySeizeSettle =
+                isSeizing && TransientSlots.getSeizurePrimarySettleAllowed() && isDepositLane;
