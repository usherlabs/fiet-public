[Medium] Missing recipient sink-address guard in LiquidityHubLib.transferUnderlying causes permanent loss of reserves

# Description

LiquidityHub fallback payouts can send assets to sink addresses (WETH9 or the underlying ERC20 contract itself), permanently locking funds and depleting shared reserves used for user settlements.

When settling queued claims, [LiquidityHubLib.transferUnderlying](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/LiquidityHubLib.sol#L562) deducts reserve and pays the recipient. For native ETH, it [attempts raw ETH only for EOAs or INativeSettlementReceiver contracts](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/LiquidityHubLib.sol#L587-L592); otherwise it [deposits into WETH9 and transfers WETH to the recipient](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/LiquidityHubLib.sol#L595-L597). There is no guard against recipient == weth9(), so the Hub can transfer newly minted WETH to the WETH9 contract’s own address, permanently locking value. For ERC20 underlyings, [transferUnderlying similarly transfers tokens directly to the recipient](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/LiquidityHubLib.sol#L600) without forbidding recipient == underlying token contract address, which typically cannot move its self-held tokens. Queue admission ( [queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/LiquidityHub.sol#L731) ) allows arbitrary non-exempt/non-DEX recipients holding market-derived LCC; ProxyHook’s deficit routing [first transfers deficit LCC to the chosen recipient (crediting marketDerivedBalances) and then queues settlement](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/ProxyHook.sol#L284-L286), making these sink recipients admissible. Later, [processSettlementFor burns the recipient’s LCC and executes transferUnderlying](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/LiquidityHubLib.sol#L550-L553), irreversibly blackholing the underlying. This drains shared reserves, harming other legitimate queued recipients.

# Severity

**Impact Explanation:** [High] Principal funds in the Hub’s reserves are permanently locked at sink addresses (WETH9 or the ERC20 token contract), directly reducing liquidity available to legitimate queued recipients with no recovery path.

**Likelihood Explanation:** [Low] Exploitation is primarily griefing: the attacker routes value to a sink they do not control, gaining no direct profit while harming others. Although feasible in normal operation, the attacker’s irrational trade-off places this in low likelihood per the rules.
...
### Scenario 1.
Native-underlying market: An attacker performs an exact-input proxy swap with hookData specifying deficitRecipient = WETH9. ProxyHook transfers deficit LCC to WETH9, queues settlement for WETH9, and later settlement deposits ETH into WETH9 then transfers WETH to the WETH9 contract address, permanently locking the funds and depleting Hub reserves.
#### Preconditions / Assumptions
- (a). An LCC exists with native ETH as underlying
- (b). ProxyHook supports user-defined deficitRecipient via hookData
- (c). WETH9 is canonical (no EIP-165, no INativeSettlementReceiver) and cannot move self-held tokens
- (d). WETH9 is not bound EXEMPT/DEX in BoundRegistry (defaults to NONE)
- (e). LiquidityHub has or will have marketDerived reserve for native ETH

### Scenario 2.
ERC20-underlying market: An attacker sets deficitRecipient = the underlying ERC20 token contract address. ProxyHook credits the contract address with market-derived LCC and queues settlement. On settlement, transferUnderlying sends the ERC20 tokens to the token’s own contract address, permanently locking them and depleting Hub reserves.
#### Preconditions / Assumptions
- (a). An LCC exists with an ERC20 underlying
- (b). The underlying ERC20 token contract cannot move tokens held at its own address
- (c). The ERC20 token contract is not bound EXEMPT/DEX (defaults to NONE)
- (d). LiquidityHub has or will have marketDerived reserve for that ERC20

### Scenario 3.
Batch settlement exacerbation: After creating one or more sink queue entries as above, automated batch processors include these recipients and call processSettlementFor, repeatedly blackholing reserves into WETH9 or ERC20 token contract addresses and reducing liquidity for legitimate recipients.
#### Preconditions / Assumptions
- (a). At least one sink recipient queue entry exists as in Scenarios 1 or 2
- (b). Batch settlement automation or a caller executes processSettlementFor on these recipients
- (c). LiquidityHub has sufficient reserve to settle
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/LiquidityHubLib.sol)
...
+            // Prevent blackholing: do not send WETH to the WETH9 contract itself.
+            if (account == wrappedNative) revert Errors.NotApproved(account);
+
...
+        // Prevent blackholing: do not send ERC20 to its own token contract address.
+        if (account == underlying) revert Errors.NotApproved(account);
