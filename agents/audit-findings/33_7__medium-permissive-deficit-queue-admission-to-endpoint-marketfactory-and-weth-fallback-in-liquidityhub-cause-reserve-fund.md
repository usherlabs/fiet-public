[Medium] Permissive deficit queue admission to endpoint MarketFactory and WETH fallback in LiquidityHub cause reserve-funded assets to be stranded on MarketFactory

# Description

Exact-input proxy swaps can route market-derived LCC deficits to MarketFactory, which LiquidityHub admits as a valid endpoint recipient. Subsequent settlement burns the queued LCC and spends the Hub’s market-derived reserve, transferring underlying (WETH for native or ERC20 for non-native) to MarketFactory. Since MarketFactory is not an INativeSettlementReceiver and has no ERC20 sweep, these assets are irrecoverably stranded, draining shared reserve without benefiting users.

ProxyHook allows the swapper to specify a deficit recipient via hookData. When an exact-input proxy swap produces a deficit, ProxyHook transfers market-derived LCC to that recipient and calls LiquidityHub.queueForTransferRecipient. LiquidityHub admits recipients that are not Hub/exempt/DEX and that hold sufficient market-derived LCC. MarketFactory marks itself as a BOUND_ENDPOINT in initialise and therefore passes admission. Later, anyone can call LiquidityHub.processSettlementFor to settle the queue: it burns the recipient’s market-derived LCC, reduces the Hub’s market-derived reserve, and transfers underlying to the recipient. For native-backed LCCs, LiquidityHubLib.transferUnderlying uses WETH fallback for contracts not implementing INativeSettlementReceiver, then transfers WETH to MarketFactory; for ERC20-backed LCCs, it transfers the ERC20 directly. MarketFactory exposes no INativeSettlementReceiver support and no ERC20 sweep/withdraw functions, so the transferred assets become permanently stuck on MarketFactory. This results in a deterministic, in-protocol sink of reserve funds.

# Severity

**Impact Explanation:** [High] Settlement spends the Hub’s market-derived principal reserve and transfers underlying to MarketFactory where it is irrecoverably stranded, constituting direct, material loss of protocol funds and reducing ability to service user settlements.

**Likelihood Explanation:** [Low] The attacker gains no direct profit and must deliberately grief the system by routing deficits to a sink and timing settlements with reserve availability.
...
### Scenario 1.
Native-backed market: an attacker performs an exact-input proxy swap with hookData set to MarketFactory, creating a deficit; ProxyHook transfers deficit LCC to MarketFactory and queues it; processSettlementFor later burns the LCC and sends WETH to MarketFactory where it is stranded.
#### Preconditions / Assumptions
- (a). A native-backed market (ETH) exists with LCCs created via MarketFactory
- (b). MarketFactory.initialise has set MarketFactory as BOUND_ENDPOINT (not exempt/DEX)
- (c). ProxyHook is an issuer for the market’s LCCs
- (d). Some market-derived reserve is or will be present in the Hub
- (e). MarketFactory does not implement INativeSettlementReceiver and has no ERC20/WETH sweep
- (f). LiquidityHub.processSettlementFor is permissionless

### Scenario 2.
ERC20-backed market: same flow as above but the underlying is a standard ERC20; processSettlementFor transfers the ERC20 to MarketFactory, which has no sweep function, permanently stranding the tokens.
#### Preconditions / Assumptions
- (a). An ERC20-backed market exists with LCCs created via MarketFactory
- (b). MarketFactory.initialise has set MarketFactory as BOUND_ENDPOINT (not exempt/DEX)
- (c). ProxyHook is an issuer for the market’s LCCs
- (d). Some market-derived reserve is or will be present in the Hub
- (e). MarketFactory has no ERC20 sweep/withdraw function
- (f). LiquidityHub.processSettlementFor is permissionless

### Scenario 3.
Progressive drain: the attacker repeatedly routes deficits to MarketFactory and settles them whenever reserves become available, cumulatively depleting the Hub’s market-derived reserves and degrading serviceability for legitimate users.
#### Preconditions / Assumptions
- (a). At least one market (native or ERC20) exists with LCCs created via MarketFactory
- (b). MarketFactory.initialise has set MarketFactory as BOUND_ENDPOINT (not exempt/DEX)
- (c). ProxyHook is an issuer for the market’s LCCs
- (d). Market-derived reserves are intermittently added to the Hub through normal operation
- (e). MarketFactory lacks INativeSettlementReceiver (for native) and sweep functions
- (f). LiquidityHub.processSettlementFor is permissionless and can be called over time
...
+        // For native-backed LCCs, require explicit INativeSettlementReceiver opt-in for contract recipients to avoid WETH fallback sinks.
+        if (s.lccToUnderlying[lcc] == address(0) && recipient.code.length > 0) {
+            // ERC-165 supportsInterface(selector) = 0x01ffc9a7; INativeSettlementReceiver id = bytes4(keccak256("supportsNativeSettlementFromFiet()"))
+            bytes4 iid = bytes4(keccak256("supportsNativeSettlementFromFiet()"));
+            (bool ok, bytes memory ret) = recipient.staticcall(abi.encodeWithSelector(0x01ffc9a7, iid));
...
+        // Disallow factories (including this LCC's market factory) as queue owners to prevent reserve-funded sinks.
+        if (isFactory[recipient] || recipient == s.lccToMarket[lcc].factory) {
+            revert Errors.NotApproved(recipient);
+        }
+
