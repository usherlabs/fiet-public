[Medium] Double-flooring of commitment shortfall in VTSCommitLib._checkpointWithCommitment causes hidden deficits and disables insolvency controls

# Description

When backing is insufficient, VTSCommitLib._checkpointWithCommitment [floors the USD shortfall to whole basis points and then floors per-lane token deficits from that bps value](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSCommitLib.sol#L402-L405). Any shortfall below 1 bps writes zero per-lane token deficits and zero bps severity, even if the absolute token shortfall is large for big positions. Downstream insolvency protections that hinge on these values (freeze, RFS inflation, and deficit-based immediate seizure bypass) fail to trigger.

In VTSCommitLib._checkpointWithCommitment, under-backing is computed as [deficitUsd = issuedUsd − (signalUsd + settledUsd)](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSCommitLib.sol#L401). The code then computes deficitBps = floor(deficitUsd * 10000 / issuedUsd) and per-lane token deficits as [floor(effN * deficitBps / 10000)](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSCommitLib.sol#L404-L405). When the USD shortfall ratio is below 1 bps, deficitBps = 0, so both lane deficits are written as 0 and commitmentDeficitBps = 0. Key protections use these floored values: (1) VTSPositionLib._touchExistingPositionPath [blocks non-seizing liquidity changes only if per-lane token deficits > 0](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSPositionLib.sol#L1104-L1114). With rounding to zero, the freeze does not apply. (2) [VTSPositionLib.getRFS inflates required settlement by these token deficits](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/VTSPositionLib.sol#L1367-L1383); if zero, RFS is not increased. (3) [CheckpointLibrary.isSeizable permits deficit-based immediate seizure bypass only when per-lane token deficits are non-zero and either the bps severity](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/Checkpoint.sol#L58-L66) or [per-token absolute thresholds/age gates are met](https://github.com/usherlabs/fiet-protocol/blob/cbac5017a6e67bb7f59c30dfcf35a5fb9142fb4c/contracts/evm/src/libraries/Checkpoint.sol#L78-L81). With bps = 0 and token deficits = 0, bypass cannot trigger, even for large absolute token shortfalls at sub-1 bps ratios. This contradicts the intended design of handling large single-token deficits even when overall USD severity is small and allows large positions to avoid insolvency controls under sub‑1 bps conditions.

# Severity

**Impact Explanation:** [Medium] The bug disables important insolvency controls (deficit-based seizure bypass, freeze triggers, and RFS inflation) for sub‑1 bps shortfalls, weakening risk management but not directly minting, burning, stealing, or freezing funds.

**Likelihood Explanation:** [Medium] Maintaining sub‑1 bps under-backing while exceeding large absolute per-token thresholds generally requires notable capital and operational tuning for large positions, but is realistic and feasible.
...
### Scenario 1.
Evading deficit-based immediate seizure: A large position maintains a live VRL signal such that the USD shortfall ratio is 0.9 bps while the implied token1 shortfall (if computed directly) exceeds the configured per-token threshold. After checkpoint(withCommitment=true), commitmentDeficitBps=0 and per-lane token deficits=0, so CheckpointLibrary.isSeizable cannot use the deficit-bypass path despite a large absolute token deficit.
#### Preconditions / Assumptions
- (a). MarketVTSConfiguration has unbackedCommitmentGraceBypassBps > 1 bps and a non-zero per-token absolute threshold (e.g., token1.unbackedCommitmentGraceBypassThreshold).
- (b). A very large position with issuedUsd high enough that a <1 bps USD ratio implies a large absolute token shortfall.
- (c). A live VRL-backed signal (non-expired) with reserves within <1 bps of issuedUsd.
- (d). A commitment checkpoint is executed with withCommitment=true.

### Scenario 2.
Avoiding insolvency freeze on non-seizing removes: With a sub-1 bps shortfall rounded to zero per-lane deficits, VTSPositionLib._touchExistingPositionPath does not block a non-seizing MM remove-liquidity operation. Provided RFS is closed (no deficit inflation), the MM can remove liquidity while under-backed, bypassing the intended freeze keyed off non-zero token deficits.
#### Preconditions / Assumptions
- (a). A position with sub-1 bps USD shortfall so that checkpointed per-lane token deficits are zero.
- (b). RFS lanes are closed (or not inflated) because commitment deficits were rounded to zero.
- (c). A live VRL-backed signal for non-seizing MM operation.
- (d). The MM calls a remove-liquidity operation.

### Scenario 3.
Aggregated hidden shortfalls across many large positions: Multiple large positions each maintain sub-1 bps shortfalls. Checkpoints write zero per-lane deficits and do not start commitmentDeficitSince timers. Absolute token-threshold and age-gated deficit-bypass seizure never trigger, and pool-wide totals exclude commitmentDeficit, creating a risk-management blind spot until a small drift pushes some positions above 1 bps.
#### Preconditions / Assumptions
- (a). Many large positions across one or more pools each maintain a sub-1 bps USD shortfall.
- (b). Periodic checkpoints are executed so per-lane deficits remain zero and commitmentDeficitSince timestamps do not start.
- (c). Per-token absolute thresholds and age-gates are configured but cannot trigger due to zeroed deficits.
