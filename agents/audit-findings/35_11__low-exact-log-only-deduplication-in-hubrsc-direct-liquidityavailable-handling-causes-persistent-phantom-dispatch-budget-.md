[Low] Exact-log-only deduplication in HubRSC direct LiquidityAvailable handling causes persistent phantom dispatch budget and redispatch churn

# Description

HubRSC [credits a persisted dispatch budget from LiquidityAvailable events](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L427-L434) using only [exact-log deduplication](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L826-L834). If a semantically identical LiquidityAvailable is delivered twice with different identities (e.g., due to reorg re-execution), the budget is double-credited without a corresponding increase in real reserves, causing persistent phantom budget and [repeated failed redispatch attempts](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L372-L379).

HubRSC [directly subscribes to LiquidityHub’s LiquidityAvailable and, upon receipt](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L427-L434), only performs [exact-log deduplication using (chain_id, contract, tx_hash, log_index)](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L826-L834) before [crediting a persisted dispatch budget](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L636-L640) for the underlying lane. Unlike the callback path (which uses nonce-based dedup), LiquidityAvailable has no semantic identifier. If the same semantic reserve arrival is delivered twice with different identities (e.g., because of a reorg re-execution), HubRSC credits the dispatch budget twice while LiquidityHub’s real reserve increased only once. Dispatch then builds batches up to the inflated budget and emits settlement callbacks. LiquidityHub [clamps actual settlement to available reserves and holder balances](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L528-L541), so the excess items revert as [SettlementFailed](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/periphery/BatchProcessSettlement.sol#L41-L53). HubRSC [restores the reserved budget on failure and immediately retries](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L372-L379), creating a persistent redispatch loop. Zero-batch retry limits do not apply because batches are built and only fail downstream. Budgets are not reconciled to live reserve, so the phantom credit persists and can also be spent across LCCs sharing the same underlying lane. This results in operational churn and liveness degradation (repeated failed attempts) but no funds loss, as LiquidityHub enforces reserve-backed settlement.

# Severity

**Impact Explanation:** [Low] No loss of principal funds due to reserve and balance clamps in LiquidityHub; the main effect is operational churn and repeated failed settlement attempts without materially reducing availability for funded entries.

**Likelihood Explanation:** [Low] Depends on infrastructure delivering duplicated origin-chain LiquidityAvailable logs as distinct events (outside attacker control and plausibly canonicalized in many environments); pending queues are common but duplication is not guaranteed.
...
### Scenario 1.
Duplicated LiquidityAvailable (same semantic event, different identities) credits the underlying dispatch budget twice; batches settle up to real reserves; remaining items fail; HubRSC restores budget and immediately redispatches indefinitely until real new liquidity arrives.
#### Preconditions / Assumptions
- (a). Reactive feed for direct LiquidityAvailable delivers a semantically identical event twice with different log identities (e.g., due to reorg re-execution).
- (b). LiquidityHub.confirmTake increased market-derived reserve once (real reserve X).
- (c). HubRSC has pending external settlement entries for the affected underlying.

### Scenario 2.
A duplicated LiquidityAvailable for LCC_A double-credits the shared underlying budget; with shared-underlying routing enabled, dispatch attempts also include LCC_B recipients on the same underlying, causing repeated failed attempts across multiple LCCs.
#### Preconditions / Assumptions
- (a). Two or more LCCs share the same underlying.
- (b). Reactive feed duplication as above for one LCC’s LiquidityAvailable credits the shared underlying budget twice.
- (c). Shared-underlying routing in HubRSC is active and queues exist for multiple LCCs on that underlying.

### Scenario 3.
With phantom-inflated budget, an initial batch partially settles until real reserves are exhausted; subsequent redispatches repeatedly fail for remaining recipients (even if serviceable by holder balance) until new real liquidity is available.
#### Preconditions / Assumptions
- (a). Reactive feed duplication as above inflates budget beyond real reserve.
- (b). Queues contain a mix of serviceable and non-serviceable recipients.
- (c). After partial success spends available reserve, remaining dispatch attempts occur while reserve is zero.
...
-        _dispatchLiquidityIfBudgetAvailable(lcc, true);
+        _releaseInFlightReservation(lcc, recipient, failedAmount, false);
     }
