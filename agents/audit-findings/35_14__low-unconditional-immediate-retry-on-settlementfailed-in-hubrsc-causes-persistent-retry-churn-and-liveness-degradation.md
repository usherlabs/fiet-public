[Low] Unconditional immediate retry on SettlementFailed in HubRSC causes persistent retry churn and liveness degradation

# Description

HubRSC immediately and unconditionally retries any failed destination settlement by restoring dispatch budget and re-dispatching without classifying failure types. Since [SpokeRSC discards revert reasons](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/SpokeRSC.sol#L216-L218) and [LiquidityHub.processSettlementFor can legitimately revert with zero-settleability](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L526-L537), HubRSC can livelock on the same failing entry, repeatedly emitting callbacks and wasting gas/batch capacity until a state change occurs.

When the destination receiver [emits SettlementFailed](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/periphery/BatchProcessSettlement.sol#L50), [SpokeRSC forwards only the (lcc, recipient, maxAmount) to HubCallback](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/SpokeRSC.sol#L216-L218), which emits SettlementFailedReported without the revert reason. HubRSC._handleSettlementFailed then [calls _releaseInFlightReservation with restoreBudget = true](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L376-L376) and [immediately invokes _dispatchLiquidityIfBudgetAvailable](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L377-L377), causing the same entry to be re-dispatched. [LiquidityHubLib.processSettlementLogic computes toSettle = min(queued, availableReserves, maxAmount, holderBal) and reverts with LiquidityError when toSettle == 0 for external recipients](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L526-L537) (e.g., zero available reserves or zero market-derived balance). Because [HubRSC budgets are credited by LiquidityAvailable](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L433-L438) and restored on each failure, and there is no backoff or failure classification, the same failing key remains dispatchable and is retried indefinitely. [Zero-batch retry credits do not apply because batches with failing items have batchCount > 0](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L539-L547).

# Severity

**Impact Explanation:** [Low] The issue causes gas/batch churn and liveness degradation on the reactive/destination pipeline but does not directly cause fund loss, break core invariants, or materially DoS functional settlement paths beyond what zero liquidity already implies. When liquidity returns, normal processing can resume.

**Likelihood Explanation:** [Medium] Requires plausible operational ordering (budget credited then reserves drained) or realistic lane starvation; no attacker control or profit motive is needed, but it depends on states outside the protocol’s direct control and thus is not unconditional.
...
### Scenario 1.
Third-party settlement drains all market-derived reserves after HubRSC receives LiquidityAvailable and credits budget; HubRSC repeatedly dispatches the same pending entry, destination reverts (available == 0 → toSettle == 0), SpokeRSC signals failure without reason, HubRSC restores budget and re-dispatches indefinitely until new reserves arrive.
#### Preconditions / Assumptions
- (a). At least one queued settlement for (lcc, recipient) is mirrored in HubRSC pending
- (b). HubRSC has nonzero dispatch budget due to a prior LiquidityAvailable
- (c). After budget credit, third parties successfully process settlements on-chain, consuming all market-derived reserves (available == 0)
- (d). Reactive callbacks continue to be executed

### Scenario 2.
A trusted issuer creates a decoupled queue for a recipient with insufficient market-derived balance; HubRSC dispatches, destination reverts (holderBal == 0 → toSettle == 0), SpokeRSC signals failure without reason, HubRSC restores budget and re-dispatches indefinitely until the recipient gains sufficient balance.
#### Preconditions / Assumptions
- (a). A trusted issuer uses cancelWithQueue to create a queued settlement for a recipient lacking sufficient market-derived balance
- (b). HubRSC has nonzero dispatch budget
- (c). Reactive callbacks continue to be executed

### Scenario 3.
Multiple pending entries share the same underlying lane with zero market-derived reserves while HubRSC still has positive budget; each dispatched item fails and triggers immediate re-dispatch, causing high-volume retry churn across the lane without progress until reserves return.
#### Preconditions / Assumptions
- (a). Multiple pending entries exist for LCCs sharing the same underlying
- (b). Market-derived reserve for that underlying is zero while HubRSC retains nonzero dispatch budget
- (c). Reactive callbacks continue to be executed
...
-        _dispatchLiquidityIfBudgetAvailable(lcc, true);
+        // Do not immediately re-dispatch on failure; wait for a new state-change signal to avoid retry loops.
     }
