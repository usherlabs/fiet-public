[Medium] Incorrect decrement of historical backfill counter in HubRSC::_enqueueUnderlyingKey causes starvation of pre-registration recipients during shared-underlying dispatch

# Description

HubRSC [decrements the historical-backfill counter](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L756-L764) for newly created post-registration keys, [prematurely ending backfill](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L926-L929) and hiding true historical recipients from [shared-underlying scans](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L460-L461), which can significantly delay their settlements.

HubRSC maintains [underlyingBackfillRemainingByLcc[lcc]](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L116) to count only pre-registration (historical) per-LCC queue entries that still need mirroring into the shared-underlying lane. It is [incremented only when a pending entry is created before underlying registration](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L304). After registration, [_enqueueUnderlyingKey mirrors any unmirrored key to the shared-underlying queue and decrements this counter if still positive](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L756-L764)—even for post-registration keys that never contributed to the historical backlog. This can drive the counter to zero while true historical keys remain unmirrored. [When the counter is zero, the LCC is removed from the pending backfill set](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L926-L929), halting further bounded backfill for that LCC. Once pending backfill is empty and the shared-underlying queue is non-empty, [routing prefers the shared-underlying lane](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L648-L656); unmirrored historical recipients remain only in per-LCC queues and are [not visible to shared-underlying scans](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L460-L461). As a result, those recipients can be starved until the shared-underlying queue is empty at the start of a dispatch or they receive a fresh increase (which would mirror them). No principal loss or invariant break occurs, but settlement liveness/fairness is degraded.

# Severity

**Impact Explanation:** [Medium] Significant liveness/fairness degradation for settlement dispatch: historical recipients can experience prolonged delays, but there is no principal loss or invariant break, and progress can resume when the shared-underlying queue is empty at dispatch start or when recipients get new increases.

**Likelihood Explanation:** [Medium] Preconditions such as pre-registration backlog exceeding the initial backfill cap, ongoing activity keeping shared-underlying queues non-empty, and dormant recipients without new increases are plausible in active deployments without requiring attacker behavior.
...
### Scenario 1.
Organic high-volume usage on one LCC: Before HubRSC registers the underlying for LCC Y, a historical backlog larger than the initial backfill cap exists. Registration mirrors up to the cap, leaving some historical recipients unmirrored. Normal post-registration usage creates many new recipients, each mirrored immediately and incorrectly decrementing the historical counter. When the counter reaches zero, backfill for Y stops. With shared-underlying queues staying non-empty due to ongoing activity, dispatch prefers shared-underlying scans and never sees the unmirrored historical recipients, delaying their settlements until the shared-underlying queue empties at a dispatch start or they get a new increase.
#### Preconditions / Assumptions
- (a). For the LCC, there exists a pre-registration historical backlog larger than the initial backfill cap (so not all historical keys are mirrored at registration).
- (b). After registration, normal usage creates enough post-registration recipients to exceed the leftover historical count.
- (c). Shared-underlying queue remains non-empty during dispatch windows due to ongoing activity.
- (d). Affected historical recipients do not receive new increases.

### Scenario 2.
Systemic starvation across multiple LCCs: Multiple LCCs on the same underlying each have large pre-registration backlogs. Initial bounded backfill for each mirrors only up to the cap. Post-registration growth across these LCCs mirrors many new keys and erroneously decrements each LCC’s historical counter to zero, removing all from pending backfill. With sustained activity keeping the shared-underlying queue non-empty, dispatch always uses the shared-underlying lane, and dormant historical recipients across these LCCs remain invisible and delayed until a lull empties the shared-underlying queue at a dispatch start or they get new increases.
#### Preconditions / Assumptions
- (a). Multiple LCCs on the same underlying each have pre-registration backlogs larger than the initial backfill cap.
- (b). Post-registration growth across these LCCs mirrors many new keys, collectively driving each LCC’s historical counter to zero.
- (c). Shared-underlying queue remains non-empty during dispatch windows due to sustained activity across these LCCs.
- (d). Dormant historical recipients do not receive new increases.

### Scenario 3.
Adversarial churn on one LCC: An attacker creates many legitimate post-registration SettlementQueued entries for LCC X to new recipients. Each new key is mirrored immediately and incorrectly decrements the historical counter. Once the counter hits zero, X is removed from pending backfill. By keeping the shared-underlying queue non-empty (via normal activity or additional mirrored entries), subsequent dispatches for X use the shared-underlying lane, hiding unmirrored historical recipients and delaying their settlements until the underlying queue empties at a dispatch start or they get new increases.
#### Preconditions / Assumptions
- (a). The target LCC has a pre-registration historical backlog larger than the initial backfill cap (leaving some unmirrored historical recipients after registration).
- (b). The attacker can legitimately create many post-registration SettlementQueued entries for new recipients on this LCC.
- (c). Shared-underlying queue can be kept non-empty during dispatch windows (via normal activity or attacker-driven entries).
- (d). Dormant historical recipients do not receive new increases.
...
+            // FIX-ME: Decrementing here reduces the historical-backlog counter for any first-time mirrored key,
+            // including post-registration keys. Gate this decrement on a per-key 'historical' flag.
             if (remaining > 0) {
     ...
     function _pruneIfFullySettled(Pending storage entry, bytes32 key) internal {
         ...
         if (mirroredToUnderlyingByKey[key] && hasUnderlyingForLcc[lcc]) {
             ...
             uint256 remaining = underlyingBackfillRemainingByLcc[lcc];
+            // FIX-ME: Only decrement when the key was part of the pre-registration backlog (historical).
             if (remaining > 0) {
                 underlyingBackfillRemainingByLcc[lcc] = remaining - 1;
                 _syncUnderlyingBackfillState(lcc);
             }
...
## [Medium] Missing dispatch after LCCCreated registration in HubRSC causes settlement dispatch stall

### Description

HubRSC [registers and backfills LCCs on LCCCreated](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L417-L423) but does not attempt dispatch, so previously queued settlements can remain stalled despite available shared-underlying budget until a later unrelated event occurs.

In HubRSC, SettlementQueuedReported for an unregistered LCC [enqueues work per-LCC](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L300-L305) and [immediately tries to dispatch](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L325), but [budget lookup is per-LCC until the LCC is registered](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L628-L634), making any existing shared-underlying budget invisible. When LCCCreated later arrives, [HubRSC._handleLccCreated calls _registerLccUnderlying](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L417-L423) [to set hasUnderlyingForLcc and to backfill historical per-LCC keys into the shared underlying queue via _initializeUnderlyingBackfill](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L730-L744), but it does not call _dispatchLiquidityIfBudgetAvailable. As a result, even if availableBudgetByDispatchLane[underlying] > 0 and the keys are now visible in the shared queue, no dispatch is attempted. The queued settlements remain stalled until a subsequent LiquidityAvailable/MoreLiquidityAvailable or other lane-activating callback arrives. This is a liveness DoS (no loss of funds), caused by asymmetric handling between [LiquidityAvailable (register + credit + dispatch)](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L429-L434) and LCCCreated (register + backfill only).

### Severity

**Impact Explanation:** [Medium] Causes significant availability loss of the settlement pipeline for affected users (stalled settlements) without principal loss; it is a liveness DoS of an important protocol function.

**Likelihood Explanation:** [Medium] Requires plausible cross-chain out-of-order delivery or replay order and the presence of shared-underlying budget without an immediate follow-up event; these are uncommon but realistic conditions in reactive, event-driven systems.
...
### Scenario 1.
Out-of-order delivery: A sibling LCC on the same underlying U has previously credited shared-underlying budget. SettlementQueuedReported(recipientX, LCC_A) is delivered before LCCCreated(LCC_A). The per-LCC queue is created and initial dispatch sees zero per-LCC budget. Later LCCCreated(LCC_A) registers and backfills keys into the shared-underlying queue, but no dispatch occurs. Pending work for LCC_A remains stalled until a future LiquidityAvailable/MoreLiquidityAvailable or similar event.
#### Preconditions / Assumptions
- (a). Two LCCs share the same underlying U.
- (b). Shared-underlying budget on U already exists from a sibling LCC LiquidityAvailable event.
- (c). SettlementQueuedReported for LCC_A is delivered before LCCCreated(LCC_A).
- (d). No immediate LiquidityAvailable/MoreLiquidityAvailable or other dispatch-triggering event follows LCCCreated.

### Scenario 2.
Post-factum deployment/replay: HubRSC is deployed after some LiquidityAvailable for a sibling LCC credited budget on U. SettlementQueuedReported for an unregistered LCC_C is processed first, creating a per-LCC queue and attempting a dispatch that sees zero per-LCC budget. LCCCreated(LCC_C) then arrives, registration/backfill happens, but dispatch is not attempted, stalling recipients under LCC_C until a later wake-up event.
#### Preconditions / Assumptions
- (a). HubRSC is deployed after prior LiquidityAvailable events credited shared-underlying budget on U.
- (b). SettlementQueuedReported for an unregistered LCC_C is processed before LCCCreated(LCC_C) on HubRSC.
- (c). No immediate LiquidityAvailable/MoreLiquidityAvailable or other dispatch-triggering event follows LCCCreated.

### Scenario 3.
Large pre-registration backlog: Many entries are queued for LCC_D before LCCCreated is seen by HubRSC and shared-underlying budget on U already exists. After LCCCreated(LCC_D), _initializeUnderlyingBackfill mirrors up to maxDispatchItems keys into the shared-underlying queue but does not dispatch. The mirrored subset and remaining backlog both remain stalled until a follow-up liquidity-related event triggers dispatch.
#### Preconditions / Assumptions
- (a). Many SettlementQueuedReported entries exist for LCC_D before LCCCreated(LCC_D) is processed on HubRSC.
- (b). Shared-underlying budget on U already exists from sibling LCC events.
- (c). maxDispatchItems is smaller than the pre-registration backlog size.
- (d). No immediate LiquidityAvailable/MoreLiquidityAvailable or other dispatch-triggering event follows LCCCreated.
