[Medium] Live-signal gate in VTSOrchestrator.extendGracePeriod causes proof-backed positions to become seizable and slashed

# Description

[extendGracePeriod requires a live VRL signal (unexpired and non-empty reserves)](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/VTSOrchestrator.sol#L649-L666), blocking grace extension even with a valid settlement proof. [Seizure does not require a live signal](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/VTSOrchestrator.sol#L779-L787), enabling slashing despite bona fide proof in expiry/empty-reserve cases.

[VTSOrchestrator.extendGracePeriod calls _assertSignalValid(commitId, true)](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/VTSOrchestrator.sol#L649-L666), which via [VTSLifecycleLinkedLib.isSignalValid](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L120-L141) enforces that the commit is initialised, unexpired, and has non-empty reserves. This prevents grace extension from proceeding when the signal has expired or [was renewed to empty reserves](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L425-L438). However, [CheckpointLibrary.extendGracePeriod](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/Checkpoint.sol#L156-L175) and [VRLSettlementObserver.verifySettlementProof](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/VRLSettlementObserver.sol#L113-L141) only require an open RFS lane and a valid, allowlisted proof bound to (poolId, tokenIndex, positionId). Meanwhile, [VTSOrchestrator.onSeize uses _assertSignalValid(commitId, false)](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/VTSOrchestrator.sol#L779-L787), allowing seizure even when the signal is expired/empty. As a result, positions with valid, verifier-authorized pending-settlement proofs can be prevented from extending grace and subsequently become seizable, leading to slashing of liquidity units.

# Severity

**Impact Explanation:** [High] Seizure slashes liquidity units from the victim position, constituting direct, material loss of principal.

**Likelihood Explanation:** [Low] All scenarios depend on low-likelihood conditions: user/operator key compromise (advancer), user/operator timing error near expiry, or external VRL/prover availability issues (integration outage).
...
### Scenario 1.
Advancer sabotage: An attacker controlling the advancer renews the commit to empty reserves. The owner/approved operator attempts extendGracePeriod with a valid, allowlisted proof but it reverts due to the live-signal gate. After grace elapses, a third party seizes the position (seizure allows expired/empty), slashing liquidity units despite the bona fide proof.
#### Preconditions / Assumptions
- (a). Active position with RFS open on at least one lane (checkpoint.openMask includes the lane)
- (b). Valid settlement proof bound to (poolId, tokenIndex, positionId) by an allowlisted verifier
- (c). TokenConfiguration permits grace extension (maxGracePeriodTime > gracePeriodTime)
- (d). Attacker controls mmState.advancer and can renew the commit to empty reserves
- (e). Factory-bound caller and pool match satisfied for orchestrator and MMPM flows

### Scenario 2.
Expiry race: The owner/approved operator submits extendGracePeriod with a valid proof near commit expiry, but the transaction lands just after expiration. The live-signal check fails, blocking extension. If renewal to non-empty cannot be performed in time, once grace elapses, seizure proceeds and slashes liquidity.
#### Preconditions / Assumptions
- (a). Active position with RFS open on at least one lane
- (b). Valid settlement proof bound to (poolId, tokenIndex, positionId)
- (c). Commit expiry time is near; transaction lands post-expiry
- (d). Owner/approved initiates extendGracePeriod; renewal to non-empty not completed before grace elapses
- (e). Factory-bound caller and pool match satisfied

### Scenario 3.
VRL/prover outage: A temporary outage prevents renewing to a non-empty, live VRL signal while a valid proof exists. The live-signal requirement blocks extendGracePeriod. After grace elapses, seizure is allowed and slashes liquidity units.
#### Preconditions / Assumptions
- (a). Active position with RFS open on at least one lane
- (b). Valid settlement proof bound to (poolId, tokenIndex, positionId)
- (c). Commit is expired or renewed to empty reserves due to temporary VRL/prover/ops unavailability
- (d). Owner/approved initiates extendGracePeriod; cannot re-establish a live (non-empty) signal before grace elapses
- (e). Factory-bound caller and pool match satisfied
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/VTSOrchestrator.sol)
