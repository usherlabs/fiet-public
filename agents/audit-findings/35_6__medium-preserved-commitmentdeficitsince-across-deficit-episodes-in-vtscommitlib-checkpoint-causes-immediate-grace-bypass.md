[Medium] Preserved commitmentDeficitSince across deficit episodes in VTSCommitLib/Checkpoint causes immediate grace-bypass and early third-party seizure

# Description

The age gate for commitment-deficit grace bypass is preserved across episodes because [commitmentDeficitSince](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L90-L98) is only reset when the stored commitmentDeficit is written to zero. If a first short-lived deficit is checkpointed and not cleared back to zero in storage, its age can be 'banked' and later reused to bypass grace immediately on a second short-lived deficit, enabling third-party seizure earlier than intended.

In [VTSCommitLib._writeCommitmentDeficitToken](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L90-L98), commitmentDeficitSince is set when a lane's stored commitmentDeficit transitions from 0 to nonzero, and cleared only when it transitions from nonzero to 0. [VTSCommitLib._checkpointWithCommitment](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L540-L549) updates per-lane commitmentDeficit based on live spot, but preserves the since timestamp if the previous stored value was nonzero and the next remains nonzero. Before seizure, [VTSCommitLib.validateSeize](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L668-L682) first [settles growth](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSPositionLib.sol#L729-L735) and then re-runs a commitment-backed checkpoint. If growth settlement does not fully net commitmentDeficit to zero, the subsequent checkpoint preserves the old since, and [CheckpointLibrary.isSeizable](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/Checkpoint.sol#L37-L62) treats the age as satisfied once unbackedCommitmentGraceBypassTime has elapsed, provided severity gates (bps or per-lane threshold) are met. [MMPositionActionsImpl._seizePosition](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/MMPositionActionsImpl.sol#L331-L336) allows third-party seizure when isSeizable is true. Consequently, a first short-lived manipulated checkpoint can start the age timer; if not cleared to zero in storage during recovery, a later short-lived deficit can immediately bypass grace and be seized by a third party.

# Severity

**Impact Explanation:** [High] Immediate third-party seizure bypasses the intended grace delay and can force removal of liquidity units and settlement at unfavorable times, constituting a direct, material impact on the MM’s principal.

**Likelihood Explanation:** [Low] Exploitation depends on multiple constraints: two timed spot manipulations; the stored deficit not being cleared to zero (operator/keeper diligence could mitigate); and validateSeize’s growth settlement not fully netting the stored deficit to zero. These factors collectively reduce likelihood.
...
### Scenario 1.
Age-banking then immediate third-party seizure: An attacker briefly manipulates spot to create a deficit and runs a [commitment checkpoint](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L540-L549), starting commitmentDeficitSince. After price recovery, no one clears the stored deficit to zero. Once unbackedCommitmentGraceBypassTime elapses, the attacker briefly manipulates spot again and calls SEIZE_POSITION. [validateSeize](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L668-L682) [settles growth](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSPositionLib.sol#L729-L735) and re-checkpoints; since remains from the first episode because stored deficits are still nonzero, so age and severity gates pass and third-party seizure proceeds immediately.
#### Preconditions / Assumptions
- (a). Active MM position with positive liquidity
- (b). MarketVTSConfiguration sets unbackedCommitmentGraceBypassTime > 0 for at least one lane
- (c). Bypass severity can be met during the later manipulation (bps or per-lane threshold)
- (d). No zero-writing checkpoint or full-netting settlement/inflow occurs that would reset stored commitmentDeficit to zero before the second episode
- (e). PoolManager is unlocked when actions occur; oracle integrity holds; Uniswap behavior canonical

### Scenario 2.
Per-lane threshold bypass: Similar to the first scenario, but the attacker ensures that at the second manipulation the per-lane raw deficit meets tokenX.unbackedCommitmentGraceBypassThreshold even if deficit bps is below the bps bypass. The preserved since satisfies the age gate and the threshold satisfies severity, enabling immediate third-party seizure.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1
- (b). Per-lane unbackedCommitmentGraceBypassThreshold configured to a reachable level under manipulated spot composition
- (c). Attacker can shape eff0/eff1 via spot to exceed the per-lane threshold at the second checkpoint

### Scenario 3.
Minimizing growth-settlement clearing: The attacker times the second manipulation and seizure attempt shortly after any growth settlement (or during low inflow), so [validateSeize](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/VTSCommitLib.sol#L668-L682)’s growth settlement does not fully net stored commitmentDeficit to zero. The subsequent checkpoint preserves the old since, and immediate third-party seizure is allowed once severity is met.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1
- (b). The second manipulation is timed soon after growth settlement (or during low inflows) so validateSeize’s growth settlement does not fully zero the stored commitmentDeficit
...
```diff
 ...
 library VTSCommitLib {
     ...
     function _checkpointWithCommitment(
         ...
         PositionAccounting storage pa = s.positionAccounting[positionId];
+        uint16 prevBps = pa.commitmentDeficitBps;
         {
...
+            if (prevBps == 0) {
+                if (pa.commitmentDeficit.token0 > 0) pa.commitmentDeficitSince.set(0, block.timestamp);
+                if (pa.commitmentDeficit.token1 > 0) pa.commitmentDeficitSince.set(1, block.timestamp);
+            }
         }
     }
