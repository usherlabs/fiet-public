[Medium] Unbounded native ETH push and no per-item gas cap in settlement batching causes cross-recipient DoS

# Description

For native-backed LCCs, [LiquidityHubLib.transferUnderlying](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L578) forwards nearly all gas to recipients that claim [INativeSettlementReceiver via ERC165](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L604-L605); the destination batch processor [calls processSettlementFor](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/periphery/BatchProcessSettlement.sol#L47) without a per-item gas cap. A malicious or buggy recipient can burn gas in receive() and return success, draining the batch’s gas and preventing subsequent items from executing. Funds are safe; impact is per-transaction liveness/throughput degradation. Reactive failure callbacks are expected to [release in-flight reservations and restore budgets](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L666-L670).

In [LiquidityHubLib.transferUnderlying](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L578), when settling native-backed LCCs (underlying == address(0)), the hub attempts a raw ETH push using [account.call{value: amount}("")](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L608) for EOAs or contracts that [support INativeSettlementReceiver via ERC165](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol#L604-L605). This call has no explicit gas cap and forwards nearly all remaining gas. If the recipient returns success after high gas consumption in a payable receive(), the outer context retains only minimal gas (per EIP-150), leaving little for subsequent operations. The destination batch processor ([AbstractBatchProcessSettlement.processSettlements](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/periphery/BatchProcessSettlement.sol#L35)) iterates and [calls LiquidityHub.processSettlementFor](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/periphery/BatchProcessSettlement.sol#L47) for each item without constraining per-item gas; thus, one gas-heavy recipient can cause later items in the same batch to fail to execute or the entire batch to revert. This creates a cross-recipient denial of service within a batch window. It does not risk principal funds. The reactive architecture is designed to report per-item failures via HubCallback, allowing HubRSC to [release in-flight reservations and restore budgets](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/reactive/src/HubRSC.sol#L666-L670) even if the destination transaction reverts, limiting the issue to per-transaction liveness degradation.

# Severity

**Impact Explanation:** [Medium] The issue causes significant but temporary availability loss of settlement processing across a batch (cross-recipient DoS) without loss of principal funds. Reactive failure callbacks are expected to free reservations and restore budgets.

**Likelihood Explanation:** [Medium] Exploitation requires a native-backed LCC, attacker inclusion and ordering within a batch, and available budget; these are realistic but not fully under attacker control. Implementing INativeSettlementReceiver and queueing settlement are straightforward.
...
### Scenario 1.
An attacker deploys a contract that supports INativeSettlementReceiver and implements a gas-burning payable receive() that returns success. They queue a native-backed LCC settlement to their contract. When included early in a destination batch, LiquidityHubLib.transferUnderlying performs a raw ETH push to the attacker, whose receive() drains most gas. Subsequent items in the batch fail to execute or the transaction reverts, delaying other recipients’ settlements until later retries.
#### Preconditions / Assumptions
- (a). A native-backed LCC exists (underlying == address(0))
- (b). Attacker contract implements ERC165 support for INativeSettlementReceiver and has a gas-burning payable receive() that returns success
- (c). Attacker queues settlement to their own contract (e.g., by creating market-derived shortfall via unwrap when liquidity is scarce)
- (d). Reactive dispatch constructs a batch that includes the attacker among multiple recipients with sufficient available budget and typical scan ordering
- (e). Destination batch executes with a high gas limit (e.g., CALLBACK_GAS_LIMIT) such that multiple items would normally be processed

### Scenario 2.
A legitimate integrator’s recipient contract for a native-backed LCC opts into INativeSettlementReceiver but has a buggy, gas-heavy payable receive(). When included in a batch, the raw ETH push consumes excessive gas and returns success, preventing later batch items from executing and delaying unrelated recipients.
#### Preconditions / Assumptions
- (a). A native-backed LCC exists
- (b). A legitimate recipient contract opts into INativeSettlementReceiver but has a buggy or unexpectedly expensive payable receive()
- (c). The recipient has a queued settlement and is included in a destination batch

### Scenario 3.
Outside batching, a caller triggers LiquidityHub.processSettlementFor directly against a malicious INativeSettlementReceiver recipient with a gas-burning receive(). The call wastes gas and may fail locally but does not affect other recipients; impact is limited to that single attempt.
#### Preconditions / Assumptions
- (a). LiquidityHub.processSettlementFor is callable permissionlessly
- (b). A malicious recipient contract supports INativeSettlementReceiver and has a gas-burning payable receive()
- (c). The caller targets a single settlement for that recipient outside of batch processing
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/fae0d20fe33523515109a523f8c048cd6765ecdb/contracts/evm/src/libraries/LiquidityHubLib.sol)
...
+    uint256 internal constant NATIVE_PUSH_GAS_STIPEND = 30000;
