[Low] Unbounded MarketMaker.State strings and deep-copy in commitment valuation cause gas-cost amplification on checkpoint/seizure paths

# Description

Commit admission does not bound dynamic string sizes in MarketMaker.State; the entire state is stored and later deep-copied from storage to memory during commitment-backed valuation, making checkpoint(withCommitment) and certain seizure pre-checks significantly more expensive until commit expiry.

Liquidity signals include MarketMaker.State with dynamic strings (sourceState, prover, nonce, and per-reserve asset). Admission only checks that the advancer is nonzero, reserves are priceable, and the reserve count ≤ 100; it does not bound string lengths. [MarketMaker.save stores the entire mmState](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/libraries/MarketMaker.sol#L60-L76), including large strings. Later, [VTSCommitLib._signalValueForCommit loads Commit.mmState into a memory struct (deep-copying all dynamic fields)](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/libraries/VTSCommitLib.sol#L562-L568) before computing reserve value. This read path is invoked by [checkpointAfterGrowthWithCommitment (used by checkpoint(..., true))](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/VTSOrchestrator.sol#L857-L862) and by [validateSeize when a stored commitment deficit exists (it recomputes a commitment-backed checkpoint)](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/libraries/VTSCommitLib.sol#L694-L699). As a result, a one-time oversized but valid mmState can amplify the recurring gas cost of these commitment-backed flows until the commit expires. Ticker registration is owner-only, bounding reserve.asset abuse; the main unbounded fields are sourceState/prover/nonce. The attacker bears the one-time SSTORE cost to write the large state; the effect window is bounded by commit expiry.

# Severity

**Impact Explanation:** [Low] No principal loss or invariant break; core protocol functionality remains. The issue increases gas costs on optional commitment-backed safety flows (checkpoint with commitment and certain seizure pre-checks) within a bounded window.

**Likelihood Explanation:** [Medium] The attacker must pay a one-time high gas cost to write a large state and the effect persists only until expiry, but it is plausible that watchers or enforcers call these paths during that window.
...
### Scenario 1.
A malicious market maker submits or renews a commit with valid reserves (≤100 registered tickers) but very large non-priced strings in mmState (sourceState/prover/nonce). A third party calls checkpoint(commitId, positionIndex, true); the flow deep-copies the large mmState and computes reserve value, causing materially higher gas usage for the caller until the commit expires.
#### Preconditions / Assumptions
- (a). The VRL pipeline admits and signs a liquidity signal whose mmState contains very large non-priced strings (sourceState, prover, nonce).
- (b). Reserves count ≤ 100 and all tickers are registered so OracleHelper.getTotalValue succeeds.
- (c). The attacker pays the one-time gas cost to store the large mmState via commitSignal or renewSignal.
- (d). The commit is not expired when checkpoint(withCommitment=true) is called by a third party.

### Scenario 2.
A malicious market maker has an active oversized commit and a position with a stored commitment deficit. An enforcer calls onSeize(commitId, positionIndex); validateSeize refreshes growth and runs a commitment-backed checkpoint, which deep-copies the large mmState before computing signal value, increasing the enforcer’s gas cost prior to the seizability check.
#### Preconditions / Assumptions
- (a). An oversized mmState commit has been admitted and is still not expired.
- (b). A position under the commit has a stored non-zero commitment deficit from a prior checkpoint.
- (c). An enforcer calls onSeize(commitId, positionIndex), triggering a commitment-backed checkpoint inside validateSeize.
...
+        Commit storage c = s.commits[commitId]; uint256 n = c.mmState.reserves.length;
+        if (n > MAX_MM_UNIQUE_RESERVE_TICKERS) revert Errors.MMReserveTickerLimitExceeded(n, MAX_MM_UNIQUE_RESERVE_TICKERS);
+        string[] memory tickers = new string[](n); uint256[] memory amounts = new uint256[](n);
+        for (uint256 i; i < n; i++) { tickers[i] = c.mmState.reserves[i].asset; amounts[i] = c.mmState.reserves[i].amount; }
+        return oracleHelper.getTotalValue(tickers, amounts);
     }
