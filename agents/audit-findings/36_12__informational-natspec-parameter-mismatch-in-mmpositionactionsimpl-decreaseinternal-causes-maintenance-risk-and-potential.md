[Informational] NatSpec parameter mismatch in MMPositionActionsImpl._decreaseInternal causes maintenance risk and potential future parameter misordering

# Description

[The NatSpec for _decreaseInternal](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L844-L848) omits tokenId, amount0Min, and amount1Min and [does not match the function’s parameter order](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L850-L858), risking future misuses despite current correct usage.

The internal function _decreaseInternal in MMPositionActionsImpl has [incomplete and misordered NatSpec documentation](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L844-L848): it omits tokenId, amount0Min, and amount1Min, and the [documented order does not match the implemented signature](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L850-L858). While all current call sites ([_decrease](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L903), [_burnPosition](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L569), [_seizePosition](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L362)) pass the correct arguments and the function is internal-only, the mismatch increases the risk of future developer mistakes because some parameters share the same types (uint256 for tokenId and amountToDecrease; uint128 for amount0Min and amount1Min). This could lead to subtle correctness issues in custody semantics ([tokenId > 0 commit-bucket behavior](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/modules/PositionManagerImpl.sol#L202-L212)) or misapplied [slippage floors](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L878) if parameters are ever misordered by maintainers. There is no present attacker exploit or principal loss; the issue is purely documentation/maintenance risk.

# Severity

**Impact Explanation:** [Informational] Documentation-only discrepancy on an internal function with no current state or funds impact; all current call sites are correct and external paths are correctly decoded.

**Likelihood Explanation:** [Low] Real harm would require a future trusted developer mistake (misordering same-typed parameters); the current implementation does not exhibit misuse.

# Exploitation

No exploitation scenarios available.
...
+    /// @param tokenId The commitment NFT token ID (must be > 0 for commit-bucket custody)
     /// @param amountToDecrease The amount of liquidity to remove
     /// @param hookData The hook data for the modification
+    /// @param amount0Min Minimum per-leg immediate non-fee LCC token0 after fee netting (min-out basis)
+    /// @param amount1Min Minimum per-leg immediate non-fee LCC token1 after fee netting (min-out basis)
...
+        if (tokenId == 0) revert Errors.InvalidPosition(tokenId, 0, PositionId.wrap(bytes32(0)));
