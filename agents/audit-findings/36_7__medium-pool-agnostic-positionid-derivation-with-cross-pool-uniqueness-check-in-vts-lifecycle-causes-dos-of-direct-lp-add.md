[Medium] Pool-agnostic PositionId derivation with cross-pool uniqueness check in VTS lifecycle causes DoS of direct-LP adds via shared routers

# Description

[Positions are keyed only by (owner, tickLower, tickUpper, salt)](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/types/Position.sol#L236-L239) without poolId, but [VTS enforces that a PositionId maps to a single pool](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L811-L818). An attacker using a shared router can pre-register the same PositionId on another pool, causing victims’ direct-LP adds on the intended pool to revert.

PositionId is [derived from (owner, tickLower, tickUpper, salt)](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/types/Position.sol#L236-L239) and [stored globally in VTS without including poolId](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/VTSOrchestrator.sol#L286-L290). During processing, [if a PositionId already exists for a different pool, VTS reverts](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L66-L76). For non-MM (direct LP) positions, [the owner is the initial caller (often a shared router)](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/CoreHook.sol#L146-L151), so multiple users share the same PositionId owner. An attacker can front-run by adding minimal liquidity through the same shared router to any other core pool using the same (tickLower, tickUpper, salt), registering that PositionId for the attacker’s pool. When the victim’s add-liquidity executes for the intended pool with the same tuple, VTS detects the existing PositionId pointing to a different pool and reverts with InvalidPosition. MM-managed positions are unaffected due to [strict auth](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/MMPositionActionsImpl.sol#L895-L900) and [commit-linked salts](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/types/Position.sol#L249-L255); direct-LP from EOAs is also unaffected because the owner is the EOA.

# Severity

**Impact Explanation:** [Medium] This causes a significant availability loss/DoS of a supported direct-LP add path (reverts on add-liquidity) without directly stealing funds or breaking core invariants.

**Likelihood Explanation:** [Medium] Shared routers and predictable salts are common; attacker costs are low (wrap minimal LCC, mempool watching). Incentives can be rational (e.g., blocking competitor LP). However, not all users/routers are affected (some randomize or namespace salts).
...
### Scenario 1.
Cross-pool PositionId collision DoS: attacker observes a victim’s pending direct-LP add via a shared router for pool P with specific ticks and salt, pre-registers the same PositionId on a different pool Q, and the victim’s add to P reverts due to cross-pool mismatch.
#### Preconditions / Assumptions
- (a). At least two core pools (P and Q) exist under the same VTSOrchestrator
- (b). Victim uses a shared/public router as the direct-LP caller (owner in PositionId)
- (c). Attacker can observe mempool to read ticks and salt from pending tx
- (d). Attacker can obtain minimal LCC and add small direct LP on pool Q
- (e). Tick spacing/range on Q admits the same tickLower/tickUpper as P
- (f). Router allows or defaults to the salt value used by the victim

### Scenario 2.
Persistent grief with deterministic salts: if a shared router or UI uses a predictable/default salt, the attacker repeatedly pre-registers common (ticks, salt) tuples on another pool so future adds on the intended pool consistently revert until users change salts.
#### Preconditions / Assumptions
- (a). Shared/public router derives a deterministic or default salt for direct-LP
- (b). Same orchestrator hosts multiple pools; at least one pool Q is available
- (c). Attacker can acquire minimal LCC and add small direct LP on Q
- (d). Victims commonly use the same (ticks, default salt) tuples

### Scenario 3.
Landmine planting across multiple pools: attacker cheaply pre-registers many (router, ticks, salt) tuples across several pools to cause widespread reverts for victims attempting to add direct LP on their target pools using those tuples.
#### Preconditions / Assumptions
- (a). Many pools exist under the same orchestrator
- (b). Attacker can cheaply add minimal LP across pools via the shared router
- (c). Victims commonly use shared router and predictable (ticks, salt) tuples
- (d). Tick spacing across pools admits reuse of these tuples
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol)
...
+        // POOL-NAMESPACE TODO: PositionId is pool-agnostic; prefer a pool-scoped composite key to prevent cross-pool collisions for direct-LP.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/types/Position.sol)
...
+    // POOL-NAMESPACE TODO: this id is pool-agnostic (owner,ticks,salt). Consider pool-scoping in VTS (composite key) or pool-namespaced salts for direct-LP integrations.
      */
...
+        // POOL-NAMESPACE TODO: this id omits poolId; VTS should use a pool-scoped key to avoid cross-pool collisions for direct-LP via shared routers.
...
+        // POOL-NAMESPACE TODO: consider deriving/using a pool-scoped key here and throughout VTS to prevent cross-pool PositionId collisions.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/4579c7cb0b8410b5bf160da4b6d822fa52b26ccb/contracts/evm/src/CoreHook.sol)
...
+        // POOL-NAMESPACE TODO: use a pool-scoped id (poolId + PositionId) to avoid cross-pool collisions for direct-LP via shared routers.
...
+        // POOL-NAMESPACE TODO: switch to a pool-scoped settlePositionGrowths variant to avoid cross-pool PositionId reuse.
