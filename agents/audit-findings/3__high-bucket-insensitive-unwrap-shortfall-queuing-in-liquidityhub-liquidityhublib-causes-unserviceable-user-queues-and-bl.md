[High] Bucket-insensitive unwrap shortfall queuing in LiquidityHub/LiquidityHubLib causes unserviceable user queues and blocked unwraps

# Description

Unwrap shortfalls for external recipients are queued without regard to the user’s bucket composition, but settlement later can only burn market-derived LCC. Users who are wrapped-heavy can create unserviceable queues that permanently revert on settlement, and headroom rules then block further unwraps; attempting to regain headroom via protocol-bound transfers annuls (forfeits) the queued claim.

During unwrap, LiquidityHubLib.unwrapInternalLogic consumes direct supply and market liquidity, then unconditionally [queues any remaining shortfall to the recipient via queueSettlement](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/LiquidityHubLib.sol#L448-L461). For external recipients, LiquidityHubLib.processSettlementLogic [can only settle queued claims by burning the recipient’s market-derived LCC](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/LiquidityHubLib.sol#L575-L579); if the user lacks sufficient market-derived balance, [toSettle is zero and the call reverts with LiquidityError](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/LiquidityHubLib.sol#L532-L543). Meanwhile, LiquidityHub._unwrap [enforces an admission gate amount <= fromBalance - existingQueue](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/LiquidityHub.sol#L1175-L1181), so once a large queue exists the user cannot unwrap further. Attempting to transfer to protocol endpoints to proceed (e.g., DEX sink) [triggers LiquidityHub.annulSettlementBeforeTransfer before bucket updates](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/LCC.sol#L222-L224), [silently reducing or eliminating the queued claim without payout](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/LiquidityHubLib.sol#L742-L752). Thus, a wrapped-heavy user under liquidity stress can end up with an unserviceable queued claim, permanently reverting on settlement, and further unwraps blocked unless they forfeit the claim or acquire market-derived LCC via nontrivial means.

# Severity

**Impact Explanation:** [High] Per-user claims can be frozen (external settlement permanently reverts due to lack of market-derived) and further unwraps are blocked by headroom; attempting to proceed via protocol-bound transfers annuls the queued claim (material loss of the queued entitlement).

**Likelihood Explanation:** [Medium] Requires stressed liquidity conditions (low direct supply and/or constrained market liquidity) and wrapped-heavy users; these are uncommon but realistic states under market stress and do not rely on admin misuse.
...
### Scenario 1.
Fully wrapped user under stressed liquidity: s.directSupply for the LCC is near zero and market liquidity returns 0. The user unwraps 100, queues 100 to themselves, receives no payout now, and later processSettlementFor reverts (holder’s market-derived = 0). Further unwraps are blocked by headroom (fromBalance - existingQueue = 0).
#### Preconditions / Assumptions
- (a). Direct supply for the LCC is low/zero due to normal operations
- (b). User holds only wrapped LCC (market-derived balance ≈ 0)
- (c). MarketFactory.useMarketLiquidity returns 0 for the user’s unwrap window
- (d). User calls unwrap for an amount near their full balance

### Scenario 2.
Mixed buckets with partial market redemption: User holds wrapped=70, market-derived=30, s.directSupply≈0, and useMarketLiquidity returns 20. Unwrap 100 burns 20 market-derived now and queues 80. A later settlement succeeds only for 10 (remaining market-derived), leaving 70 queued and unserviceable; headroom then blocks further unwraps.
#### Preconditions / Assumptions
- (a). Direct supply for the LCC is low/zero
- (b). User holds a mix: wrapped>0 and market-derived>0
- (c). Market liquidity is constrained (partial useMarketLiquidity result)
- (d). User calls unwrap for an amount exceeding immediate liquidity

### Scenario 3.
User attempts to regain headroom by sending LCC to a protocol endpoint (e.g., DEX sink) and later receiving market-derived back. Before bucket updates, annulSettlementBeforeTransfer cancels a portion or all of the user’s queued claim with no payout, effectively forfeiting the previously queued entitlement.
#### Preconditions / Assumptions
- (a). User already has a significant queued shortfall and insufficient market-derived to settle it
- (b). User transfers LCC to a protocol endpoint to free headroom or attempt bucket conversion
- (c). Annulment logic runs before bucket updates (annulSettlementBeforeTransfer)
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/LiquidityHubLib.sol)
...
-        uint256 holderBal = 0;
+        uint256 marketSettle;
+        uint256 directSettle;
+        uint256 toSettle;
         if (isForHub) {
...
+            uint256 holderBal = balanceOf(lcc, recipient);
...
+            // Settle first from market-derived (bounded by reserve), then from direct (wrapped) using direct reserve
+            (uint256 wrappedBal, uint256 marketBal) = balancesOf(lcc, recipient);
+            marketSettle = Math.min(Math.min(queued, available), Math.min(maxAmount, marketBal));
+            uint256 directAvail = s.reserveOfUnderlying[underlying].direct;
+            directSettle = Math.min(
+                Math.min(queued - marketSettle, maxAmount - marketSettle),
+                Math.min(wrappedBal, directAvail)
+            );
+            toSettle = marketSettle + directSettle;
         }
...
+            pay(s, lcc, recipient, recipient, directSettle, marketSettle);
         }
     }
