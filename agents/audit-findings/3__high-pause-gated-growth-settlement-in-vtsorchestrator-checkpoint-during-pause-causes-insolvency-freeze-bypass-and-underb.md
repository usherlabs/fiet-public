[High] Pause-gated growth settlement in VTSOrchestrator.checkpoint during pause causes insolvency-freeze bypass and underbacked MM removals

# Description

The PR made [VTSOrchestrator.checkpoint](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/VTSOrchestrator.sol#L803) call a [pause-gated settlePositionGrowths wrapper](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/VTSOrchestrator.sol#L509-L521) before running commitment backing. During pause, non-CoreHook callers cannot checkpoint(withCommitment), so newly emerging commitment underbacking cannot be stored. Since remove-liquidity is still allowed via [CoreHook](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/CoreHook.sol#L114-L119), MMs can unwind while underbacked, bypassing the insolvency freeze and weakening seizure enforcement.

This PR moved growth settlement out of [VTSLifecycleLinkedLib.checkpoint](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L635-L646) and into [VTSOrchestrator.checkpoint](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/VTSOrchestrator.sol#L803) via a public [settlePositionGrowths wrapper](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/VTSOrchestrator.sol#L509-L521). That wrapper is pause-gated to only allow the canonical CoreHook during pause. Consequently, while paused, public checkpoint(withCommitment=true) reverts at settlePositionGrowths and cannot persist commitmentDeficit. Meanwhile, CoreHook removal remains permitted during pause and settles growths before decrease, but does not write commitmentDeficit. [VTSPositionLib.getRFS (used during decreases) only inflates by a stored commitmentDeficit](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1535-L1547); without it, RFS may appear closed and removal proceeds. The insolvency freeze in touchPosition ([CommitmentDeficitBlocksLiquidityChange](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1016-L1022)) also keys off stored commitmentDeficit, so it never triggers if the deficit was not stored during the pause window. [validateSeize recomputes commitment backing only when a stored commitmentDeficit already exists](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L666-L678), so it does not correct this mid-pause. Net effect: a position that becomes commitment-underbacked after a pause begins cannot have that underbacking checkpointed while paused, yet can still remove liquidity via CoreHook, undermining solvency protections. This behavior is introduced by the PR’s reordering and gating of checkpoint’s growth settlement.

# Severity

**Impact Explanation:** [High] By enabling underbacked MMs to remove liquidity during pause without the insolvency freeze or seizure enforcement, the protocol’s core solvency protections are bypassed, risking direct, material loss of principal to the protocol/LPs and breaking a core safety invariant.

**Likelihood Explanation:** [Medium] Exploitation requires a pause (plausible operational event) and either commit expiry or oracle USD price movement during the pause (both realistic), followed by a simple MM removal via CoreHook. These are uncommon but plausible conditions not reliant on trusted-role misuse.
...
### Scenario 1.
Commit signal expires during pause; observers cannot checkpoint(withCommitment); MM removes liquidity via CoreHook while RFS remains closed (no stored deficit), reducing issuance and clamping settled, thus escaping the insolvency freeze and potential seizure.
#### Preconditions / Assumptions
- (a). Pool or global pause is active (adds/swaps halted, CoreHook removes allowed).
- (b). MM-managed position initially has no stored commitmentDeficit and RFS is closed.
- (c). Commit tied to the position expires after the pause begins.
- (d). Observers/MM cannot run checkpoint(withCommitment) due to pause-gated settlePositionGrowths.
- (e). MM initiates remove-liquidity via CoreHook during pause.

### Scenario 2.
Oracle USD price changes during pause make the position underbacked; public checkpoint(withCommitment) is blocked by pause-gated settlement; MM removes liquidity via CoreHook while RFS (without stored deficit) is closed, shrinking obligations before any deficit can be recorded.
#### Preconditions / Assumptions
- (a). Pool or global pause is active.
- (b). MM-managed position initially has no stored commitmentDeficit and RFS is closed.
- (c). Oracle USD prices change during pause, creating commitment underbacking.
- (d). Observers/MM cannot run checkpoint(withCommitment) due to pause-gated settlePositionGrowths.
- (e). MM initiates remove-liquidity via CoreHook during pause.

### Scenario 3.
During or after pause, a third party attempts to seize a newly underbacked position; validateSeize does not recompute commitment backing without a stored deficit; seizure fails while the MM can remove liquidity during pause, further reducing enforceable exposure.
#### Preconditions / Assumptions
- (a). Pool or global pause is active or has recently ended.
- (b). Position became commitment-underbacked during pause but no commitmentDeficit was stored.
- (c). A third party attempts to seize; validateSeize only recomputes backing if a stored commitmentDeficit exists.
- (d). MM can remove liquidity during pause via CoreHook.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/VTSOrchestrator.sol)
...
```diff
     ...
     ReentrancyGuardTransient
 {
     ...
     function checkpoint(uint256 commitId, uint256 positionIndex, bool withCommitment) external nonReentrant {
         ...
         _assertPositionValid(positionId, true);
-        settlePositionGrowths(positionId);
+        // During pause, allow checkpoint(withCommitment) without settling growths to persist deficits
+        // while keeping CoreHook-only growth settlement. When not paused, preserve settle-first ordering.
+        PoolId poolId = s.positions[positionId].poolId;
+        if (!(s.isPaused || s.pools[poolId].isPaused)) {
+            settlePositionGrowths(positionId);
+        }
