[Medium] Active-add fee-growth rebase without banking in VTSPositionLib during residual episode causes under-collection of protocol fees

# Description

On active liquidity adds while a residual-burn episode is open, [VTSPositionLib._rebaseResidualFeeGrowthOnActiveIncrease](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSPositionLib.sol#L1128-L1167) resets feeGrowthInsideLast to the current fee growth without first burning or banking pre-add fresh fees for the already-live liquidity. Because [VTSFeeLib only advances feeGrowthInsideLast when a burn actually materializes](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSFeeLib.sol#L329-L336), this rebase can permanently erase pending fresh fees, leading to underfunded protocolFeeAccrued/slashed pot and unfair fee-sharing.

The PR introduced [_rebaseResidualFeeGrowthOnActiveIncrease()](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSPositionLib.sol#L1128-L1167), [called on active increases](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSPositionLib.sol#L1056) when [pendingResidualBurnBase is non-zero for the opposite lane](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSPositionLib.sol#L1133-L1139). This function [writes the current getFeeGrowthInside(...) directly into pa.feeGrowthInsideLast](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSPositionLib.sol#L1139-L1146) for the fee token lane(s). However, VTSFeeLib [advances feeGrowthInsideLast only when a non-zero burn occurs in _finaliseBurnAccounting](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSFeeLib.sol#L329-L336); if a burn is not eligible at settle time (e.g., ofDelta == 0) or rounds to zero, [fresh fees since the last snapshot remain pending](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSFeeLib.sol#L256-L263). The new rebase overwrites feeGrowthInsideLast without first burning or banking those pending fresh fees for the surviving live slice. Because [increases do not bank historical fee backing (banking exists only on partial decrease or full deactivation)](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/libraries/VTSFeeLib.sol#L588-L612), the erased pre-add window is never counted later. Subsequent burns compute freshFees from the rebased last and thus miss the erased window, under-collecting protocolFeeAccrued/slashed pot and reducing distributions to fee-sharing beneficiaries.

# Severity

**Impact Explanation:** [Medium] Direct, material loss of yield/fees to protocolFeeAccrued/slashed pot and unfair fee-sharing distributions due to erased fresh-fee windows.

**Likelihood Explanation:** [Medium] Exploitation requires realistic but state-dependent conditions (open residual episode and timing adds before outflows or above rounding thresholds). These occur in normal operation and can be targeted by an LP/MM, but are not always-on.
...
### Scenario 1.
Quiet outflow interval: A position is active with a residual-burn episode open on the deficit lane and fresh fees accrued on the opposite fee lane. SettlePositionGrowths finds no eligible burn (ofDelta == 0), leaving feeGrowthInsideLast unchanged. An attacker performs a small active add; the rebase resets feeGrowthInsideLast to current fg for the fee lane, erasing the pre-add fresh fees that would later be burned once outflows occur.
#### Preconditions / Assumptions
- (a). Position is active (liquidity > 0).
- (b). Residual-burn episode open: pa.pendingResidualBurnBase[deficitLane] > 0.
- (c). Fresh fees accrued on the opposite fee lane: current fg > pa.feeGrowthInsideLast for that lane.
- (d). No new outflows since residual outflow floor: ofDelta == 0 at settle time.
- (e). Fee sharing enabled (coverageFeeShare bps > 0).

### Scenario 2.
Rounding-to-zero suppression: With small bps or small outflow windows, attempted burns round to zero during SettlePositionGrowths, so feeGrowthInsideLast is not advanced. The attacker repeats small active adds to repeatedly rebase feeGrowthInsideLast to current fg, continually erasing small positive fresh-fee windows before they can aggregate into a non-zero burn.
#### Preconditions / Assumptions
- (a). Position is active (liquidity > 0).
- (b). Residual-burn episode open: pa.pendingResidualBurnBase[deficitLane] > 0.
- (c). Fresh fees accrued on the fee lane: current fg > pa.feeGrowthInsideLast.
- (d). Burn attempts at settle time round to zero (small bps or small ofDelta).
- (e). Attacker can perform repeated small active adds.

### Scenario 3.
Cross-lane targeting: The attacker sustains a residual episode on one token (deficit lane) while higher fee growth accumulates on the opposite (fee) lane. A small active add triggers the rebase on that fee lane, resetting feeGrowthInsideLast and erasing the richer pre-add fresh-fee window, maximizing under-collection.
#### Preconditions / Assumptions
- (a). Position is active (liquidity > 0).
- (b). Residual-burn episode open on one lane: pa.pendingResidualBurnBase[thatLane] > 0.
- (c). Higher fee growth accumulates on the opposite (fee) lane: current fg > pa.feeGrowthInsideLast.
- (d). Attacker can time a small active add while no non-zero burn occurs at settle time.
...
+                    if (liveLiquidityBeforeAdd > 0) {
+                        _rebaseResidualFeeGrowthOnActiveIncrease(s, ctx.poolManager, poolId, result.id, liveLiquidityBeforeAdd);
+                    }
                     requiredSettlementDelta = BalanceDelta.wrap(0);
                 }
...
+        if (needFeeToken0 && liquidityBeforeAdd > 0 && fg0 > pa.feeGrowthInsideLast.token0) { pa.pendingResidualFeeBacking.token0 += FullMath.mulDiv(fg0 - pa.feeGrowthInsideLast.token0, uint256(liquidityBeforeAdd), FixedPoint128.Q128); }
+        if (needFeeToken1 && liquidityBeforeAdd > 0 && fg1 > pa.feeGrowthInsideLast.token1) { pa.pendingResidualFeeBacking.token1 += FullMath.mulDiv(fg1 - pa.feeGrowthInsideLast.token1, uint256(liquidityBeforeAdd), FixedPoint128.Q128); }
