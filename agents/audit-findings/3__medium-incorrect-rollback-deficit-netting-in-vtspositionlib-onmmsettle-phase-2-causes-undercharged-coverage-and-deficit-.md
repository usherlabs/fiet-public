[Medium] Incorrect rollback deficit netting in VTSPositionLib.onMMSettle Phase 2 causes undercharged coverage and deficit mis-accounting

# Description

When vault withdrawals face shortfall, [onMMSettle rolls back part of the settlement](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1759-L1789) using _updateSettlement with a positive delta, which incorrectly [nets cumulative/commitment deficits](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L227-L253) (and [reduces pool totalDeficitPrincipal](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L161-L167)) despite no underlying being received. This PR increases exploitability by [recording underlyingDeltaSettlement](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1576-L1583) (including unqueued shortfall remainder) for MM decreases, making such rollbacks more frequent in seizing flows.

In VTSPositionLib.onMMSettle, [Phase 2 clamps withdrawals by vault availability using dryModifyLiquidities](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1759-L1789). If a shortfall exists, the code retroactively ['adds back' the shortfall via _sUpdateSettlement with a positive delta](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1759-L1789). _updateSettlement treats positive deltas as real deposits: it first [nets cumulativeDeficit and then commitmentDeficit](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L227-L253), and [decrements poolAccounting.totalDeficitPrincipal](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L161-L167) by the covered amount before increasing settled up to commitment maxima. However, in this rollback path no underlying was actually received for the shortfall, so netting deficits and reducing pool-wide deficit principal is incorrect and causes mis-accounting. This PR modifies MM decrease handling to [record underlyingDeltaSettlement](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1576-L1583) (the immediate settleable slice plus any unqueued shortfall remainder) [in DynamicCurrencyDelta](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1455-L1459), instead of only the immediately settleable amount. Consequently, seizing withdrawals are more likely to exceed vault availability and trigger Phase 2 rollbacks, amplifying the accounting error. The concrete impact is material undercharging of coverage (DICE) and distortion of deficit-based accounting; it can also prematurely clear commitmentDeficit and lift the MM non-seizure liquidity-change freeze without real repayment. There is no reliable path to direct principal theft established in the re-analysis; the primary harm is fee/coverage undercollection and integrity violations in deficit accounting.

# Severity

**Impact Explanation:** [Medium] Direct, material loss of coverage/fees and distortion of deficit-based accounting without a reliable, demonstrated path to direct principal theft or complete core functionality breakage.

**Likelihood Explanation:** [Medium] Requires seizing context and vault partial dryness—uncommon but plausible operational conditions; the PR change makes such rollbacks more frequent by including unqueued shortfall in the recorded underlying delta.
...
### Scenario 1.
Undercharging coverage via repeated seizing withdrawals under vault dryness: An MM executes decreases that record underlyingDeltaSettlement; during seizing, withdrawal requests exceed vault availability, causing Phase 2 rollbacks that net down cumulativeDeficit and pool totalDeficitPrincipal without actual deposits, reducing DICE charges over time.
#### Preconditions / Assumptions
- (a). Authorized seizing flow is available for the target position
- (b). Position has non-zero cumulativeDeficit on a token (RFS open)
- (c). Vault has less immediately available liquidity than the requested withdrawal (partial dryness)
- (d). Prior MM decrease recorded underlyingDeltaSettlement (includes unqueued shortfall remainder) due to this PR change
- (e). Attacker can invoke onMMSettle for seizing settlements on the position

### Scenario 2.
Bypassing MM non-seizure liquidity-change freeze: Using the same rollback mechanism, repeated shortfall add-backs net down commitmentDeficit to zero, lifting the insolvency gate for non-seizure MM liquidity changes without true repayment.
#### Preconditions / Assumptions
- (a). Authorized seizing flow is available for the target position
- (b). Position has non-zero commitmentDeficit (insolvency gate active)
- (c). Vault has partial dryness to induce shortfall rollbacks
- (d). Prior MM decrease recorded underlyingDeltaSettlement due to this PR change
- (e). Attacker can repeatedly perform seizing withdrawals to trigger rollbacks

### Scenario 3.
Skewed fee-sharing/coverage sizing: After repeatedly shrinking pool totalDeficitPrincipal via rollbacks, subsequent coverage exercises are computed on an understated principal base, undercharging the attacker’s position and distorting fee distribution.
#### Preconditions / Assumptions
- (a). Pool has non-zero totalDeficitPrincipal
- (b). Authorized seizing flow is available and vault experiences partial dryness
- (c). Prior MM decrease recorded underlyingDeltaSettlement due to this PR change
- (d). Attacker can perform repeated seizing withdrawals causing rollbacks before coverage exercises occur
...
+                // Retroactively add back shortfall to settled only (no deficit netting on rollback)
+                if (shortfall0 > 0 || shortfall1 > 0) {
+                    PositionAccounting storage pa = s.positionAccounting[p.positionId];
+                    if (shortfall0 > 0) {
+                        uint256 cur0 = pa.settled.token0; uint256 next0 = cur0 + uint256(uint128(shortfall0));
+                        pa.settled.token0 = next0; _updatePoolAccounting(s, p.positionId, 0, cur0, next0, 0);
+                    }
+                    if (shortfall1 > 0) {
+                        uint256 cur1 = pa.settled.token1; uint256 next1 = cur1 + uint256(uint128(shortfall1));
+                        pa.settled.token1 = next1; _updatePoolAccounting(s, p.positionId, 1, cur1, next1, 0);
+                    }
                 }
-                if (shortfall1 > 0) {
