[Low] Residual CISE flush plus first-settler checkpoint in VTSPositionLib on totalSettled 0→>0 causes unallocatable denominator and diluted/stuck bonuses

# Description

When a pool’s totalSettled transitions from 0 to >0, the code [flushes deferred CISE residual into the pool’s coverage-per-settled index and also adds it to the bonus denominator](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L359-L364). The PR then [immediately checkpoints the first post-zero settler past that index jump](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L206-L208). Because no positions had settled before the transition, no one can ever realize exposure matching the flushed residual, leaving a permanent denominator chunk that cannot be cleaned out. This dilutes allocations and can leave a remainder of protocolFeeAccrued/slashedPot stuck unless future exposure occurs.

On totalSettled[token] transitioning from 0 to >0, [VTSPositionLib._updatePoolAccounting calls _flushCISEResidualIfNeeded](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L178), which adds the deferred residual to coveragePerSettledIndexX128 and to totalCISEExposureSinceLastMod (the bonus denominator), then zeroes the residual. Immediately after, the PR-introduced [_checkpointFirstPostZeroSettlerCISE sets the first post-zero settler’s ciseIndexLastX128 to the post-flush index](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L179). Since there were no settled holders prior to the transition, no position can realize exposure corresponding to the flushed residual. Later, [VTSFeeLib._queueBonusForToken allocates bonuses proportionally to per-position ciseExposure over the pool-wide totalCISEExposureSinceLastMod](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSFeeLib.sol#L80), but VTSFeeLib._cleanupAfterAllocationForToken only subtracts realized per-position exposure from the denominator. As a result, the flushed residual becomes a permanent, unallocatable portion of the denominator. In low-activity markets, this creates persistent dilution and can leave protocolFeeAccrued/slashedPot partially stuck until additional CISE exposure is generated. For existing positions, [CoreHook and onMMSettle always run settlePositionGrowths first](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/CoreHook.sol#L93), [checkpointing indices before any settlement change, preventing later realization of the flushed jump](https://github.com/usherlabs/fiet-protocol/blob/9b72a3b41873a70e187650b7345cadf2727a49ae/contracts/evm/src/libraries/VTSPositionLib.sol#L1776). For new positions, initialization snapshots plus the immediate checkpoint also prevent capturing the residual. This behavior is introduced by this PR’s new checkpoint on the 0→>0 transition. A mitigation is to avoid adding the residual to totalCISEExposureSinceLastMod during the 0→>0 flush (or to neutralize it) so the denominator reflects only realizable exposure.

# Severity

**Impact Explanation:** [Medium] No principal loss or core breakage, but allocations are diluted and a portion of protocolFeeAccrued/slashedPot can remain unallocated for extended periods until more exposure occurs, constituting material loss of fees relative to a no-bug baseline.

**Likelihood Explanation:** [Low] Requires uncommon preconditions: coverage increments while totalSettled == 0 followed by the specific 0→>0 transition and limited subsequent exposure. Attacker influence is limited to timing; residual accrual depends on market conditions.
...
### Scenario 1.
Low-activity dilution: A lane accrues a CISE residual while totalSettled == 0. On the first dust settlement, the residual is flushed into the index and denominator, and the first settler is checkpointed past the jump. Later, sporadic coverage generates small exposure F. Each bonus allocation spends at most potAvail * F / (R + F); cleanup subtracts only F so the dead R persists, causing diluted payouts and leaving a remainder stuck until more exposure occurs.
#### Preconditions / Assumptions
- (a). A CISE residual R has accumulated because coverage increments occurred while totalSettled == 0.
- (b). On the next settlement, totalSettled changes 0→>0, triggering residual flush and first-settler checkpoint past the flush.
- (c). Subsequent coverage occurs but only sporadically, creating limited realizable exposure F per allocation cycle.

### Scenario 2.
Adversarial timing: An attacker monitors and times the first dust settlement right after a large residual accumulates with totalSettled == 0, maximizing the dead denominator R at flush. This increases dilution and prolongs the stuck remainder in protocolFeeAccrued/slashedPot, especially if subsequent exposure is sparse.
#### Preconditions / Assumptions
- (a). A CISE residual R has accumulated while totalSettled == 0.
- (b). An attacker can observe the state and submit the first dust settlement to trigger 0→>0.
- (c). Subsequent exposure is limited, prolonging dilution and a stuck remainder.
...
+            // Discard CISE residual accrued while totalSettled == 0 to avoid creating an unallocatable
+            // denominator/index jump. Do not modify coveragePerSettledIndexX128 or totalCISEExposureSinceLastMod here.
