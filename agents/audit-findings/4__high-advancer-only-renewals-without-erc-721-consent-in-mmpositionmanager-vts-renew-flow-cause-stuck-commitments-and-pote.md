[High] Advancer‑only renewals without ERC‑721 consent in MMPositionManager/VTS renew flow cause stuck commitments and potential third‑party seizure

# Description

Renewals are authorized solely by the VRL “advancer” and are not gated by ERC‑721 ownership, while MM operations require both NFT approval/ownership and the same advancer. After an ERC‑721 transfer without advancer rotation, the new owner cannot operate or unwind positions; over time, positions can be seized by third parties.

The renew path ( [MMPositionManager._renewSignal](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/MMPositionManager.sol#L277-L287) → VTSOrchestrator.renewSignal → VTSCommitLib.renewSignal) does not check ERC‑721 ownership/approval. [VTSCommitLib._renewSignalInternal instead authorizes renewal if and only if the caller equals the VRL-verified advancer and the signal’s owner matches the stored commit owner](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSCommitLib.sol#L302-L307). Separately, all MM operations (mint/increase/decrease/burn/settle) are dual-gated: the caller must be both (a) the ERC‑721 commitment NFT owner/approved ([checked in MMPositionActionsImpl via MMHelpers.assertApprovedOrOwner](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/MMPositionActionsImpl.sol#L504-L509)) and (b) the commit’s current advancer ([checked in VTSLifecycleLinkedLib.validateMMOperation](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L751-L756)). After transferring the commitment NFT without rotating the advancer, the new owner cannot satisfy both gates and is therefore unable to operate or unwind. [Non‑seizure settlement also requires a live signal](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/VTSOrchestrator.sol#L711), further blocking unwinds if the advancer withholds renewal. Over time, checkpointing and RFS/grace rules ([CheckpointLibrary.isSeizable](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/Checkpoint.sol#L165-L183)) can render the position seizable, allowing a third party (neither owner nor approved) to seize value. Settlement and LCC routing [send outflows to the locker (the seizer)](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/MMPositionActionsImpl.sol#L433-L442), not the NFT owner, causing principal loss. This is not a direct authorization bypass to drain funds by the old advancer, but it is a severe design flaw where ERC‑721 transfer does not convey effective control, resulting in permanent stuck funds and realistic loss via third‑party seizure.

# Severity

**Impact Explanation:** [High] Funds can be frozen for longer than a week with no on-chain workaround and can be lost via third‑party seizure; both qualify as high impact (principal loss and long-term freeze).

**Likelihood Explanation:** [Medium] Requires plausible but not universal conditions: transfer of an active commitment without advancer rotation and subsequent market/RFS evolution leading to seizure or persistent frozen state.
...
### Scenario 1.
Buyer acquires a live commitment NFT with active positions while the old advancer remains unchanged; buyer cannot perform any MM operations (fails advancer match) and is permanently stuck; later, a third party seizes value after RFS/grace elapses, routing funds to the seizer.
#### Preconditions / Assumptions
- (a). Commit has active positions at transfer time
- (b). Stored commit.mmState.advancer is not rotated to the buyer
- (c). Signal remains live via periodic renewals by the old advancer
- (d). RFS opens and grace elapses per protocol rules
- (e). A third party (neither owner nor approved) performs seizure

### Scenario 2.
Old advancer withholds renewal so the signal expires after the NFT transfer; the new owner cannot operate because non‑seizure settlement requires a live signal; expired signal also worsens commitment-derived deficits; after grace, a third party seizes and extracts value.
#### Preconditions / Assumptions
- (a). Commit has active positions at transfer time
- (b). Stored commit.mmState.advancer is not rotated to the buyer
- (c). Old advancer withholds renewal causing the signal to expire
- (d). Checkpointing and RFS/grace rules apply
- (e). A third party (neither owner nor approved) performs seizure
...
+        // TODO: Align advancer to `to` before transfer (e.g., vtsOrchestrator.forceAlignAdvancer(id, to)) so control follows ERC-721 owner.
...
+        // TODO: Enforce an owner-controlled `allowedAdvancer` on `commitId` (e.g., VTSStorage mapping)
+        //       so renewals must use the on-chain allowed advancer and cannot be performed by a stale advancer post-transfer.
...
+        // TODO: Record `s.commitRouter[commitId] = caller` (router) and optionally seed `allowedAdvancer`
+        //       to support post-transfer advancer alignment and renewal gating.
     }
...
```diff
     ...
     ReentrancyGuardTransient
 {
         ...
         nonReentrant
     {
         // Validate commit exists (but don't require live signal - expired signals can be seized)
         _assertSignalValid(commitId, false);
+        // TODO: Enforce an owner-controlled `allowedAdvancer` for `commitId` before delegating,
+        //       preventing stale advancers from renewing post-transfer.
