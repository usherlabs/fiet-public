[High] Unbacked settled clamp by queued shortfall in VTSPositionLib MM decrease path causes permanent loss of position settled value

# Description

In non-seizing MM decreases, the code [clamps a position’s settled amounts by the full vault shortfall (queuedShortfallDelta)](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1486-L1493) even when only a smaller, principal-backed subset is actually queued. The unqueued remainder is not credited to underlying deltas and is removed from pa.settled, resulting in permanent loss of the position’s withdrawable/settled claim.

During an MM decrease, the PR introduced a split between immediate settlement (settleableDelta) and [vault shortfall (queuedShortfallDelta)](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1612). The helper that stages LCC cancellation+queue ([planCancelWithQueue](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1625-L1633)) only queues per-token up to retainedPrincipal = min(shortfall, principal minted for that token in this remove) and only if that token actually had principal minted. Immediately after, the code [clamps pa.settled by the full queuedShortfallDelta](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1486-L1493) for each token, not the smaller retainedPrincipal actually queued. [DynamicCurrencyDelta](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1502-L1504) is then [set only to settleableDelta](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1495), excluding the queued shortfall remainder. When required settlement on a token exceeds the principal minted for that token in this decrease (or the remove returns principal solely in the other token), the difference between queuedShortfallDelta and retainedPrincipal is neither queued nor recorded as an underlying delta claim, yet it is deducted from pa.settled. This permanently destroys the position’s settled claim for that amount. This behavior was introduced by the PR’s new settleable/queued split and immediate clamp logic in the MM decrease path.

# Severity

**Impact Explanation:** [High] The position’s settled amounts (which determine withdrawable principal and RFS states) are reduced without a corresponding queue or underlying delta claim, causing direct, material loss of user principal.

**Likelihood Explanation:** [Medium] The bug manifests under realistic but not universal conditions: vault partial or zero availability at the time of the decrease and asymmetric or insufficient same-token principal minted for the shortfall token. These are plausible CLMM and market states and do not require trusted role misuse.
...
### Scenario 1.
Asymmetric removal returns principal only in token1 while excess and shortfall occur in token0; the vault is dry for token0 so queuedShortfallDelta0 equals the full required0, but no token0 principal is minted and no queue is staged for token0. The code then clamps pa.settled.token0 by the full shortfall without a compensating queue or underlying delta credit, permanently reducing the position’s settled0.
#### Preconditions / Assumptions
- (a). Victim’s position will have pa.settled.token0 > new commitmentMax.token0 after the decrease (excess arises on token0)
- (b). Vault availability for token0 at this moment is 0 (dry)
- (c). Remove-liquidity returns principal only in token1 (principalAmount0 = 0)
- (d). Non-seizing MM decrease path

### Scenario 2.
Partial principal is minted for the shortfall token: required0 > available0 and 0 < principalAmount0 < shortfall0. Only principalAmount0 is queued for token0, but pa.settled.token0 is reduced by the full shortfall0. The difference shortfall0 - principalAmount0 is not queued or credited and is permanently lost from settled0.
#### Preconditions / Assumptions
- (a). Victim’s position has excess on token0 after the decrease
- (b). Vault availability for token0 is less than required (available0 < required0, shortfall0 > 0)
- (c). The remove mints some but insufficient token0 principal (0 < principalAmount0 < shortfall0)
- (d). Non-seizing MM decrease path

### Scenario 3.
An attacker with queued claims on the same underlying first settles their queue permissionlessly to drain available reserves just before the victim’s decrease. This increases the victim’s queuedShortfallDelta on the affected token. If the remove mints insufficient principal for that token, only part of the shortfall is queued while the full shortfall is deducted from pa.settled, amplifying the victim’s permanent loss.
#### Preconditions / Assumptions
- (a). Attacker holds a queued settlement for the same underlying and can call [processSettlementFor](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/LiquidityHub.sol#L939) to consume reserves
- (b). Attacker settles just before the victim’s decrease, reducing vault availability for the victim’s shortfall token
- (c). Victim’s decrease creates excess on that token and mints insufficient same-token principal
- (d). Non-seizing MM decrease path
...
+            // Clamp by the actually queued principal per token (retainedPrincipal), not the full vault shortfall.
+            // This ensures every decrement to pa.settled is matched by a real queued claim.
+            BalanceDelta cancelPrincipal = isSeizing ? requiredSettlementDelta : principalDelta;
+            uint256 qs0 = LiquidityUtils.safeInt128ToUint256(decreaseResult.queuedShortfallDelta.amount0());
+            uint256 qs1 = LiquidityUtils.safeInt128ToUint256(decreaseResult.queuedShortfallDelta.amount1());
+            uint256 qp0 = LiquidityUtils.safeInt128ToUint256(cancelPrincipal.amount0());
+            uint256 qp1 = LiquidityUtils.safeInt128ToUint256(cancelPrincipal.amount1());
+            uint256 clamp0 = qs0 < qp0 ? qs0 : qp0;
+            uint256 clamp1 = qs1 < qp1 ? qs1 : qp1;
