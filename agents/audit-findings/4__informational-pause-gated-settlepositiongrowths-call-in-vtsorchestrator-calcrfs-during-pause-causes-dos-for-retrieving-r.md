[Informational] Pause-gated settlePositionGrowths call in VTSOrchestrator.calcRFS during pause causes DoS for retrieving RFS

# Description

VTSOrchestrator.calcRFS now [calls a pause-gated settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L400), which reverts for non-CoreHook callers during pause, preventing RFS retrieval.

The calcRFS function in VTSOrchestrator was modified to [invoke settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L400) before returning the RFS delta. [settlePositionGrowths enforces pause policy](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L507-L524): when the protocol (globally or per-pool) is paused, [only the canonical CoreHook may call it](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L511-L518). As a result, calcRFS reverts for non-CoreHook callers during pause, denying users, routers, and off-chain tools from retrieving RFS (Required For Settlement) information. This creates an observability/read-path denial precisely when pause is active. No state changes, funds safety, or invariants are affected.

# Severity

**Impact Explanation:** [Informational] Read-path/view-only denial during pause that does not affect state, funds, or invariants; it reduces observability but not safety.

**Likelihood Explanation:** [High] Occurs whenever pause is engaged; no attacker action is required and no special constraints beyond pause being active.
...
### Scenario 1.
End-user UI or off-chain monitor calls [calcRFS(positionId, ...)](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L400) during a global or pool-level pause; the internal [settlePositionGrowths call reverts](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L511-L518) for non-CoreHook callers, blocking RFS retrieval.
#### Preconditions / Assumptions
- (a). Global or pool-level pause is active
- (b). Caller is not the canonical CoreHook
- (c). Caller invokes VTSOrchestrator.calcRFS to read RFS

### Scenario 2.
An MM router/integration calls [calcRFS(commitId, positionIndex, ...)](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L415) during a pool-level pause to plan operations; the internal [settlePositionGrowths call reverts](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L511-L518) for the non-CoreHook caller, preventing RFS inspection.
#### Preconditions / Assumptions
- (a). Pool-level pause is active
- (b). Integration/router caller is not the canonical CoreHook
- (c). Caller invokes VTSOrchestrator.calcRFS(commitId, positionIndex, ...)

### Scenario 3.
Auditor or custodial scripts run during an incident pause and call calcRFS for multiple positions; each call reverts at [settlePositionGrowths due to pause gating](https://github.com/usherlabs/fiet-protocol/blob/79d35154c7db92f2387d77faa9958da4c59b639a/contracts/evm/src/VTSOrchestrator.sol#L511-L518), denying visibility into RFS status.
#### Preconditions / Assumptions
- (a). An incident or maintenance triggers a pause
- (b). Auditor/custodial script is not the canonical CoreHook
- (c). Script invokes VTSOrchestrator.calcRFS for positions
...
+        // During pause, allow view-only RFS without settling growths to preserve observability.
+        PoolId poolId = s.positions[positionId].poolId;
+        bool poolOrGlobalPaused = s.isPaused || s.pools[poolId].isPaused;
+        if (!poolOrGlobalPaused) {
+            settlePositionGrowths(positionId);
+        }
