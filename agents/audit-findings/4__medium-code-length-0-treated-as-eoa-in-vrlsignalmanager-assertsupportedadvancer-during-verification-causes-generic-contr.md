[Medium] code.length==0 treated as EOA in VRLSignalManager._assertSupportedAdvancer during verification causes generic contract-wallet advancer control until expiry

# Description

The PR’s new advancer shape check in VRLSignalManager accepts code.length==0 as an EOA at proof time, which is also true for undeployed or in-constructor addresses. An MM can commit with such an address and later deploy arbitrary code to it. Subsequent MM-operation checks only compare addresses and never revalidate code shape, allowing a generic contract-wallet advancer during the commit window.

The PR introduced _assertSupportedAdvancer in VRLSignalManager, intended to allow only EOAs (adv.code.length==0) or canonical EIP-7702 delegation stubs (23-byte code with non-zero delegate). This check runs during _verifyLiquiditySignalInternal. However, at verification time an undeployed CREATE2 address or an address in a contract’s constructor also has code.length==0 and is accepted as an EOA. After the commit is stored with that advancer address, the attacker can deploy arbitrary code to it. Later MM operations (e.g., via VTSLifecycleLinkedLib.validateMMOperation) only require that the locker address equals the stored advancer; no code-shape revalidation occurs. Thus, the PR’s intended restriction against generic contract-wallet/1271 advancers is unenforced for the lifetime of a commit (until expiry or explicit renewal, where the shape is rechecked). This is a policy/authorization hardening failure introduced by the PR’s new logic.

# Severity

**Impact Explanation:** [Low] This is a correctness/authorization policy enforcement gap. It does not directly cause principal loss, invariant breaks, or funds freeze; it undermines a newly introduced auth restriction during the active commit window.

**Likelihood Explanation:** [High] Exploitation uses standard EVM behavior (code.length==0 for undeployed or in-constructor addresses) and normal MM commit/operation flows; no rare states or special constraints are required.
...
### Scenario 1.
CREATE2 advancer bypass: An MM commits a signal with mmState.advancer set to an undeployed CREATE2 address (code.length==0). VRLSignalManager accepts it as an EOA at verification time. After commit storage, the MM deploys code to that address and performs non-seizing MM operations from the contract (locker==advancer), which pass since only address equality is checked.
#### Preconditions / Assumptions
- (a). Attacker is a Market Maker controlling mmState in the signed VRL leaf
- (b). Valid VRL proof available
- (c). mmState.advancer chosen as an undeployed CREATE2 address (code.length==0 at verification)
- (d). Commit submitted via MMPositionManager; VRLSignalManager accepts sender==owner
- (e). Later deployment of arbitrary code at the advancer address
- (f). Non-seizing MM operations performed while the signal is live

### Scenario 2.
In-constructor advancer: A contract’s constructor submits the commit with mmState.advancer set to its own address. During constructor execution extcodesize==0, so the advancer is accepted as an EOA. After deployment, the contract performs non-seizing MM operations as locker (equal to advancer), with no code-shape revalidation blocking it.
#### Preconditions / Assumptions
- (a). Contract address A submits commit during its constructor
- (b). During constructor, extcodesize(A)==0 so advancer passes as EOA
- (c). After deployment, A has code and can call MMPositionManager
- (d). Non-seizing MM operations performed while the signal is live

### Scenario 3.
Operate then rotate on renewal: The MM operates with a contract advancer during the initial commit window. Near expiry, they submit a renewal with a compliant advancer (EOA or canonical 7702 stub) as sender, passing the recheck. The initial window still ran under a non-compliant contract-wallet advancer.
#### Preconditions / Assumptions
- (a). Initial commit accepted with an advancer that later has code
- (b). Non-seizing MM operations occur during the initial commit window
- (c). Near expiry, a renewal is prepared with a compliant advancer (EOA/7702)
- (d). Renewal sender equals the new compliant advancer so renewal passes
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/c4701cca39c86cfb6c6e2b3416fee5d89294b87a/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol)
...
-                revert Errors.InvalidSender();
+            address advancer = s.commits[mmData.commitId].mmState.advancer;
+            if (locker != advancer) { revert Errors.InvalidSender(); }
+            bytes32 h; assembly { h := extcodehash(advancer) }
+            uint256 len = advancer.code.length;
+            if (h == 0 || (len != 0 && (len != 23 || EIP7702Utils.fetchDelegate(advancer) == address(0)))) {
+                revert Errors.InvalidAdvancer(advancer);
             }
         }
