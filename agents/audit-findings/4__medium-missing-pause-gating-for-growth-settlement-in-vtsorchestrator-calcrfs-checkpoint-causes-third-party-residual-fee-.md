[Medium] Missing pause gating for growth settlement in VTSOrchestrator.calcRFS/checkpoint causes third‑party residual fee-backing consumption and yield redistribution

# Description

calcRFS and checkpoint call settlePositionGrowths without the CoreHook-only pause restriction, allowing any caller to force another position’s residual fee-backing consumption/clearing and advance pot accounting while paused.

This PR introduced pendingResidualFeeBacking and new residual-burn/cleanup side effects during growth settlement. VTSOrchestrator.settlePositionGrowths(PositionId) correctly enforces that, when paused, only the canonical CoreHook may settle growths. However, VTSOrchestrator.calcRFS(...) and checkpoint(...) still call into VTSPositionLib.settlePositionGrowths(...) indirectly without applying the same pause gating. As a result, while the market or pool is paused, any address can invoke calcRFS or checkpoint for another user’s active position to trigger growth settlement and the new residual fee-backing mutations: consuming/clearing pendingResidualFeeBacking and increasing protocolFeeAccrued (pot). The attacker can then perform an allowed remove-liquidity operation (permitted during pause) to trigger fee processing and capture bonuses from the now-larger pot via CISE-based allocation. This violates the intended pause semantics and can cause material yield/fee redistribution.

# Severity

**Impact Explanation:** [Medium] Enables third parties to mutate others’ accounting during pause, consuming/clearing residual fee-backing and increasing the pot, which can be immediately harvested via allowed remove-liquidity to capture bonuses. This is a material yield/fees redistribution and a break of intended pause semantics, but not a direct principal loss.

**Likelihood Explanation:** [Medium] Exploitation requires paused state and positions with residual-backing episodes, which are realistic but not universal. Public entrypoints and straightforward enumeration make the attack feasible when conditions exist.
...
### Scenario 1.
While paused, an attacker enumerates victim positions and calls calcRFS(positionId). The call settles growths and consumes/clears the victim’s pendingResidualFeeBacking, increasing the pool’s protocolFeeAccrued. The attacker then removes their own liquidity (allowed during pause) so fee processing allocates bonuses from the enlarged pot to the attacker via CISE exposure.
#### Preconditions / Assumptions
- (a). Global or pool pause is active
- (b). Victim position is active (non-zero liquidity)
- (c). Victim position has a current residual-burn episode (pendingResidualBurnBase > 0) and non-zero pendingResidualFeeBacking
- (d). Sufficient growth/outflow context exists so residual burn can consume a portion during settlement
- (e). Attacker can enumerate commits and positions via public getters

### Scenario 2.
While paused, an attacker calls checkpoint(commitId, positionIndex, withCommitment=any) on a victim’s active position. This settles growths and triggers residual fee-backing consumption/clearing, growing the pot. The attacker then removes liquidity on their own position to claim bonuses from the increased pot.
#### Preconditions / Assumptions
- (a). Global or pool pause is active
- (b). Victim position is active (non-zero liquidity)
- (c). Victim position has a current residual-burn episode (pendingResidualBurnBase > 0) and non-zero pendingResidualFeeBacking
- (d). Attacker can resolve (commitId, positionIndex) to a PositionId via public getters

### Scenario 3.
During a pause, an attacker bulk-calls calcRFS or checkpoint against many active positions with residual-backing episodes, forcing widespread residual consumption and pot growth. The attacker then removes liquidity from their positions to capture a larger aggregate share of the pot via CISE allocations.
#### Preconditions / Assumptions
- (a). Global or pool pause is active
- (b). Multiple active positions have ongoing residual-burn episodes with non-zero pendingResidualFeeBacking
- (c). Attacker holds positions with CISE exposure and can remove liquidity during pause
- (d). Attacker can enumerate commits and positions via public getters
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/VTSOrchestrator.sol)
...
+        // Enforce pause policy and settle via the gated entrypoint, then compute RFS without further mutation
...
+        settlePositionGrowths(positionId);
+        (bool rfsOpen, BalanceDelta delta) = VTSPositionLib.getRFS(s, positionId);
+        if (requireClosedRfS && rfsOpen) revert Errors.RFSOpenForPosition(positionId);
     ...
     function checkpoint(uint256 commitId, uint256 positionIndex, bool withCommitment) external nonReentrant {
         ...
         PositionId positionId = getPositionId(commitId, positionIndex);
         _assertPositionValid(positionId, true);
+        // Enforce pause policy and settlement via the gated entrypoint
+        settlePositionGrowths(positionId);
