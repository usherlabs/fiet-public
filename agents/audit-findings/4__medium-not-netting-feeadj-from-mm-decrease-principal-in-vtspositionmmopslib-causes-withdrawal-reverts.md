[Medium] Not netting feeAdj from MM decrease principal in VTSPositionMMOpsLib causes withdrawal reverts

# Description

MM decreases/burns stage planned-cancel/queue using [principal = callerDelta - feesAccrued (not net of feeAdj)](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/VTSPositionMMOpsLib.sol#L101). The router later forwards only the immediate non-fee LCC actually received post-hook and post-burn. When feeAdj > feesAccrued and shortfall is queued, the immediate non-fee is less than the queued amount, triggering a revert and blocking decreases/burns.

The PR changed MM decrease/burn principal staging in VTSPositionMMOpsLib to [principalDelta = p.callerDelta - p.feesAccrued (explicitly not net of feeAdj)](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/VTSPositionMMOpsLib.sol#L101). During a decrease/burn, VTS uses this gross principal to plan cancel/queue via [LiquidityHub.planCancelWithQueue](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/VTSPositionMMOpsLib.sol#L595). After the hook returns, PoolManager [applies feeAdj to the caller’s delta](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/lib/v4-periphery/lib/v4-core/src/libraries/Hooks.sol#L242) and transfers LCC to the router. [LCC._afterTransfer executes the planned cancel (burning the cancellable slice and queuing the rest)](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/LiquidityHub.sol#L1068-L1079). The router classifies the immediate, post-transfer non-fee LCC as [nonFee = max(inc - max(feesAccrued - hookDelta, 0), 0)](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/LiquidityUtils.sol#L277-L285), where inc is the balance increase post-transfer and hookDelta is feeAdj. If feeAdj > feesAccrued and there is a queued shortfall, nonFee becomes less than the queued amount (qCommitted), and PositionManagerImpl [reverts (Errors.InsufficientBalance)](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/modules/PositionManagerImpl.sol#L191). This blocks decreases/burns for affected positions until accounting or settleability improves. Previously, netting feeAdj in principal aligned the staged queue with what could be immediately forwarded, avoiding this mismatch.

# Severity

**Impact Explanation:** [Medium] Significant but conditional DoS of withdrawal paths (decreases/burns/seizure routing) for affected positions, with no direct funds loss or invariant break; persistence depends on evolving accounting/settleability.

**Likelihood Explanation:** [Medium] Requires concurrent conditions feeAdj > feesAccrued and shortfall > 0, which are plausible but not universal; arises under realistic market/accounting states.
...
### Scenario 1.
Non-seizure MM decrease: VTS stages queue from gross principal; feeAdj > feesAccrued and shortfall > 0; after transfer and planned-cancel, router computes nonFee < qCommitted and [reverts, blocking the decrease](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/modules/PositionManagerImpl.sol#L191).
#### Preconditions / Assumptions
- (a). Valid MM position and commit; locker/advancer authorized
- (b). During this touch, feeAdj (hook delta) > feesAccrued
- (c). Vault settleability limited so shortfall is queued (> 0)

### Scenario 2.
Full position burn: With minimal settleable underlying, queued ~ principal; feeAdj > feesAccrued; post-transfer [nonFee < queued](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/LiquidityUtils.sol#L277-L285); router reverts, preventing position closure.
#### Preconditions / Assumptions
- (a). Valid MM position attempting full burn
- (b). feeAdj (hook delta) > feesAccrued in this call
- (c). Vault settleability ~ 0 so queued principal ~ full principal

### Scenario 3.
Seizure decrease: Seizure retains/queues principal to guarantor; feeAdj > feesAccrued; post-transfer nonFee < retained queue; router [reverts, blocking seizure custody routing](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/modules/PositionManagerImpl.sol#L191).
#### Preconditions / Assumptions
- (a). Authorized seizure path (locker/seizer permitted)
- (b). feeAdj (hook delta) > feesAccrued in this call
- (c). Shortfall exists so retained principal is queued to guarantor
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/VTSPositionLib.sol)
...
+        // FIX/TODO: For MM decreases/burns that stage a queued shortfall on a leg in this call,
+        // cap the per-leg positive feeAdj materialised in this call to that leg's feesAccrued.
+        // 1) Precompute per-leg "queue will be staged" booleans from principalDelta (callerDelta - feesAccrued)
+        //    and requiredSettlementDelta (use existing decrease/seizure split helpers).
+        // 2) Call a capped fee processing variant (e.g. afterTouchPositionCapped) that enforces:
+        //    if (queuedLeg && pending_leg > 0) mat_leg = min(pending_leg, feesAccrued_leg); defer the rest in pending.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/VTSFeeLib.sol)
...
```diff
 ...
 library VTSFeeLib {
     ...
     {
+        // FIX/TODO: When MM decreases/burns stage a queued shortfall on a leg in this call,
+        // bound the per-leg positive materialisation (mat_leg) by that leg's feesAccrued for custody parity:
+        //   if (queuedLeg && pend_leg > 0) mat_leg = min(pend_leg, feesAccrued_leg);
+        // Defer any remainder in pa.pendingFeeAdj for later calls (when no queue or higher feesAccrued).
+        // Note: requires passing per-leg queued flags and per-leg feesAccrued into this function (signature change).
+
