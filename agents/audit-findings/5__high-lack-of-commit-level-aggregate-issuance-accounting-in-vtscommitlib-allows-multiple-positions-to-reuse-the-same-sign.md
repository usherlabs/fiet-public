[High] Lack of commit-level aggregate issuance accounting in VTSCommitLib allows multiple positions to reuse the same signal, causing disproportionate fee capture and reserve/queue pressure

# Description

The commit’s VRL signal value is treated as fully available for every linked position. Neither issuance gating nor commitment checkpointing aggregates or allocates the signal across positions under the same commit. This enables aggregate over-issuance of exposure across multiple positions while each remains individually healthy, allowing disproportionate fee extraction and increased settlement queues.

In VTSCommitLib, [_signalValueForCommit returns a single total signal value for a commit](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSCommitLib.sol#L410-L421). During MM add-liquidity, [VTSPositionLib._handleLiquidityIncrease calls VTSCommitLib.validateLiquidityDelta](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSPositionLib.sol#L1561-L1580) which [checks only the one-position liquidityDelta as issuedValue <= full commit signalValue + that position’s settledValue](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSCommitLib.sol#L134-L153). At commitment checkpointing, [VTSCommitLib.checkpointWithCommitment again compares a single position’s total issuedUsd against the same full commit signalUsd (unless expired) plus that position’s settledUsd](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/libraries/VTSCommitLib.sol#L334-L373). The [Commit struct maintains no aggregate issued accounting across positions](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/types/Commit.sol#L24-L35). LiquidityHub [confirmTake is balance-backed](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/LiquidityHub.sol#L910-L914) and [unwrap shortfalls are explicitly queued](https://github.com/usherlabs/fiet-protocol/blob/6a0c94e37c95e1f67e3fc5b57cf49e352233d6f0/contracts/evm/src/LiquidityHub.sol#L576-L595), so immediate unbacked payouts are prevented; however, the attacker can farm disproportionate swap fees via over-issued liquidity and steadily unwrap fee LCC to underlying as liquidity becomes available, increasing settlement queues and degrading serviceability for others.

# Severity

**Impact Explanation:** [Medium] Enables direct, material loss of yield/fees (disproportionate fee capture) and serviceability degradation via larger settlement queues; does not directly force unbacked principal withdrawals due to confirmTake and queue safeguards.

**Likelihood Explanation:** [High] Requires no special constraints; an MM can open multiple positions under the same commit using normal flows; clear economic incentive to exploit.
...
### Scenario 1.
Two positions under one commit reuse the same signal to over-issue liquidity and extract disproportionate fees: (1) Attacker obtains a commit with ~100 USD signal; opens Position A and adds ~80 USD issued value (passes against full 100). (2) Opens Position B and adds another ~80 USD (also passes against full 100). (3) Both positions farm swap fees; attacker unwraps fee LCC to underlying, depleting reserves and creating larger settlement queues for others.
#### Preconditions / Assumptions
- (a). Attacker can obtain a valid commit (commitId) with non-zero VRL-backed signal value (e.g., ~100 USD).
- (b). Uniswap v4 PoolManager and hooks function canonically; oracle integrity holds; standard ERC20 behaviors.
- (c). Active trading exists so positions accrue fees; LiquidityHub and MarketVault can service unwraps (immediate or queued).

### Scenario 2.
Cross-pool multiplication of the effect: (1) Attacker uses the same commit to open Position A in Pool X and Position B in Pool Y, each validated per-position against the full commit signal. (2) Attacker farms fees across multiple pools and unwraps fee LCC to underlying. (3) This amplifies fee redistribution to the attacker and increases queued settlements across markets.
#### Preconditions / Assumptions
- (a). Same as Scenario 1, plus multiple pools available for the same market/commit use.
- (b). Positions across pools can link to the same commitId.

### Scenario 3.
Pre-expiry extraction, post-expiry deficit reveal: (1) Attacker over-issues across multiple positions under the same commit and extracts fees via unwrap while the signal is live. (2) After commit expiry, checkpointWithCommitment uses signalUsd = 0 and can record commitment deficits on positions; however, earlier fee extraction remains with the attacker. (3) System and other participants face larger queues and later insolvency mechanics (deficits/seizure) without recovering earlier gains.
#### Preconditions / Assumptions
- (a). Same as Scenario 1, and the commit will eventually expire.
- (b). Checkpointing after expiry sets signalUsd = 0 for commitment checks.
...
```diff
 ...
 library VTSCommitLib {
     ...
     function validateLiquidityDelta(
         ...
         IOracleHelper oracleHelper,
         uint256 commitId,
+        // SECURITY NOTE: This check is per-position only; enforce commit-level aggregate budget (remaining signal) at the call site before issuing to prevent aggregate overuse across positions.
         PositionId positionId,
...
+        // SECURITY TODO: Adjust commit-level allocation on settlement deltas (USD): decrease allocation on settled increases (up to current), and increase on settled decreases (bounded by maxima-derived net need).
...
+        // SECURITY TODO: On liquidity decrease, free commit allocation derived from the reduction in commitment maxima (USD): decrement commit.{total,position} allocation by the freed amount.
         if (hookData.isMMOperation) {
...
+        // SECURITY TODO: Gate add-liquidity by commit-level remaining signal: compute incrementalNeedUsd from maxima delta (USD) and require incrementalNeedUsd <= (signalUsd - commit.totalAllocatedUsd), then increment commit.{total,position} allocation.
...
+// SECURITY TODO: Add commit-level allocation fields (e.g., totalAllocatedUsd and mapping(PositionId=>uint256) allocatedUsd) to prevent aggregate overuse of a commit's signal across multiple positions by enforcing budget on add/remove/settle.
