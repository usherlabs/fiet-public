[High] Missing min-out slippage checks in MMPositionActionsImpl decrease/burn causes sandwichable withdrawals and principal loss

# Description

The [DECREASE_LIQUIDITY and BURN_POSITION flows in MMPositionActionsImpl](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/MMPositionActionsImpl.sol#L154-L173) do not accept amount0Min/amount1Min and [never validate minimum output after modifyLiquidity](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/modules/PositionManagerImpl.sol#L283-L300), enabling front-running to shift price and force unfavorable withdrawals without revert protection.

In MMPositionActionsImpl, the DECREASE_LIQUIDITY and BURN_POSITION actions [decode parameters without amount0Min/amount1Min](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/MMCalldataDecoder.sol#L121-L138), and the downstream [PositionManagerImpl._modifySyntheticLiquidity settles/takes deltas from poolManager.modifyLiquidity without any min-out enforcement](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/modules/PositionManagerImpl.sol#L283-L300). As a result, the amount of token0/token1 returned on removal depends solely on the current core pool state and can be adversarially worsened via a sandwich (front-run/back-run) with no on-chain slippage guard. This deviates from Uniswap v4 PositionManager, which enforces [SlippageCheck.validateMinOut on removal](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/lib/v4-periphery/src/PositionManager.sol#L345) / [burn](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/lib/v4-periphery/src/PositionManager.sol#L435). RFS/coverage/queueing logic is orthogonal and does not mitigate this risk. An attacker can obtain LCC via LiquidityHub.wrap and trade the core pool to manipulate price before a victim’s removal, then revert price after, extracting value while the victim suffers reduced proceeds.

# Severity

**Impact Explanation:** [High] Users can suffer direct, material loss of principal value when removing or burning liquidity at an adversarially moved price without any on-chain min-out revert protection.

**Likelihood Explanation:** [Medium] Exploitation requires typical MEV-style front-run/back-run and capital proportional to pool depth; it is realistic with clear profit motive but not guaranteed in every market condition.
...
### Scenario 1.
Attacker sandwiches a DECREASE_LIQUIDITY: front-runs a swap on the core pool to push price against the victim, the victim’s removal executes at the worse price with no min-out checks, and the attacker back-runs to restore price, capturing profit while the victim receives fewer tokens.
#### Preconditions / Assumptions
- (a). A core market (LCC/LCC) exists and allows swaps via Uniswap v4 PoolManager and CoreHook.
- (b). Victim holds a live position with liquidity and invokes DECREASE_LIQUIDITY through MMPositionManager (no min-out parameters available).
- (c). Attacker can view mempool and swap in the core pool (can acquire LCC by calling LiquidityHub.wrap/wrapTo).

### Scenario 2.
Attacker sandwiches a full BURN_POSITION: the attacker front-runs to move price adversely, the victim’s full removal executes at that price with no min-out guard, and the attacker back-runs to reset price; the victim’s entire exit is realized at a worse rate, increasing loss magnitude.
#### Preconditions / Assumptions
- (a). A core market (LCC/LCC) exists and allows swaps.
- (b). Victim holds a live position and invokes BURN_POSITION through MMPositionManager (no min-out parameters available).
- (c). Attacker can view mempool and swap in the core pool (can acquire LCC via LiquidityHub.wrap/wrapTo).

### Scenario 3.
Removal proceeds routed to MM queue are reduced: the attacker front-runs a price move before the victim’s DECREASE_LIQUIDITY, causing fewer LCC to be credited/forwarded to the custodian; later collection via LiquidityHub settles less underlying than expected because the loss was crystallized at removal.
#### Preconditions / Assumptions
- (a). A core market (LCC/LCC) exists and allows swaps.
- (b). Victim performs DECREASE_LIQUIDITY; positive LCC deltas are synced/forwarded to MM queue custody.
- (c). Attacker can view mempool and swap in the core pool (can acquire LCC via LiquidityHub.wrap/wrapTo).
...
```diff
     ...
     DelegateCallGuard
 {
     ...
     /// @param tokenId The commitment NFT token ID
     /// @param positionIndex The position index within the commitment
+    // TODO(Security): Add a slippage-aware burn variant that accepts (amount0Min, amount1Min) and,
+    // after _modifySyntheticLiquidity returns, validates principal delta via SlippageCheck.validateMinOut.
...
+        // NOTE: When adding slippage-aware paths, capture (callerDelta, feesAccrued) here and validate
+        // (callerDelta - feesAccrued) against user-provided minima to mirror Uniswap v4 periphery semantics.
         _modifySyntheticLiquidity(poolKey, params, tokenId, hookData);
     }
     ...
     /// @param positionIndex The position index within the commitment
     /// @param amountToDecrease The amount of liquidity to remove
+    // TODO(Security): Provide a slippage-aware decrease action that accepts (amount0Min, amount1Min) and,
+    // after _modifySyntheticLiquidity returns, computes principal delta and enforces
+    // SlippageCheck.validateMinOut(principalDelta, amount0Min, amount1Min).
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/43c9a5549e2b848453d63ca8246ed0db39b18c3b/contracts/evm/src/libraries/MMCalldataDecoder.sol)
...
+    // TODO(Security): Introduce a slippage-aware decoder variant that also decodes (amount0Min, amount1Min)
+    // for use in a new DECREASE_LIQUIDITY_WITH_SLIPPAGE action.
     ...
     /// @return positionIndex The position index within the commitment
+    // TODO(Security): Introduce a slippage-aware decoder variant that also decodes (amount0Min, amount1Min)
+    // for use in a new BURN_POSITION_WITH_SLIPPAGE action.
...
+    // NOTE(Security): For slippage-aware decrease/burn callers should validate min-out on
+    // (callerDelta - feesAccrued) immediately after this returns, mirroring Uniswap v4 periphery.
