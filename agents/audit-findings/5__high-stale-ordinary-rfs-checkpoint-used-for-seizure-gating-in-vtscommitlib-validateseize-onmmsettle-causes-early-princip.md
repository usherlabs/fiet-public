[High] Stale ordinary-RFS checkpoint used for seizure gating in VTSCommitLib.validateSeize/onMMSettle causes early principal seizure before grace elapses

# Description

Ordinary-RFS seizure gating reads a cached checkpoint (openMask/openSince*) without refreshing it when there is no stored commitment deficit. Because live RFS can close and later re-open without updating this checkpoint, a stale openSince* can make grace appear elapsed and allow a seizer to proceed. The subsequent seizure logic enforces only that RFS is open (not grace), enabling early liquidity seizure.

The ordinary-RFS grace gate for seizure relies solely on the cached checkpoint (s.positions[positionId].checkpoint) via [CheckpointLibrary.isSeizable](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/Checkpoint.sol#L45-L52). [VTSCommitLib.validateSeize only refreshes growth and re-checkpoints when a stored commitmentDeficit exists](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L591-L597); otherwise it [immediately calls isSeizable against the cached checkpoint](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSCommitLib.sol#L598-L598). [VTSOrchestrator.onMMSettle (isSeizing=true) also calls isSeizable](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L766-L768) without re-marking ordinary RFS. However, [settlePositionGrowths](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSPositionLib.sol#L638-L658) and [calcRFS](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSPositionLib.sol#L730-L748) can change pa.settled and pa.cumulativeDeficit (fully closing or later re-opening live RFS) without calling markCheckpoint, leaving openSince* and openMask stale across a full close→reopen cycle. If the cached openSince* is older than the current live RFS-open episode start, isSeizable can incorrectly conclude that grace has elapsed. After passing this gate, _executeMMSettleFromParams only checks that live RFS is open and then computes seizedLiquidityUnits in [_calcSeizure, which does not enforce grace](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L616-L625). As a result, an attacker can seize liquidity before the intended grace period for the current live RFS-open episode has elapsed.

# Severity

**Impact Explanation:** [High] This enables early, unintended seizure of user principal (liquidity units) before the grace period elapses in the current live RFS-open episode, causing direct material loss.

**Likelihood Explanation:** [Medium] Exploitation requires a realistic but non-trivial state: a stale ordinary checkpoint across a full close→reopen cycle of RFS and timing the attack before any ordinary re-mark occurs. This is plausible under normal operations but not guaranteed at all times.
...
### Scenario 1.
Early seizure via stale checkpoint: A prior ordinary checkpoint (openSince* = t0) remains cached. Live RFS later fully closes (via settlePositionGrowths/calcRFS) without a checkpoint mark, then re-opens at t1 > t0. Because the cached openSince* is still t0, isSeizable treats grace as elapsed even though the current live episode at t1 is within grace. Attacker passes the gate and, since live RFS is open, [_calcSeizure](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L616-L625) seizes liquidity immediately.
#### Preconditions / Assumptions
- (a). Position is MM-managed and active
- (b). An ordinary checkpoint exists with openMask != 0 and openSince* = t0
- (c). Live RFS fully closes via settlePositionGrowths/calcRFS without markCheckpoint being called (checkpoint remains stale)
- (d). Live RFS later re-opens at t1 > t0, but checkpoint still has openSince* = t0
- (e). Attacker calls onSeize and then [onMMSettle(isSeizing=true)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L766-L768) before anyone re-marks the ordinary checkpoint

### Scenario 2.
Amplified early seizure with partial deposit: After passing the stale checkpoint gate, the attacker deposits roughly half of the current live rfsDelta on a lane. In [_calcSeizure](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L616-L625), settleOfRfsBps ≈ 1 due to rounding up with the remainder as denominator, and exposureBps is floored by baseVTSRate. This yields significant seizedLiquidityUnits with a deposit smaller than the full RFS requirement.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1
- (b). Live RFS amount R > 0 on at least one lane; commitment maxima and baseVTSRate configured normally
- (c). Attacker chooses a deposit s ≈ R/2 + 1 on that lane after passing the stale-checkpoint seizure gate

### Scenario 3.
Early seizure on expired commit: Even if the commit’s VRL signal is expired (seizing allows requireLiveSignal=false), the attacker uses a stale ordinary checkpoint to pass grace and proceeds to seize as long as live RFS is open. The checkpoint is corrected only after settlement, but the early seizure has already removed principal.
#### Preconditions / Assumptions
- (a). All preconditions from Scenario 1
- (b). Commit’s VRL signal is expired; seizing path permits expired signals
- (c). Attacker performs onSeize and [onMMSettle(isSeizing=true)](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/VTSOrchestrator.sol#L766-L768) using the stale checkpoint while live RFS is open
...
+        } else {
+            // Refresh growth and ordinary (non-commitment) checkpoint so grace is evaluated against the
+            // current live RFS-open episode, preventing stale checkpoint timers from bypassing grace.
+            VTSPositionLib.settlePositionGrowths(s, ctx.poolManager, positionId);
+            _checkpointAfterGrowthSettled(s, ctx, commitId, false, positionId);
         }
