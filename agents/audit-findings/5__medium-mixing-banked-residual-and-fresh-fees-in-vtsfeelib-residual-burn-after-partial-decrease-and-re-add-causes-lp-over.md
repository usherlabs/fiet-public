[Medium] Mixing banked residual and fresh fees in VTSFeeLib residual burn after partial decrease and re-add causes LP over-slash and inflated protocolFeeAccrued

# Description

After a partial decrease, the PR banks removed-slice fees into [pendingResidualFeeBacking](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L552-L566) while preserving feeGrowthInsideLast for surviving liquidity. If liquidity is re-added before the residual burn is consumed, a residual-only burn [sums banked residual with fresh fees](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L274-L301) computed using the enlarged post-add liquidity and the old baseline. This double-counts the same pre-add interval, overstating feesBurn, inflating protocolFeeAccrued, and over-slashing the LP.

The PR introduced pendingResidualFeeBacking and [captureResidualFeeBackingOnPartialDecrease](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L598-L618) to bank historical fee backing for removed liquidity while intentionally leaving feeGrowthInsideLast unchanged for surviving liquidity. Later, when a residual-only burn is applied ([consumeResidualFeeBacking=true](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L630-L637)) and coverage-per-deficit index did not advance in that settle, VTSFeeLib._calculateFeesBurn computes [totalFees = freshFees + bankedFees](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L274-L301), where [freshFees = (fg - feeGrowthInsideLast) × positionLiquidity / Q128](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L257-L263) uses the current (potentially enlarged) liquidity, and bankedFees is the previously stored residual backing. Because feeGrowthInsideLast was not rebased on a non-zero-liquidity add, the same pre-add interval is counted twice—banked for the removed slice and again as fresh for the newly added liquidity—causing overstated feesBurn. This [inflates paPool.protocolFeeAccrued](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L368-L372) and increases the LP’s pending slash (feesShared/pendingFeeAdj). While coverage burn (fresh-only) often runs first and mitigates the mix, residual-only windows remain realistic, so the overcharge can materialize. Episode scoping and ofDelta/outflowFloor limit magnitude per-settle but do not remove the underlying attribution error.

# Severity

**Impact Explanation:** [Medium] The effect is a direct, material reduction in LP yield/fees due to overstated feesBurn (over-slash) and inflated protocolFeeAccrued; however, it does not affect principal or core invariants.

**Likelihood Explanation:** [Medium] Requires realistic but non-trivial alignment: a residual episode, partial decrease followed by re-add before residual burn, and a settle where only residual burn applies. These states and timings are plausible for active LPs.
...
### Scenario 1.
Partial decrease banks removed-slice fees, then the LP re-adds liquidity before residual burn; a later residual-only burn mixes banked and fresh fees and over-mints feesBurn against the LP.
#### Preconditions / Assumptions
- (a). Active position with non-zero liquidity.
- (b). Open residual episode on a deficit lane (pendingResidualBurnBase[token] > 0).
- (c). LP executes a partial decrease that captures pendingResidualFeeBacking while preserving feeGrowthInsideLast for survivors.
- (d). LP re-adds liquidity without reactivating from zero (no fee snapshot rebase on add).
- (e). Subsequent settlePositionGrowths applies residual-only burn (coveragePerDeficitIndexX128 unchanged in that step), triggering consumeResidualFeeBacking=true.

### Scenario 2.
The LP performs multiple partial decreases and re-adds within a single unresolved residual episode; a residual-only burn then mixes all accumulated banked residual with fresh from enlarged liquidity, amplifying the over-slash.
#### Preconditions / Assumptions
- (a). Same as Scenario 1.
- (b). LP repeats partial decreases and re-adds before any residual burn is applied, accumulating pendingResidualFeeBacking slices within the same episode.

### Scenario 3.
A third party times settlement after the LP re-adds and before any new coverage increment, so settlePositionGrowths applies a residual-only burn that mixes banked and fresh fees, over-slashing the LP.
#### Preconditions / Assumptions
- (a). Same base setup as Scenario 1 (partial decrease then re-add while episode is open).
- (b). An actor capable of triggering settle flows (e.g., via orchestrator paths) calls settlePositionGrowths when no coverage-per-deficit index change has occurred, producing a residual-only burn.
- (c). Residual episode remains open (pendingResidualBurnBase[token] > 0).

# Related findings

## [Medium] Residual fee-backing double-count in VTS residual handling during partial remove/add cycles causes over-slashing of LP fees

### Description

When a residual-burn episode is open, [partial decreases bank historical fee-backing using (fg − feeGrowthInsideLast)](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L538-L566) without advancing the fee baseline; [ordinary partial adds keep the old baseline](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1048-L1056) if the position remains active. A later partial decrease at the same fee growth level re-banks the same historical window on re-added liquidity, inflating residual-backed fees and causing over-slashing on subsequent residual burns.

The protocol introduces residual fee-backing accounting that [captures removedLiquidity × (fg − feeGrowthInsideLast) during partial decreases](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L538-L566) while a residual-burn episode is open. This capture intentionally [does not advance feeGrowthInsideLast for surviving liquidity](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L538-L566). [Ordinary partial adds do not rebase feeGrowthInsideLast unless the position is reactivated from zero liquidity](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1048-L1056). Consequently, a remove → add → remove sequence, with the position remaining active and fee growth unchanged (or with burns consuming banked fees first so feeGrowthInsideLast does not advance), allows the second partial decrease to bank the same historical (fg − last) window again for the re-added slice, even though those units did not exist during that interval. The inflated pendingResidualFeeBacking is then [treated as real fees during the residual burn path](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L262-L266), [increasing total fees used for fee burns](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L288-L296) and [materializing as positive fee adjustments (slashes)](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSFeeLib.sol#L403-L408) against the same position, reducing the LP’s payout. This is a correctness/fairness bug introduced by the PR’s residual-backing capture and consumption flow; it does not require malicious behavior and can affect users during normal rebalancing.

### Severity

**Impact Explanation:** [Medium] The misaccounting directly causes material loss of fees for affected LP/MM positions via over-slashing during residual burns, constituting a direct, material loss of yield/fees without principal theft or system breakage.

**Likelihood Explanation:** [Medium] Exploitation requires an open residual-burn episode, the position remaining active across partial add, and limited fresh-fee baseline advancement between operations—conditions that are plausible and expected at times (especially in quiet periods or when banked fees are consumed before fresh fees).
...
### Scenario 1.
An LP performs a [partial remove (banking residual backing)](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol#L1037-L1041), then immediately re-adds the same liquidity while the position remains active, and then partially removes again at the same fee growth level; the second remove re-banks the same historical window for the re-added liquidity, inflating residual-backed fees that are later burned, over-slashing the LP’s payout.
#### Preconditions / Assumptions
- (a). A residual-burn episode is open (pendingResidualBurnBase > 0) for a deficit token, so the opposite fee lane is needed.
- (b). The position remains active (no full deactivation) across the add; ordinary partial adds do not rebase feeGrowthInsideLast.
- (c). There is historical fee growth since the last baseline (fg − feeGrowthInsideLast > 0) at the time of the first partial remove.
- (d). Between the remove and the later remove, fee growth does not advance significantly or fresh-fee burns do not advance feeGrowthInsideLast (e.g., burns consume banked fees first).

### Scenario 2.
A market-maker repeatedly executes small partial remove/add micro-cycles during an open residual episode in a quiet market; each remove banks residual backing while subsequent adds do not rebase the baseline, accumulating inflated pendingResidualFeeBacking that later causes compounded over-slashing when burned.
#### Preconditions / Assumptions
- (a). A residual-burn episode is open and the position remains active throughout micro-cycles.
- (b). Quiet market conditions or residual burns consuming banked fees first prevent feeGrowthInsideLast from advancing between cycles.
- (c). Repeated partial remove/add sequences occur without full deactivation.

### Scenario 3.
An actor with two positions uses remove/add/remove on position A to inflate residual-backed fees and over-slash A, increasing the fee pot; later, position B with CISE exposure receives a bonus from the enlarged pot, enabling unintended value redistribution and fairness distortion.
#### Preconditions / Assumptions
- (a). A residual-burn episode is open; position A has (fg − feeGrowthInsideLast > 0) at the initial remove.
- (b). Position A remains active across remove/add/remove, accumulating inflated pendingResidualFeeBacking.
- (c). Position B has or will have CISE exposure to receive bonuses from the fee pot.
- (d). Timing allows residual burns to over-slash A before B’s bonus allocation.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/63e3be0475ac79858fc61f93a61cc6c359389d0a/contracts/evm/src/libraries/VTSPositionLib.sol)
...
+                    if (liveLiquidityBeforeAdd > 0) {
+                        PositionAccounting storage paRebase = s.positionAccounting[result.id];
+                        bool needFeeToken0 = paRebase.pendingResidualBurnBase.token1 > 0;
+                        bool needFeeToken1 = paRebase.pendingResidualBurnBase.token0 > 0;
+                        if (needFeeToken0 || needFeeToken1) {
+                            (uint256 fg0, uint256 fg1) =
+                                StateLibrary.getFeeGrowthInside(ctx.poolManager, poolId, posStorage.tickLower, posStorage.tickUpper);
+                            if (needFeeToken0) {
+                                paRebase.feeGrowthInsideLast.token0 = fg0;
+                            }
+                            if (needFeeToken1) paRebase.feeGrowthInsideLast.token1 = fg1;
+                        }
+                    }
                 } else {
