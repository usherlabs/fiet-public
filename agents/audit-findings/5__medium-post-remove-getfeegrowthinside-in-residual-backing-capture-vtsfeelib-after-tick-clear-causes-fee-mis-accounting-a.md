[Medium] Post-remove getFeeGrowthInside in residual-backing capture (VTSFeeLib) after tick clear causes fee mis-accounting and potential stuck removals

# Description

The PR-added residual fee-backing capture samples Uniswap v4 fee growth after modifyLiquidity may have cleared ticks. Calling [getFeeGrowthInside](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/lib/v4-periphery/lib/v4-core/src/libraries/StateLibrary.sol#L300-L329) post-remove can return incorrect or underflowed values, leading to over/under-banking of residual fee backing and, in some cases, reverts during full liquidity removals.

Uniswap v4 Pool.modifyLiquidity first computes position fees using the correct fee-growth-inside snapshot, then [clears tick data (clearTick)](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/lib/v4-periphery/lib/v4-core/src/libraries/Pool.sol#L196-L206) for any boundary that flips to uninitialized, and only then calls the hook’s afterModifyLiquidity. The PR introduced [VTSFeeLib._captureResidualFeeBackingOnDeactivation](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L530-L589), called from [VTSPositionLib.touchPosition](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSPositionLib.sol#L1210-L1215) when a position deactivates (initialLiquidity > 0 and live liquidity == 0). This function re-reads fee-growth-inside via [StateLibrary.getFeeGrowthInside](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/lib/v4-periphery/lib/v4-core/src/libraries/StateLibrary.sol#L300-L329) after modifyLiquidity returns. Because clearTick may have zeroed feeGrowthOutside on one or both boundaries, getFeeGrowthInside can produce incorrect values or underflow (unchecked subtraction), especially for out-of-range positions with partially cleared ticks. The code then computes backing = (fg - last) * liquidityBeforeRemove / Q128 and updates pa.pendingResidualFeeBacking and pa.feeGrowthInsideLast based on these incorrect values. In extreme cases, the huge wrapped value can cause [FullMath.mulDiv](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/lib/v4-periphery/lib/v4-core/src/libraries/FullMath.sol#L1-L20) or addition to overflow and revert, blocking full removals. Otherwise, over-/under-banked residual fees distort subsequent _applyBankedResidualBurn results (feesBurn), inflating or deflating [paPool.protocolFeeAccrued, pa.feesShared, and pa.pendingFeeAdj](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/src/libraries/VTSFeeLib.sol#L360-L370). This path is introduced by the PR (new residual-backing feature and post-remove capture), making the bug PR-specific.

# Severity

**Impact Explanation:** [Medium] Economic harm arises from incorrect fee accounting (material loss of yield/fees) rather than direct principal loss; the DoS on full removal is per-position and likely temporary.

**Likelihood Explanation:** [Medium] Requires plausible but specific AMM states: out-of-range or in-range at remove, pending residual episode, and partial or full tick clearing—conditions realistic in active pools.
...
### Scenario 1.
Full removal reverts (stuck funds): After a full remove, if one boundary tick is cleared while the position is out-of-range, [getFeeGrowthInside](https://github.com/usherlabs/fiet-protocol/blob/bf07b4bad49774e59e40121952fa929cf91648f3/contracts/evm/lib/v4-periphery/lib/v4-core/src/libraries/StateLibrary.sol#L300-L329) returns a wrapped (underflowed) inside value. Computing and adding backing with this value can revert, causing the entire remove transaction to fail.
#### Preconditions / Assumptions
- (a). Victim LP has a VTS-tracked position with liquidity > 0
- (b). Pending residual burn base active on the opposite lane (pa.pendingResidualBurnBase.get(oppositeTokenIndex) > 0)
- (c). Position is out-of-range at removal time
- (d). Only one boundary tick is cleared by the full removal (partial tick clearing)

### Scenario 2.
Over-banking residual fees → over-slashing the victim: With one boundary cleared but the position in-range (or similar non-underflow state), post-remove inside is inflated. The capture banks too much residual backing, and later _applyBankedResidualBurn over-slashes the victim by increasing protocolFeeAccrued and pendingFeeAdj more than warranted.
#### Preconditions / Assumptions
- (a). Victim LP has a VTS-tracked position
- (b). Pending residual burn base active on the opposite lane
- (c). State such that one boundary is cleared without triggering underflow (e.g., in-range at removal or equivalent)

### Scenario 3.
Under-banking residual fees → under-slashing (others harmed): If both boundaries are cleared (or similar state), post-remove inside can be too small or zero, causing missed residual-backing capture. Later, _applyBankedResidualBurn under-slashes this episode, underfunding the pot and reducing yield for other participants.
#### Preconditions / Assumptions
- (a). Victim LP has a VTS-tracked position
- (b). Pending residual burn base active on the opposite lane
- (c). Both boundary ticks are cleared by the full removal (or similar state causing near-zero post-remove inside)
...
+        (, uint256 fg0, uint256 fg1) =
+            StateLibrary.getPositionInfo(poolManager, pos.poolId, PositionId.unwrap(id));
