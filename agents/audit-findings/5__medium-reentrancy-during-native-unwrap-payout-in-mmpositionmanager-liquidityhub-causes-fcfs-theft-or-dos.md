[Medium] Reentrancy during native unwrap payout in MMPositionManager/LiquidityHub causes FCFS theft or DoS

# Description

[MMPositionManager forwards newly queued unwrap shortfalls based on (qAfter − qBefore)](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/MMPositionManager.sol#L438). For native-backed LCCs unwrapped to a payable locker, LiquidityHub [pays ETH via a low-level call](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/libraries/LiquidityHubLib.sol#L617), allowing the locker’s fallback to reenter and [annul its Hub queue](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/LiquidityHub.sol#L1090-L1101) mid-payout. This can make qAfter < qBefore (revert) or reduce the delta so MMPositionManager under-forwards, leaving stealable LCC on the router.

The PR added unwrap shortfall custody in MMPositionManager that [measures the beneficiary’s queued shortfall as (settleQueueAfter − settleQueueBefore) around LiquidityHub.unwrapTo](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/MMPositionManager.sol#L478), then forwards exactly that amount to a custodian. LiquidityHub now [queues shortfalls first and then pays underlying](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/LiquidityHub.sol#L626-L646). For native-backed LCCs, transferUnderlying [uses a low-level ETH call](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/libraries/LiquidityHubLib.sol#L617) to the payout recipient. If the recipient is the locker and is a payable contract, its fallback can execute during payout and transfer LCC to a protocol-bound address, which triggers [LiquidityHub.annulSettlementBeforeTransfer](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/LiquidityHub.sol#L1090-L1101) and reduces the same settleQueue key before MMPositionManager reads qAfter. If qAfter < qBefore, the subtraction underflows and unwrap reverts (DoS). If 0 < (qAfter − qBefore) < the newly queued shortfall, MMPositionManager under-forwards, leaving the remainder as LCC on the router. Any locker can then [SYNC](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/MMPositionManager.sol#L570-L578)/[TAKE](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/modules/PositionManagerEntrypoint.sol#L83-L97) that LCC (FCFS theft), under-custodying the victim’s queued claim and preventing full settlement until more LCC is forwarded.

# Severity

**Impact Explanation:** [High] Attacker can extract valuable LCC from the router that should have been forwarded to custody, under-custodying the victim’s queued claim and preventing full settlement until additional LCC is provided; this is a direct economic loss and degradation of redemption.

**Likelihood Explanation:** [Low] Requires multiple specific and uncommon, though realistic, conditions: native-backed LCC, payout to a payable locker, a fallback that actively transfers LCC to a protocol-bound address during payout, and availability of locker-held LCC at that instant.
...
### Scenario 1.
Partial under-forward: Victim unwraps native-backed LCC to a payable locker. LiquidityHub queues shortfall then pays ETH, triggering fallback that transfers LCC to a protocol-bound address. LiquidityHub annuls some of the victim’s queue mid-payout, so MMPositionManager forwards only a reduced delta. The un-forwarded LCC remains on MMPositionManager and is drained by a third party via SYNC/TAKE, under-custodying the victim’s queue.
#### Preconditions / Assumptions
- (a). LCC is native-backed (underlying == address(0))
- (b). MMPositionManager unwraps with payout to locker (to == locker)
- (c). Locker is a payable contract whose fallback transfers LCC to a protocol-bound address during payout
- (d). Locker holds some of the same LCC at payout time (more likely when payerIsUser=false)

### Scenario 2.
Underflow revert DoS: Victim unwraps native-backed LCC to a payable locker. The fallback transfers enough LCC to protocol-bound that annulment during payout exceeds the newly queued shortfall, making qAfter < qBefore. MMPositionManager’s (qAfter − qBefore) underflows and reverts, blocking the unwrap.
#### Preconditions / Assumptions
- (a). LCC is native-backed (underlying == address(0))
- (b). MMPositionManager unwraps with payout to locker (to == locker)
- (c). Locker is a payable contract whose fallback transfers a large LCC amount to a protocol-bound address during payout
- (d). Annulment during fallback exceeds the newly queued shortfall

### Scenario 3.
No pre-existing queue: Victim unwraps native-backed LCC with qBefore=0 to a payable locker. Fallback transfers LCC to protocol-bound during payout, partially annulling the newly queued shortfall. MMPositionManager forwards only the reduced delta, leaving leftover LCC on the router that a third party drains via SYNC/TAKE, again under-custodying the victim’s queued claim.
#### Preconditions / Assumptions
- (a). LCC is native-backed (underlying == address(0))
- (b). MMPositionManager unwraps with payout to locker (to == locker), qBefore=0
- (c). Locker is a payable contract whose fallback transfers LCC to a protocol-bound address during payout
- (d). Locker holds some of the same LCC at payout time
...
+        uint256 beforeBal = isNativeUnderlying ? payoutTo.balance : IERC20(underlying).balanceOf(payoutTo);
...
+            liquidityHub.unwrapTo(lccAddr, payoutTo, queueTo, toUnwrap);
...
+        uint256 afterBal = isNativeUnderlying ? payoutTo.balance : IERC20(underlying).balanceOf(payoutTo);
         unwrapped = afterBal - beforeBal;
 
-        if (to == address(this) && unwrapped > 0) {
+        if (payoutTo == address(this) && unwrapped > 0) {
...
+            liquidityHub.unwrapTo(lccAddr, payoutTo, payer, toUnwrap);
