[Low] Per-commit authorisedRelayer binding in MM validation causes dead-commit on canonical MM hook path

# Description

The PR binds each commit to the commit-creating caller (authorisedRelayer) and requires MM hook operations to be executed by that same bound address. Because the PR also preserves a direct orchestrator EOA commit path, commits created by an unbound EOA become permanently unusable on the canonical router-based MM path (owner != authorisedRelayer), resulting in a commit-level DoS.

This PR introduces Commit.authorisedRelayer and enforces it in VTSLifecycleLinkedLib.validateMMOperation so that, for MM hook operations, the caller (owner) must be MarketFactory-bound and equal to the commit’s stored authorisedRelayer when it is nonzero. At the same time, VTSCommitLib._resolveFreshCommitProofPrincipal preserves a direct-orchestrator path that permits an unbound caller if caller == signal.mmState.owner. When a commit is created through that EOA path, VTSCommitLib._commitSignalInternal stores authorisedRelayer = caller (the EOA). Later, MM hook-path operations run via the canonical bound router (e.g., MMPositionManager), making owner = router, which is not equal to the EOA relayer. validateMMOperation then reverts, preventing any mint/increase/decrease/seizure under that commit. Renewals do not rotate authorisedRelayer, so the dead-commit state persists for the commit’s lifetime. The only practical workaround is to re-create the commit via a bound router.

# Severity

**Impact Explanation:** [Low] This creates a localized authorization DoS for commits created via a non-canonical path. No principal loss, no invariant break, and a straightforward workaround exists (re-commit via the bound router).

**Likelihood Explanation:** [Low] The typical, intended flow is to create commits via the bound router. The preserved EOA path is a footgun and is unlikely to be used in production. The censorship scenario requires a deployment with multiple bound routers and adversarial behavior.
...
### Scenario 1.
An EOA (mmState.owner) creates a commit directly via VTSOrchestrator.commitSignal. authorisedRelayer is set to the EOA. Later, the user tries to mint/increase liquidity via the canonical router (MMPositionManager). validateMMOperation requires owner (router) == authorisedRelayer (EOA), which fails and reverts. The commit cannot be used on the MM hook path.
#### Preconditions / Assumptions
- (a). A market is deployed and the canonical MM router is MarketFactory-bound.
- (b). VRL handlers are registered and functioning.
- (c). The EOA caller is not MarketFactory-bound.
- (d). The EOA calls the orchestrator commit entrypoint directly (not via the router).

### Scenario 2.
An EOA (mmState.owner) creates a commit via VTSOrchestrator.commitSignalRelayed with valid EIP-712 authorization. authorisedRelayer is set to the EOA. Subsequent MM additions via the router fail for the same reason (owner != authorisedRelayer), leaving the commit unusable on the MM hook path.
#### Preconditions / Assumptions
- (a). A market is deployed and the canonical MM router is MarketFactory-bound.
- (b). VRL handlers are registered and functioning; EIP-712 relay auth is valid for the EOA owner.
- (c). The EOA caller is not MarketFactory-bound.
- (d). The EOA uses the relayed orchestrator commit entrypoint directly (not via the router).

### Scenario 3.
In a multi-router deployment, an adversarial bound router front-runs a victim’s intended commit creation, becoming authorisedRelayer for that commit. Any later MM operation by the intended router (owner = intended router) fails under validateMMOperation (owner != authorisedRelayer), causing persistent commit-level censorship. The victim must issue a new commit via their own router.
#### Preconditions / Assumptions
- (a). Multiple MarketFactory-bound routers exist (at least one adversarial/malicious or competing router).
- (b). VRL handlers are registered and functioning.
- (c). The adversarial router can submit the same MM liquidity signal first to bind authorisedRelayer to itself.
- (d). The intended router later attempts MM operations under that commit.
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/03e9e8a46d992ce5f3b5b3add6a13f9bc2565be6/contracts/evm/src/libraries/VTSCommitLib.sol)
