[Low] Removal of caller-binding for relayed renewals in VRLSignalManager/MMPositionManager causes front-run DoS of atomic renew+manage batches

# Description

Relayed renewals [recover to the signer](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/VRLSignalManager.sol#L170-L205) and [require sender == 0](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/VRLSignalManager.sol#L170-L205), while MMPositionManager’s relayed-renew path [does not bind msg.sender](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/MMPositionManager.sol#L320-L327), allowing any third party to front-run a valid advancer-signed renewal and cause a one-off revert of the victim’s atomic “renew + manage” batch.

VRLSignalManager.verifyLiquiditySignalRelayed for renewals (commitId != 0) now [recovers ECDSA to the signer](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/VRLSignalManager.sol#L170-L205) and [enforces sender == address(0)](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/VRLSignalManager.sol#L170-L205), removing caller-binding. MMPositionManager._renewSignal’s relayed branch [forwards the relay payload without checking msg.sender or NFT ownership](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/MMPositionManager.sol#L320-L327). As a result, any observer who sees a valid advancer-signed relayed renewal can submit it first via MMPositionManager → VTSOrchestrator → VRLSignalManager, [consuming submitAuthNonce[advancer]](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/VRLSignalManager.sol#L202-L208) and [advancing mmNonce[owner]](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/VRLSignalManager.sol#L143-L151). The maker’s original “renew + manage” batch then reverts on the renew step, preventing follow-on actions in that same transaction. Position operations remain protected by [authorisedRelayer binding and locker == advancer checks](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/libraries/VTSLifecycleLinkedLib.sol#L727-L734), so the attacker cannot operate or seize positions; the practical impact is loss of atomicity (a one-off DoS/grief). Makers can mitigate by using private relays or resubmitting manage-only after renewal has succeeded.

# Severity

**Impact Explanation:** [Low] The attacker causes a one-off loss of atomicity (bundle revert at renew) without enabling unauthorized position operations; strong authorization gates prevent direct funds loss or seizure. The victim can promptly re-submit manage-only after renewal has succeeded.

**Likelihood Explanation:** [Medium] Feasible under public mempool conditions with an attacker willing to spend gas for griefing. Commonly mitigated by private relays and deadlines, but still realistically possible when victims broadcast relayed payloads publicly.
...
### Scenario 1.
An attacker front-runs a publicly broadcast relayed renewal payload, calling MMPositionManager with only the renew action. VRLSignalManager verifies the signer and advances nonces. The victim’s atomic “renew + manage” transaction later reverts at the renew step, so manage actions do not execute in that same transaction.
#### Preconditions / Assumptions
- (a). A valid commit exists and requires renewal to continue non-seizing operations
- (b). The advancer signs an EIP-712 relayed renewal (sender == address(0)) with a valid nonce and deadline
- (c). The victim broadcasts a public transaction bundling RENEW_SIGNAL followed by position management actions
- (d). The attacker monitors the mempool and can submit transactions promptly
- (e). MMPositionManager is publicly callable and VRLSignalManager verifies signer-only (sender == 0 for renew)

### Scenario 2.
The victim intends to perform time-critical post-renew management (e.g., reduce/settle immediately). The attacker front-runs the relayed renewal, causing the victim’s bundle to revert at renew, potentially missing a narrow timing window before the victim resubmits a manage-only transaction. Position-operation gates still prevent adversarial seizure or unauthorized ops.
#### Preconditions / Assumptions
- (a). All preconditions of Scenario 1
- (b). The victim’s follow-on management is time-sensitive (tight window post-renew)
- (c). No private relay/bundling is used by the victim

### Scenario 3.
The advancer pre-signs multiple relayed operations with sequential nonces. The attacker front-runs the first renewal, consuming the nonce unexpectedly and causing subsequent pre-signed relayed operations to revert, resulting in operational delays until the victim re-orders or re-signs.
#### Preconditions / Assumptions
- (a). The advancer pre-signs multiple relayed operations with sequential submitAuthNonce values
- (b). At least two relayed payloads are publicly observable before execution
- (c). The attacker front-runs the earliest payload, consuming the expected nonce ordering
...
[Source](https://github.com/usherlabs/fiet-protocol/blob/7149455e77704bd8b9cca122d8a0b4f655f1f862/contracts/evm/src/MMPositionManager.sol)
...
+        LiquiditySignal memory signal = abi.decode(liquiditySignal, (LiquiditySignal));
+        // Idempotency: if the stored commit state already matches the incoming signal (same mmState and expiry),
+        // skip the renew to preserve atomicity of the batch (prevents front-run DoS on renew+manage).
+        {
+            (MarketMaker.State memory curState, uint256 curExpiresAt,,,) = vtsOrchestrator.getCommit(tokenId);
+            if (curExpiresAt == signal.mmState.expiryAt
+                && MarketMaker.toLeafHash(curState) == MarketMaker.toLeafHash(signal.mmState)) {
+                return;
+            }
+        }
         if (relayParams.length == 0) {
