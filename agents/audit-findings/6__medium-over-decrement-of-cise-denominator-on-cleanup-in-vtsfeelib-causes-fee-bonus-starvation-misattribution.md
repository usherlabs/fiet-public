[Medium] Over-decrement of CISE denominator on cleanup in VTSFeeLib causes fee bonus starvation/misattribution

# Description

When any non-zero bonus is allocated, [VTSFeeLib clears the position’s entire realised CISE exposure and subtracts that full amount from the pool-wide CISE denominator](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L112-L120). Because per-position exposure is realised lazily and the pool denominator is maintained eagerly and decremented by others’ allocations, a position’s exposure can exceed the live denominator. Cleanup then zeroes the denominator, starving other LPs and enabling disproportionate capture of the fee pot.

The PR introduces CISE-based bonus allocation with [eager pool-wide denominators (VTSCommitLib.incrementCoverage)](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSCommitLib.sol#L176-L186) and [lazy per-position exposure realisation (VTSPositionLib._settleCISEForToken)](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L706-L720). During allocation (VTSFeeLib._queueBonusForToken), [bonus = ceil(potAvail * ciseExposure / totalExposure), clamped to potAvail](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L85-L94). If any non-zero bonus is queued, VTSFeeLib._cleanupAfterAllocationForToken [clears the entire per-position ciseExposure and decrements the pool denominator by that full amount, clamping to zero when ciseExposure > denom](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L112-L120). [Only clear/decrement the windows if we actually queued a bonus](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L484-L490). Since other LPs’ prior allocations reduce the pool denominator independently of a given LP’s realised exposure, a position’s local ciseExposure can grow well beyond the remaining pool denominator. The subsequent cleanup over-decrements or zeroes the denominator, which prevents other LPs from receiving bonuses for that coverage window (starvation), and allows first-touch LPs with stale exposure to capture a disproportionate share of protocolFeeAccrued. This behavior and cleanup semantics are introduced by the CISE overhaul in this PR (VTSFeeLib and related CISE logic).

# Severity

**Impact Explanation:** [Medium] Other LPs suffer direct, material loss of yield/fees due to starvation and misattribution of protocolFeeAccrued bonuses for eligible exposure windows.

**Likelihood Explanation:** [Medium] Exploitation requires timing (being first to touch when potAvail > 0 and denom > 0), maintaining an active LP position to build exposure, and opportunistic monitoring of pool state—realistic but not trivial or guaranteed.
...
### Scenario 1.
Denominator-zero starvation with a tiny bonus: An attacker with large stale ciseExposure triggers a touch when potAvail is just above zero. A small bonus is allocated, but [cleanup subtracts the attacker’s full ciseExposure from the pool denominator (which may be smaller), clamping it to zero](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L112-L120) and starving other LPs from receiving bonuses for that window.
#### Preconditions / Assumptions
- (a). Fee sharing enabled (coverageFeeShare > 0) on the pool
- (b). Prior coverage increments created a positive pool CISE denominator
- (c). Attacker maintains an active LP position that has accrued large per-position ciseExposure (stale exposure)
- (d). Protocol fee pot (protocolFeeAccrued) has potAvail > 0 for the relevant token lane
- (e). Attacker can trigger a touch (e.g., [a no-op](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSPositionLib.sol#L1278-L1280) or minimal modify on an active position) to run VTS fee processing in normal (unpaused) operation

### Scenario 2.
Draining the pot using stale exposure versus a shrunken denominator: With potAvail large and the pool denominator small due to prior allocations, an attacker with large stale ciseExposure touches and receives the entire potAvail (after clamp). [Cleanup then zeroes the denominator](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L112-L120), preventing other LPs from allocating for that wedge.
#### Preconditions / Assumptions
- (a). Fee sharing enabled (coverageFeeShare > 0)
- (b). Pool denominator currently small due to prior allocations by others; attacker’s ciseExposure is large
- (c). Protocol fee pot has substantial potAvail
- (d). Attacker can trigger fee processing via a position touch

### Scenario 3.
Repeated denom resets to monopolize allocations: An attacker (or colluding LPs) lets per-position ciseExposure accumulate across coverage increments, then repeatedly touches first whenever denom > 0 and potAvail > 0 to allocate and immediately [zero the denom via cleanup](https://github.com/usherlabs/fiet-protocol/blob/8e58fda733b5f3c675e0e5375efd9f3bef5f0a5f/contracts/evm/src/libraries/VTSFeeLib.sol#L112-L120), persistently starving other LPs.
#### Preconditions / Assumptions
- (a). Fee sharing enabled (coverageFeeShare > 0)
- (b). Coverage increments and fee burns occur over time (normal operation)
- (c). Attacker(s) maintain active LP positions to accrue ciseExposure and monitor denom/potAvail states
- (d). Attacker(s) can consistently trigger touches first after denom rebuilds and potAvail becomes > 0
...
+        // WARNING: Cleanup currently subtracts the full per-position realised exposure from the pool-wide denominator.
+        // This can over-decrement (clamp to zero) when the position's exposure exceeds the remaining denominator,
+        // starving other LPs. Recommended fix: introduce a pool-wide CISE remaining-share factor + epoch,
+        // sync per-position banked exposure before allocation, and advance the factor here proportional to the
+        // exposure consumed (bounded by the pre-call denominator), rather than subtracting the full local exposure.
+        //
...
+        // TODO: Before reading ciseExposure*, sync per-position banked exposure against a pool-wide CISE
+        // remaining-share factor/epoch so previously consumed denominators proportionally shrink outstanding
+        // exposures for all positions. See CSI sync pattern for feesShared.
+        //
