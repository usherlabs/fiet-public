[Medium] Residual fee backing captured only on full deactivation in VTSPositionLib/VTSFeeLib enables staged-exit reduction of residual fee burns

# Description

Residual fee backing for unresolved residual-burn episodes is only preserved when a position fully deactivates (liquidity to zero). Partial decreases do not capture fee backing, so later residual-burn consumption uses reduced live liquidity and mints fewer feesBurn than intended, allowing LPs to lower their fee contributions by exiting in stages.

The protocol banks residual-burn base on a position when residual coverage is flushed (via DICE residual index) and the position next runs settlePositionGrowths. In that same call, an outflow floor is set so residual consumption cannot occur immediately. This PR adds a capture of unconsumed fee backing (pendingResidualFeeBacking) only when a position fully deactivates (liquidity transitions to zero). For partial decreases, no analogous capture occurs. Later, when residual burns are applied, VTSFeeLib._calculateFeesBurn uses totalFees = freshFees + bankedFees, where freshFees scales with the position’s current live liquidity and bankedFees is only populated by the full-deactivation capture. As a result, an LP can partially remove liquidity immediately after residual-base banking and before any consumption, ensuring future residual burns use reduced liquidity (smaller freshFees) with no bankedFees, thereby reducing feesBurn minted to the protocol pot compared to a one-shot full exit that captures fee backing.

# Severity

**Impact Explanation:** [Medium] Direct material reduction of feesBurn minted to the protocol fee pot and downstream distributions, affecting other participants’ fee shares.

**Likelihood Explanation:** [Medium] Requires realistic timing and state (removal after banking and before consumption for the same position, with RFS closed). This is uncommon but plausible and under the LP’s control.
...
### Scenario 1.
Partial removal immediately after residual-base banking: The LP removes part of their liquidity right after settlePositionGrowths banks pendingResidualBurnBase and sets the outflow watermark. No consumption occurs in that call. Because the removal is partial, no fee-backing capture happens; later residual-burn consumption uses reduced liquidity and yields smaller feesBurn.
#### Preconditions / Assumptions
- (a). Pool coverageFeeShare > 0
- (b). Residual coverage was previously flushed into coveragePerResidualDeficitIndexX128
- (c). The position had not run settlePositionGrowths since the flush (so banking occurs now)
- (d). RFS is closed for the position at removal time

### Scenario 2.
Multi-step staged exit across windows: The LP repeatedly performs partial removals after banking and before each residual-burn consumption window. Each subsequent consumption uses ever-smaller live liquidity for freshFees (with no bankedFees), cumulatively minimizing feesBurn minted to the pool pot.
#### Preconditions / Assumptions
- (a). Pool coverageFeeShare > 0
- (b). Residual coverage episodes occur with multiple future consumption windows
- (c). LP can time partial removals after banking and before consumption repeatedly
- (d). RFS remains closed for the position during these removals

### Scenario 3.
Opportunistic removal when others flushed residual: Residual coverage is flushed into the residual index by external activity. The LP’s first settlePositionGrowths (triggered by remove) banks residual base and sets the outflow floor; then the LP partially removes, avoiding capture. Subsequent consumption uses reduced liquidity with no bankedFees and thus mints smaller feesBurn.
#### Preconditions / Assumptions
- (a). Pool coverageFeeShare > 0
- (b). Residual coverage flushed by external actions before the LP’s own settlePositionGrowths
- (c). LP triggers remove-liquidity to cause banking (no consumption) and immediately partially removes
- (d). RFS is closed for the position at removal time
...
+                // FIXME: Residual fee-backing should also be captured proportionally on partial decreases
+                // when pendingResidualBurnBase > 0 to avoid staged-exit undercharging.
+                // Implement VTSFeeLinkedLib.captureResidualFeeBackingOnPartialDecrease(...) and invoke it here
+                // when (initialLiquidity > liq), using removedLiquidity = initialLiquidity - liq.
...
+    // TODO: Add captureResidualFeeBackingOnPartialDecrease that banks only the removed slice's fee backing
+    // and proportionally advances feeGrowthInsideLast for remaining liquidity to prevent double counting.
+    // Export via VTSFeeLinkedLib and call from VTSPositionLib.touchPosition decrease path when (initialLiquidity > liq).
+
