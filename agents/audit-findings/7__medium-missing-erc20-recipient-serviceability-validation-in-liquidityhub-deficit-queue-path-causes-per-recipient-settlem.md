[Medium] Missing ERC20 recipient serviceability validation in LiquidityHub deficit-queue path causes per-recipient settlement reverts

# Description

LiquidityHub’s issuer-driven deficit-queue path validates recipient shape and LCC backing but does not ensure the underlying ERC20 will accept transfers to that recipient. processSettlementFor pushes ERC20 to the queued recipient and will revert indefinitely if the ERC20 blocks transfers to that address, creating an uncleareable queue for that recipient. Affected recipients can still swap their LCC for a serviceable LCC and unwrap (e.g., to ETH/WETH), so principal is not permanently frozen.

In [LiquidityHub.queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LiquidityHub.sol#L814), [_assertQueueRecipientServiceable](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LiquidityHub.sol#L1128-L1153) enforces that the recipient is nonzero, not bucket-exempt (unless Hub), and holds sufficient market-derived LCC, and it adds a [native-only restriction](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LiquidityHub.sol#L1140) to avoid unserviceable ETH payouts to contracts. For ERC20-backed LCCs, it does not validate that the underlying token will transfer to the recipient. During settlement, [processSettlementFor delegates to LiquidityHubLib.processSettlementLogic](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LiquidityHub.sol#L1050), which burns the recipient’s market-derived LCC and then [pushes the underlying ERC20 via transferUnderlying](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/LiquidityHubLib.sol#L630). If the ERC20 rejects transfers to that recipient (e.g., due to blacklisting or recipient gating), the settlement reverts and the queue remains. There is no retarget or pull-claim mechanism; [annulSettlementBeforeTransfer](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/LCC.sol#L229) can only cancel the queue (forfeiting the claim) when the user moves LCC to a protocol endpoint. However, recipients can still realize value by swapping their LCC on the core pool into another LCC (e.g., native-backed) and unwrapping to ETH/WETH, preventing permanent loss of principal.

# Severity

**Impact Explanation:** [Medium] Direct redemption to the queued recipient can be indefinitely blocked for that ERC20, disrupting settlement for that recipient. However, recipients can still realize value by swapping their LCC into another LCC (e.g., native-backed) and unwrapping, so principal is not permanently frozen.

**Likelihood Explanation:** [Medium] Requires an ERC20 with blacklisting/recipient-gating and a recipient that is or becomes disallowed; this is plausible with regulated tokens but not universal across all deployments.
...
### Scenario 1.
User EOA receives deficit LCC and a queued claim; the underlying ERC20 later blacklists that EOA, so processSettlementFor to that EOA reverts indefinitely, forcing the user to swap LCC to another LCC and unwrap instead of redeeming the queued claim directly.
#### Preconditions / Assumptions
- (a). Market uses an ERC20 underlying that can blacklist or gate recipients
- (b). Recipient is the user EOA (default locker resolution)
- (c). Recipient holds sufficient market-derived LCC after deficit LCC transfer
- (d). [queueForTransferRecipient is called to create the queue](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/ProxyHook.sol#L286)
- (e). The ERC20 blacklists or otherwise disallows transfers to that EOA (before or after queue creation)

### Scenario 2.
A router contract is set as the deficit recipient; the underlying ERC20 blocks transfers to that contract, so processSettlementFor(router) reverts; the router must swap its LCC into a serviceable LCC and unwrap rather than redeem the queue.
#### Preconditions / Assumptions
- (a). Market uses an ERC20 underlying that blocks transfers to certain contracts
- (b). ProxyHook resolves the deficit recipient as the router contract
- (c). Router holds sufficient market-derived LCC after the transfer and is queued via [queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/ProxyHook.sol#L286)
- (d). ERC20 rejects transfers to the router contract at settlement time

### Scenario 3.
An attacker directs deficit LCC and a queued claim to a known-blacklisted EOA; settlement to that EOA reverts, but the EOA can still swap LCC to another LCC and unwrap, so no direct loss occurs though direct redemption is blocked.
#### Preconditions / Assumptions
- (a). Market uses an ERC20 underlying that blacklists a known EOA
- (b). Attacker crafts a swap that causes a deficit and specifies the blacklisted EOA as recipient
- (c). Recipient receives deficit LCC and is queued via [queueForTransferRecipient](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/ProxyHook.sol#L286)
- (d). ERC20 rejects transfers to the blacklisted EOA at settlement time

# Proposed fix

## LiquidityHubLinkedLib.sol

File: `contracts/evm/src/libraries/LiquidityHubLinkedLib.sol`

[Source](https://github.com/usherlabs/fiet-protocol/blob/8cefc80d0a77f70260d5024395fd7d64d8747a8f/contracts/evm/src/libraries/LiquidityHubLinkedLib.sol)
...
+    // Mitigation entrypoint (blueprint):
+    // function processSettlementLogicTo(
+    //     LiquidityHubStorage storage s, address lcc, address recipient, address to, uint256 maxAmount
+    // ) external {
+    //     LiquidityHubLib.processSettlementLogicTo(s, lcc, recipient, to, maxAmount);
+    // }
...
+    // Mitigation helper (blueprint):
+    // function processSettlementLogicTo(
+    //     LiquidityHubStorage storage s, address lcc, address recipient, address to, uint256 maxAmount
+    // ) internal {
+    //     // Same caps and queue/reserve updates as `processSettlementLogic`, but for the external path call:
+    //     // pay(s, lcc, recipient, to, 0, toSettle);
+    // }
