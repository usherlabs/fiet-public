[Low] Unguarded checkpoint during pause in VTSOrchestrator and paused-remove routing through touchPosition causes DoS of MM remove-liquidity

# Description

After the PR, CoreHook routes remove-liquidity through VTS even while paused. Meanwhile, VTSOrchestrator.checkpoint remains publicly callable during pause and can set commitmentDeficit and open RFS. VTSPositionLib.touchPosition blocks non-seizure MM liquidity changes when commitmentDeficit is non-zero or RFS is open. A third party can front-run an MM’s paused remove with checkpoint(..., true) to force a revert, whereas pre-PR the paused remove would have bypassed VTS and succeeded.

CoreHook._afterRemoveLiquidity no longer short-circuits when paused and always calls vtsOrchestrator.processPosition. Inside VTSPositionLib.touchPosition, pause policy allows negative liquidityDelta (removes), but adds two guards: (1) if the operation is MM, not seizing, liquidityDelta != 0, and pa.commitmentDeficit.{token0,token1} > 0, it reverts Errors.CommitmentDeficitBlocksLiquidityChange; (2) in _touchExistingDecrease, if RFS is open, it reverts Errors.RFSOpenForPosition. VTSOrchestrator.checkpoint(commitId, positionIndex, withCommitment) is public and not pause-gated; it calls VTSLifecycleLinkedLib.checkpoint, which runs VTSPositionLib.settlePositionGrowths (library path, no pause guard) and VTSCommitLib.checkpointWithCommitment (setting commitmentDeficit, treating expired signals as zero backing). Therefore, during pause, any third party can front-run an MM’s remove-liquidity with checkpoint(..., true) to materialise commitmentDeficit and/or open RFS, causing the now-canonical paused remove to revert. Before the PR, paused removes bypassed VTS and would have succeeded.

# Severity

**Impact Explanation:** [Medium] This creates a significant but temporary DoS of a core operation (MM remove-liquidity) during pause. There is a workaround (deposit to cure deficits or wait to unpause), and no direct funds loss or invariants broken.

**Likelihood Explanation:** [Low] Exploitation requires the pool to be in a paused state (admin-controlled, episodic) and is pure griefing without direct profit. Although the call is cheap and public, the combination of pause precondition and unprofitable intent reduces likelihood.
...
### Scenario 1.
During a pause, a third party front-runs an MM’s pending DECREASE_LIQUIDITY by calling checkpoint(commitId, positionIndex, true) on an expired commit; signalUsd=0 causes commitmentDeficit to be stored; the MM’s paused remove routes through touchPosition and reverts with CommitmentDeficitBlocksLiquidityChange, blocking emergency deleveraging.
#### Preconditions / Assumptions
- (a). Pool or global pause is active
- (b). An active MM position exists and is linked to the specified commitId
- (c). The commit is expired (signal treated as zero)
- (d). RFS is initially closed
- (e). The MM submits a DECREASE_LIQUIDITY (negative liquidityDelta) during the pause
- (f). Attacker can send a public checkpoint(commitId, positionIndex, true) before the MM’s tx is mined

### Scenario 2.
During a pause with a live but under-backed commit (issuedUsd > settledUsd + signalUsd), a third party front-runs with checkpoint(commitId, positionIndex, true) to materialise a proportional commitmentDeficit; the MM’s paused remove then reverts on the insolvency freeze in touchPosition.
#### Preconditions / Assumptions
- (a). Pool or global pause is active
- (b). An active MM position exists and is linked to the specified commitId
- (c). Commit is live but issuedUsd exceeds settledUsd + signalUsd (insufficient backing)
- (d). RFS is initially closed
- (e). The MM submits a DECREASE_LIQUIDITY during the pause
- (f). Attacker can send a public checkpoint(commitId, positionIndex, true) before the MM’s tx is mined

### Scenario 3.
During a pause, a third party front-runs with checkpoint(commitId, positionIndex, true) to open RFS lanes from the same snapshot; touchPosition._touchExistingDecrease detects rfsOpen and reverts with Errors.RFSOpenForPosition, blocking the remove even without relying on the insolvency freeze.
#### Preconditions / Assumptions
- (a). Pool or global pause is active
- (b). An active MM position exists and is linked to the specified commitId
- (c). RFS is initially closed
- (d). The MM submits a DECREASE_LIQUIDITY during the pause
- (e). Attacker can send a public checkpoint(commitId, positionIndex, true) before the MM’s tx is mined
...
```diff
     ...
     ReentrancyGuardTransient
 {
     ...
     {
+        if (s.isPaused || s.pools[s.positions[positionId].poolId].isPaused) revert Errors.EnforcedPause();
