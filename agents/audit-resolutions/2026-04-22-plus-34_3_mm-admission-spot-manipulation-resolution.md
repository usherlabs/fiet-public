...
**Date:** 2026-04-22 (UTC)  
**Last updated:** 2026-04-23 (API: MM-increase `validateMmIncreaseLiquidityDelta` renamed to the second `validateLiquidityDelta` overload; test names + resolution text aligned)  
...
**Finding `34_3` (low):** endpoint-max admission alone could admit a post-add total that was still economically consistent with `(settled + signal)` while **spot-shaped two-leg minting** (rounding, repeated small adds) issued LCC principal whose oracle-valued increment exceeded the **incremental** endpoint-max admission budget. That class is now blocked at the pre-mint gate by the **MM-increase** overload of `validateLiquidityDelta` (extra `preAddLiquidity` and mint-amount args; reverts with `Errors.InvalidAdmissionMintDelta` when the marginal inequality fails). An adversarial **full-path** Foundry test exercises a real `modifyLiquidities` increase and asserts that revert is observed end-to-end (see “Test coverage”).
...
| Admission gate (global + marginal on MM increases) | `contracts/evm/src/libraries/VTSCommitLib.sol` — second `validateLiquidityDelta` overload (`preAddLiquidity`, mint args; includes marginal mint check); use the first `validateLiquidityDelta` overload for **global** COMMIT-01 only |
| Marginal mint USD helper | `contracts/evm/src/libraries/VTSCommitLib.sol` — `_valueFromLiquidityDelta` |
...
| MM increase validation (stack-isolated) | `contracts/evm/src/libraries/VTSPositionMMOpsLib.sol` — `_validateMmIncreaseCommitBacking` → `VTSCommitLib.validateLiquidityDelta` (MM-increase overload, `revertIfInsufficientBacking=true`) |
...
| Test-only replay on live `s` | `contracts/evm/test/base/VTSOrchestratorTestable.sol` — `exposedvalidateLiquidityDeltaSoft` |
...
- **`test_validateLiquidityDelta_mmIncrease_*`** — marginal gate: revert when oracle-valued mint exceeds `issuedAdmission(post) - issuedAdmission(pre)`; pass for honest mint under marginal budget; admission still invariant to `slot0` fields; global failure still surfaces as `InvalidLiquiditySignal` first.
...
- **`test_mmIncrease_e2e_actual_mint_replays_validateLiquidityDelta_on_live_storage`** — after a real increase, measures **actual** LCC balance deltas on the batch locker and replays the **MM-increase** `validateLiquidityDelta` overload on **live** orchestrator storage (`exposedvalidateLiquidityDeltaSoft`); asserts success (global + marginal satisfied with real mint amounts).
...
| Is finding `34_3` (endpoint-max vs spot mint / rounding drift) addressed? | **Yes**, via the MM-increase `validateLiquidityDelta` overload + MM increase wiring; adversarial E2E test asserts marginal revert on full `modifyLiquidities` path. |
