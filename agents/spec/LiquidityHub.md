...
Direct holders use `unwrap(...)` / `unwrap(underlying, marketId, ...)`, which always queue shortfalls to the caller. Every `unwrapTo(...)` overload is **endpoint-only**: the caller must be `BOUND_ENDPOINT` in the LCC’s market factory namespace (`boundLevelOfLcc(lcc, msg.sender) == BOUND_ENDPOINT`). That keeps split payout vs queue-owner flows (for example `MMPositionManager` after consuming the beneficiary’s LCC or delta credit) on trusted protocol surfaces and prevents arbitrary EOAs from repeatedly inflating shared queue debt.
