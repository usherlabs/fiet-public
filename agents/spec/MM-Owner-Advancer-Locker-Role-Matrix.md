...
**`MMPositionManager` direct renew (no relay params)** additionally requires the batch **locker** (`msgSender()` on `MMPositionManager`, i.e. the unlock-session locker) to equal **`signal.mmState.advancer`**. That matches ordinary non-seizing MM ops (`locker == advancer`) and prevents an arbitrary party from routing a captured signal through the canonical bound router while impersonating the decoded advancer at the VRL layer. **Relayed renew** keeps EIP-712 / relay auth on the advancer path; lower-level `VTSOrchestrator.renewSignal` may still be used by other factory-bound integrations that forward the decoded advancer per `VTSCommitLib._resolveRenewProofPrincipal`, which is a separate policy surface from `MMPositionManager`.

This is a feature: anyone who can produce a valid renewed signal attesting to the same `mmState.owner` may rotate the stored `advancer` to the address named in that renewed proof, provided the renew path authenticates as that same named advancer at the VRL layer **and** (for direct renew through `MMPositionManager`) the batch locker is that advancer.
...
| Locker | Address encoded into MM hook data / queue attribution | Through `MMPM`, commit-time locker receives the NFT; on direct commit it must equal `owner`, on relayed commit it may differ | Direct renew through `MMPM`: batch locker must equal **`signal.mmState.advancer`**; orchestrator still derives VRL principal from the signal (`VTSCommitLib`). Relayed renew follows EIP-712 relay rules | For non-seizing MM ops, locker must equal stored advancer | For seizure, locker is the seizer | Locker semantics differ between direct commit, relayed commit, ordinary MM ops, and seizure |
| NFT owner | ERC-721 holder in `MMPositionManager` | Receives NFT on commit through `MMPM`; may later transfer it | Not sufficient for **direct** `MMPM` renew if holder is not the advancer (batch locker); NFT-gated actions remain separate | Usually required, or must approve an operator, for ordinary position-management actions | Must **not** be the seizer | NFT custody and advancer authority can diverge after transfer |
| Approved operator | ERC-721-approved operator for the commitment NFT | N/A | Same as NFT owner for direct `MMPM` renew: advancer locker requirement applies | Can perform most NFT-gated normal position actions | Cannot use the seizure path | Same ordinary operational power surface as owner for token-gated actions |
...
- **feature**: anyone who can produce a valid renewed proof of solvency for the same `mmState.owner`, and authenticate at the VRL layer as the `advancer` named in that renewed proof, can advance the proof state and name the next ordinary MM operator; on **direct** renew through `MMPositionManager`, that identity must also be the batch locker (`locker == advancer`)
