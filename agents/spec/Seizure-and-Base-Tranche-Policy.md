...
On-chain sizing uses `VTSLifecycleLinkedLib._calcSeizure` with a **pre-deposit** `BalanceDelta` snapshot of RfS (`R_pre`) passed in from the MM settle path (same ordering as `getRFS`). That snapshot reasons over **effective** settled (`live settled + settledOverflow`) per lane alongside deficits. Per lane, cured amount is `S_eff = min(S_lane, R_pre)` and seized liquidity units are `floor(L · inner / denom)` from a **piecewise rational** (commitment zero, overdue vs cap, base binds, or proportional exposure) with **Q128 fractional carry** in `SeizureCarryQ128Lib` (`contracts/evm/src/libraries/SeizureCarryQ128Lib.sol`). Carry in `PositionAccounting.seizureLiquidityCarry` resets when the position reaches **zero** live liquidity (`VTSPositionLib._trackCommitment`), not on ordinary commitment refreshes while liquidity stays positive. `LiquidityUtils.exposureBps` / `settleOfRfsBps` / `seizedUnitsFromBps` are **not** used for seizure sizing. A **full RfS close** in the same transaction can still yield a policy-consistent non-zero seizure when the snapshot showed overdue amounts to cure. Residual-threshold promotion and position-wide caps match `INVARIANTS.md` (**SETTLE-02**, **SEIZE-01**).
