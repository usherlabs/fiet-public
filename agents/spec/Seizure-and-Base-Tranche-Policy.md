...
On-chain sizing uses `VTSLifecycleLinkedLib._calcSeizure` with a **pre-deposit** `BalanceDelta` snapshot of RfS (`R_pre`) passed in from the MM settle path (same ordering as `getRFS`). That snapshot reasons over **effective** settled (`live settled + settledOverflow`) per lane alongside deficits. Per lane, cured amount is `S_eff = min(S_lane, R_pre)` and seized liquidity units are `floor(L · inner / denom)` from a **piecewise rational** (commitment zero, overdue vs cap, base binds, or proportional exposure) with **Q128 fractional carry** in `SeizureCarryQ128Lib` (`contracts/evm/src/libraries/SeizureCarryQ128Lib.sol`). Carry supports path-independent rounding across **split cures while a lane stays overdue**; after each **seizing** MM settle, `VTSLifecycleLinkedLib` clears per-lane carry when that lane’s **post-settlement** `getRFS` delta is no longer a positive open requirement, so remainder does not roll into a later distinct RfS episode. All carry is also cleared when live liquidity reaches zero (`VTSPositionLib._trackCommitment`). Non-seizing commitment refreshes while liquidity stays positive preserve carry for the same overdue shape. `LiquidityUtils.exposureBps` / `settleOfRfsBps` / `seizedUnitsFromBps` are **not** used for seizure sizing. A **full RfS close** in the same transaction can still yield a policy-consistent non-zero seizure when the snapshot showed overdue amounts to cure. Residual-threshold promotion and position-wide caps match `INVARIANTS.md` (**SETTLE-02**, **SEIZE-01**).
...
| 2026-04-21 | Implementation note: seizure sizing uses `SeizureCarryQ128Lib` piecewise rational + carry; not `LiquidityUtils` bps product helpers. |
| 2026-04-22 | Implementation note: per-lane seizure carry cleared on post-settlement RfS lane close after seizing settle; terminal zero-liquidity clear retained. |
