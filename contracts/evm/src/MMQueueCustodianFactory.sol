...
contract MMQueueCustodianFactory is IMMQueueCustodianFactory {
    /// @notice Emitted when a new `MMQueueCustodian` is deployed for `recipient` by the bound `positionManager`.
    event QueueCustodianDeployed(address indexed positionManager, address indexed recipient, address custodian);
    ...
    function deploy(address recipient, IMarketFactory marketFactory) external returns (address custodian) {
        ...
        emit QueueCustodianDeployed(msg.sender, recipient, custodian);
    }
}
