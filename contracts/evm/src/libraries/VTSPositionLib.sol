...
library VTSPositionLib {
    ...
    function touchPosition(
        ...
        if (posStorage.owner == address(0)) {
            // NEW POSITION
            requiredSettlementDelta = _touchNewPosition(s, ctx.poolManager, poolId, p.owner, p.params, result.id, hookData);
        ...
        result.feeAdj = VTSFeeLib.processPositionFees(s, ctx.poolManager, result.id, p.poolKey.currency0, p.poolKey.currency1);
    ...
    function _processMMOperations(
        ...
        if (p.params.liquidityDelta > 0) {
            ...
            _handleLiquidityIncrease(s, ctx, p.owner, p.poolKey, mmCommitId, result.id, p.params, principalDelta);
        } else if (p.params.liquidityDelta < 0) {
    ...
    function onMMSettle(
        VTSStorage storage s,
        IPoolManager poolManager,
        MMSettleParams calldata p
    ) external returns (MMSettleResult memory result) {
        ...
        if (!pos.isActive) {
            ...
            (amount0, amount1) = _settleSeizing(s, p.positionId, amount0, amount1, rfsDelta, positionRequiredSettlementDelta);
        } else {
        ...
        result.settlementDelta = LiquidityUtils.negateBalanceDelta(toBalanceDelta(amount0.toInt128(), amount1.toInt128()));
