...
contract BatchProcessSettlementTest is Test {
    ...
    bytes32 internal constant SETTLEMENT_FAILED_TOPIC = keccak256("SettlementFailed(address,address,uint256,bytes)");
    ...
    function test_process_revertsOnRecipientLengthMismatch() public {
        ...
        harness.process(lcc, recipient, maxAmount);
    }
    ...
    function test_process_revertsWhenBatchTooLarge() public {
        (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount) = _buildBatch(31);
    ...
    function test_process_maxBatchSizeAllowed_callsAllAndEmitsSuccesses() public {
        (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount) = _buildBatch(30);
    ...
    function test_process_allSuccess_emitsBatchThenPerItemSuccessesWithArgs() public {
        (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount) = _buildBatch(3);
        Vm.Log[] memory logs = _recordAndProcess(lcc, recipient, maxAmount);
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            assertEq(abi.decode(entry.data, (uint256)), maxAmount[i], "maxAmount mismatch");
        }
    }
    ...
    function test_process_mixedOutcomes_emitsExpectedSuccessAndFailureReasons() public {
        (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount) = _buildBatch(4);
        ...
        _assertFailedLog(
            logs[3], lcc[2], recipient[2], maxAmount[2], abi.encodeWithSignature("Error(string)", "mock-string")
        );

        // i=3 => success
        _assertSucceededLog(logs[4], lcc[3], recipient[3], maxAmount[3]);
    }
    ...
    function test_process_catchesRevertWithoutData_reasonIsEmpty() public {
        (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount) = _buildBatch(1);
        mockHub.setBehaviour(lcc[0], recipient[0], maxAmount[0], MockLiquidityHub.Behaviour.RevertNoData);

        Vm.Log[] memory logs = _recordAndProcess(lcc, recipient, maxAmount);
        assertEq(logs.length, 2);

        (uint256 loggedAmount, bytes memory reason) = abi.decode(logs[1].data, (uint256, bytes));
    ...
    function testFuzz_process_countGate(uint256 n) public {
        n = bound(n, 0, 64);
        (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount) = _buildBatch(n);
    ...
    function _recordAndProcess(address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount)
        internal
    ...
    function _buildBatch(uint256 n)
        internal
        pure
        returns (address[] memory lcc, address[] memory recipient, uint256[] memory maxAmount)
    {
        lcc = new address[](n);
    ...
    function _assertSucceededLog(Vm.Log memory entry, address lcc, address recipient, uint256 maxAmount) internal pure {
        assertEq(entry.topics[0], SETTLEMENT_SUCCEEDED_TOPIC, "unexpected topic");
        ...
        assertEq(abi.decode(entry.data, (uint256)), maxAmount, "maxAmount mismatch");
    }
    ...
    function _assertFailedLog(
        ...
        (uint256 loggedAmount, bytes memory reason) = abi.decode(entry.data, (uint256, bytes));
