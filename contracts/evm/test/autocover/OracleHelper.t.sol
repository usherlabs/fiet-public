...
contract OracleHelperTest_Autocover is Test, OlympixUnitTest("OracleHelper") {
    ...
    function test_validateMarketOracles_reverts_when_mainDisabledOrAssetZero() public {
        ...
        vm.mockCall(
            address(this),
            abi.encodeWithSelector(IResilientOracle.getTokenConfig.selector, asset0),
            abi.encode(tc0)
        );
        vm.mockCall(
            address(this),
            abi.encodeWithSelector(IResilientOracle.getTokenConfig.selector, asset1),
            abi.encode(tc1)
        );
    ...
    function test_getPriceForLcc_branch_true() public {
        ...
        vm.mockCall(
            resilientOracle,
            abi.encodeWithSelector(IResilientOracle.getPrice.selector, asset),
            abi.encode(1234e18)
        );
