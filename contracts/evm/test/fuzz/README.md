...
| **MKT-05**                  | **P1**   | Yes             | **Covered**           | `invariants/MKT05.sol` + `test/ProxyHook.t.sol::{testFuzz_swap_exactOutput_*_revertsWhenRequestedExceedsImmediateLiquidity,test_proxySwap_exactInput_keepsProxySlot0Unchanged}` | Residual-cancellation, strict exact-output revert hardening, and no-residual proxy-slot0 side-path checks. |
| **SETTLE-02**               | **P2**   | Yes             | **Covered**           | `invariants/SETTLE02.sol` → `echidna_settle_02_seizing_clamps_hold`                                                                             | Real `onMMSettle -> _settleSeizing` path; below/equal/above-bound requests settle exactly to clamp math. |
...
| SETTLE-02               | `SETTLE02.sol`           | `echidna_settle_02_seizing_clamps_hold` |
