...
| **MKT-05**                  | **P1**   | Yes             | **Covered**           | `ProxySwapMKT05EchidnaTest.sol` → `echidna_mkt05_amountToSwap_is_zero`                                                                           | Proxy pool AMM curve must never be utilised; prevent proxy `slot0` drift/DoS and enforce “core-only curve”. |
