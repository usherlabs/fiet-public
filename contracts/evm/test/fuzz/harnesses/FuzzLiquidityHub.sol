...
import {LiquidityHubLib} from "../../../src/libraries/LiquidityHubLib.sol";
import {LiquidityHubStorage, Market, UnderlyingReserve} from "../../../src/types/Liquidity.sol";
...
contract FuzzLiquidityHub is BoundRegistry, Ownable, ReentrancyGuardTransient {
    ...
    function _wrapWith(address lcc, address withLCC, address to, uint256 amount) internal onlyValidLcc(lcc) {
        ...
        ctx = LiquidityHubLib.wrapWithContext(s, lcc, withLCC, ctx);
    ...
    function confirmTake(address lcc, uint256 amount, bool shouldEmit) external onlyIssuer(lcc) {
        ...
        LiquidityHubLib.ConfirmTakeContext memory ctx = LiquidityHubLib.confirmTakePrepare(s, lcc, amount, shouldEmit);
        ...
        LiquidityHubLib.confirmTakeBalanceInvariant(s, ctx.underlying);
    }
    ...
    function prepareSettle(address lcc, uint256 amount) external onlyIssuer(lcc) nonReentrant {
        LiquidityHubLib.prepareSettle(s, lcc, amount, _msgSender());
    }
    ...
    function annulSettlementBeforeTransfer(
        ...
        uint256 toAnnul = LiquidityHubLib.annulSettlementBeforeTransfer(
