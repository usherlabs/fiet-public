...
/// @notice Echidna harness for the MM queue custody guard: when `tokenId > 0`, forwarded non-fee LCC (`nonFee`) must
///         cover the Hub-queued principal slice (`qCommitted == custodyForward`) or `InsufficientBalance` reverts.
/// @dev Mirrors `PositionManagerImpl._routeLccCustodyTakeAndForward` via `PositionManagerImplQueueCustodyHarness`.
///      See `agents/audit-resolutions/mm-queue-custody-nonfee-vs-custodyforward-guard-resolution.md`.
contract MMQ01 {
    ...
    constructor() {
        ...
        // Fund the harness for ERC20 forwards to the custodian.
        lcc.mint(address(harness), type(uint128).max);
    }

    /// @notice Valid region: `nonFee >= qCommitted` when `tokenId > 0` and `qCommitted > 0`; `nonFee` from
    ///         `LiquidityUtils.forwardedNonFeeLccAmount` with non-negative slack so the guard never fires.
    ...
    function action_valid_commit_custody_guard_holds(
        ...
        // `nonFee = inc - fee` path: choose `inc` so forwarded non-fee clears `qCommitted` with slack.
        ...
        if (qCommitted > 0 && nonFee < qCommitted) {
            // Construction error for this seed — not a protocol failure; skip without poisoning `validAllOk`.
            return;
        }
    ...
    /// @notice Invalid region: force `nonFee < qCommitted` with `tokenId > 0` and `qCommitted > 0` — must revert with
    ///         `InsufficientBalance`.
    // forge-lint: disable-next-line(mixed-case-function)
    ...
    /// @notice Custodian slice increases by exactly `qCommitted` on successful commit-leg forwards (`tokenId > 0`).
    // forge-lint: disable-next-line(mixed-case-function)
