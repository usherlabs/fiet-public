...
Use deterministic local simulation as the default development and CI validation lane. `forge test` runs Foundry-only coverage against the single `HubRSC` runtime by simulating Reactive VM ingress with direct `react()` calls and mocks. It is supporting coverage, not the full Lasna pseudo-e2e proof, and must not require live Reactive Network RPCs, `REACTIVE_CI_PRIVATE_KEY`, or any other live secret.
