...
- Use an lREACT-funded live key only for this lane, for example `REACTIVE_CI_PRIVATE_KEY` from GitHub Actions secrets or Vault. PR live smoke preflights the signer's native Lasna gas balance and skips with a notice when the key is underfunded or cannot be queried; manual `workflow_dispatch` remains strict.
