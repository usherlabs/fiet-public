...
main() {
  ...
  local queue_already_decreased="false"
  if dec_lt "$current_queue" "$QUEUED_AFTER_SWAP"; then
    echo "Queue already decreased below queuedAfterSwap baseline; still settling maker RFS before close."
    QUEUED_FINAL="$current_queue"
    WF_QUEUE_DECREASE="PASS"
    queue_already_decreased="true"
  fi

  run_settle_until_rfs_closed
  WF_SETTLE="PASS"

  if [ "$queue_already_decreased" != "true" ]; then
    current_queue="$(read_queue "$LCC_OUT" "$RECIPIENT")"
    if dec_lt "$current_queue" "$QUEUED_AFTER_SWAP"; then
      QUEUED_FINAL="$current_queue"
      WF_QUEUE_DECREASE="PASS"
