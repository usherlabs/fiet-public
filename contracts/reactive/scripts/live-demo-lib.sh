...
MAX_SETTLE_ATTEMPTS="${MAX_SETTLE_ATTEMPTS:-5}"
...
print_workflow_from_rfs() {
  ...
  echo ""
  echo "========================================"
  echo "Live Reactive Settlement demo (resume from RFS)"
  echo "  overall: $status"
  echo "========================================"
  printf '  %-4s %-40s %-8s %s\n' "Step" "Action" "Result" "Detail"
  printf '  %-4s %-40s %-8s %s\n' "----" "----------------------------------------" "--------" "----------------------------------------"

  print_workflow_row "1" "Resume preflight / queue baseline" "$WF_RESUME" \
    "commit=${COMMIT_ID:-?} position=${POSITION_INDEX:-?} lccOut=${LCC_OUT:-unknown}"
  print_workflow_row "2" "Maker position settlement" "$WF_SETTLE" "$(tx_mining_detail "$PROTOCOL_RPC" "${SETTLE_TX:-}")"
  print_workflow_row "3" "Reactive queue settlement decrease" "$WF_QUEUE_DECREASE" \
    "${QUEUED_AFTER_SWAP:-?} -> ${QUEUED_FINAL:-?} (settled=$queue_settled_amount)"
  if [ "$CLOSE_POSITION_AFTER_DEMO" = "true" ]; then
    print_workflow_row "4" "Close demo MM position" "$WF_CLOSE" "$(tx_mining_detail "$PROTOCOL_RPC" "${CLOSE_TX:-}")"
  else
    print_workflow_row "4" "Close demo MM position" "$WF_CLOSE" "CLOSE_POSITION_AFTER_DEMO=false"
  fi

  echo ""
  echo "Assumes live-demo steps 1-4 already completed (create, swap, queue increase, HubRSC mirror)."
  echo ""
  echo "Reactive automation path (steps 2-3 when live):"
  echo "  Protocol SettlementQueued"
  echo "    -> HubRSC.react() / applyCanonicalProtocolLog (Reactive)"
  echo "    -> BatchProcessSettlement.processSettlements (protocol)"
  echo "    -> LiquidityHub.processSettlementFor (protocol)"
...
preflight_from_rfs() {
  ...
  for name in PROTOCOL_RPC REACTIVE_RPC NETWORK CORE_POOL_ID LIQUIDITY_HUB HUB_RSC BATCH_RECEIVER RECIPIENT \
    COMMIT_ID POSITION_INDEX LCC_OUT; do
    require_env "$name"
  done
  [ -n "$MM_LOCKER_PRIVATE_KEY_VALUE" ] || fail "Missing MM_LOCKER_PRIVATE_KEY or legacy MM_PRIVATE_KEY for MMPositionManager actions."
  ...
  case "$MAX_SETTLE_ATTEMPTS" in
    ''|*[!0-9]*) fail "Invalid MAX_SETTLE_ATTEMPTS=$MAX_SETTLE_ATTEMPTS. Use a positive integer." ;;
  esac
  if [ "$MAX_SETTLE_ATTEMPTS" -lt 1 ]; then
    fail "MAX_SETTLE_ATTEMPTS must be >= 1."
  fi

  echo "Resume target: commit=$COMMIT_ID position=$POSITION_INDEX lccOut=$LCC_OUT recipient=$RECIPIENT"
...
print_summary_from_rfs() {
  local status="$1"
  local state pending exists inflight remaining queue_settled_amount
  state="$(read_hub_state "${LCC_OUT:-0x0000000000000000000000000000000000000000}" "$RECIPIENT" 2>/dev/null || true)"
  pending="$(printf '%s\n' "$state" | awk '{print $2}')"
  exists="$(printf '%s\n' "$state" | awk '{print $3}')"
  inflight="$(printf '%s\n' "$state" | awk '{print $4}')"
  remaining="${QUEUED_FINAL:-${QUEUED_AFTER_SWAP:-0}}"
  ...
  print_workflow_from_rfs "$status"

  echo ""
  echo "========================================"
  echo "Live Reactive Settlement demo summary (resume)"
  echo "  status: $status"
  echo "  commitId: ${COMMIT_ID:-unknown}"
  echo "  positionIndex: ${POSITION_INDEX:-0}"
  echo "  makerSettle0: ${MAKER_SETTLE0:-unknown}"
  echo "  makerSettle1: ${MAKER_SETTLE1:-unknown}"
  echo "  rfsOpenBeforeSettle: ${RFS_OPEN_BEFORE_SETTLE:-unknown}"
  echo "  rfsOpenAfterSettle: ${RFS_OPEN_AFTER_SETTLE:-unknown}"
  echo "  settleAttempts: ${SETTLE_ATTEMPTS:-0}"
  echo "  lccOut: ${LCC_OUT:-unknown}"
  echo "  recipient: $RECIPIENT"
  echo "  queuedAfterSwap: ${QUEUED_AFTER_SWAP:-unknown}"
  echo "  queuedFinal: ${QUEUED_FINAL:-unknown}"
...
run_settle_until_rfs_closed() {
  local attempt=1
  local settle_out settle0 settle1 title
  ...
  while [ "$attempt" -le "$MAX_SETTLE_ATTEMPTS" ]; do
    ...
    if [ "$MAKER_SETTLE0" = "0" ] && [ "$MAKER_SETTLE1" = "0" ]; then
      fail "Maker RFS remained open but SettleMMPosition reported no positive settlement delta."
    fi
    attempt=$((attempt + 1))
  done

  fail "Maker RFS remained open after ${MAX_SETTLE_ATTEMPTS} settlement attempt(s)."
}
