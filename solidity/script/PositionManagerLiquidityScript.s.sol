...
contract PositionManagerLiquidityScript is ScriptHelper {
    ...
    function setupERC20Allowance(address user) internal {
        checkAndApproveErc20(user, address(permit2), lccUSDCToken, AMOUNT_A_DESIRED);
        checkAndApproveErc20(user, address(permit2), lccUSDTToken, AMOUNT_B_DESIRED);
        checkAndApproveErc20(user, address(lccUSDCToken), IERC20(usdcToken), AMOUNT_A_DESIRED);
        checkAndApproveErc20(user, address(lccUSDTToken), IERC20(usdtToken), AMOUNT_B_DESIRED);
        checkAndApproveErc20(user, address(proxyHook), IERC20(usdcToken), AMOUNT_A_DESIRED);
        checkAndApproveErc20(user, address(proxyHook), IERC20(usdtToken), AMOUNT_A_DESIRED);
    }
    ...
    function setupPermit2Allowances(address user) internal {
        ...
        require(permit2AllowanceA >= AMOUNT_A_DESIRED, "Insufficient ERC20 allowance to Permit2 for token A");
        require(permit2AllowanceB >= AMOUNT_B_DESIRED, "Insufficient ERC20 allowance to Permit2 for token B");
    ...
    function permit2AllowanceTransfer(address user, IToken token, address spender) internal {
        ...
        uint256 requiredAmount = (address(token) == address(lccUSDCToken)) ? AMOUNT_A_DESIRED : AMOUNT_B_DESIRED;
