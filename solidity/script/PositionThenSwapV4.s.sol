...
contract PositionThenSwapV4 is ScriptHelper {
    ...
    uint256 constant AMOUNT_DESIRED = 1000; // 1000 tokens
    ...
    function run() external {
        console.log("Starting PositionThenSwapV4 script...");
        ...
        // Whitelist and setup phase
        vm.startBroadcast(uint256(vm.envBytes32("PRIVATE_KEY")));
        lccWhitelistLPAndCustodian(userAddress);
        ...
        setupERC20Allowance(userAddress);
        setupPermit2Allowances(userAddress);
        wrapToLccTokens(userAddress);
        uint256 tokenId = mintPositionToCore(userAddress);
        ...
        swapExactInputSingle(proxyPoolKey, 10e18, 0, userAddress);
        console.log("Swap executed");
        vm.stopBroadcast();

        // Verify swap results
        uint256 balanceAfterCurrency1 = IERC20(Currency.unwrap(proxyPoolKey.currency1)).balanceOf(userAddress);
        uint256 balanceAfterCurrency0 = IERC20(Currency.unwrap(proxyPoolKey.currency0)).balanceOf(userAddress);
        console.log("Final balances after swap:");
        console.log("Currency 0:", balanceAfterCurrency0 / 1e18);
        console.log("Currency 1:", balanceAfterCurrency1 / 1e18);
    }
    ...
    function approveLccCustodian(address _address, IToken token) internal {
        ...
        if (!whitelisted) {
            ...
            console.log("Whitelisted", _address, "as custodian for", address(token));
        } else {
            console.log(_address, "is already whitelisted as custodian for", address(token));
        }
    }
