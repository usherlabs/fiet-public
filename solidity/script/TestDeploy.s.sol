...
contract TestDeployScript is Script {
    ...
    function _testMarketFactoryDeployment() internal {
        ...
        address mockCoreHook = address(
            0x1111111111111111111111111111111111111111
        );
        address mockProxyHook = address(
        ...
        require(
            factory.getCoreHook() == mockCoreHook,
            "MarketFactory: wrong coreHook"
        );
        require(
            factory.getProxyHook() == mockProxyHook,
    ...
    function testHookPermissions() external {
        ...
        CoreHook coreHookInstance = new CoreHook(
            SepoliaConstants.POOL_MANAGER,
            mockMarketFactory
        );
        Hooks.Permissions memory corePermissions = coreHookInstance
            .getHookPermissions();

        require(
            corePermissions.beforeInitialize,
            "CoreHook should have beforeInitialize"
        );
        require(
            corePermissions.afterAddLiquidity,
            "CoreHook should have afterAddLiquidity"
        );
        require(
            corePermissions.afterRemoveLiquidity,
            "CoreHook should have afterRemoveLiquidity"
        );
        require(
            !corePermissions.beforeSwap,
            "CoreHook should not have beforeSwap"
        );

        console.log("CoreHook permissions verified");

        // Test ProxyHook permissions
        ProxyHook proxyHookInstance = new ProxyHook(
            SepoliaConstants.POOL_MANAGER,
            mockMarketFactory
        );
        Hooks.Permissions memory proxyPermissions = proxyHookInstance
            .getHookPermissions();

        require(
            proxyPermissions.beforeInitialize,
            "ProxyHook should have beforeInitialize"
        );
        require(
            proxyPermissions.beforeAddLiquidity,
            "ProxyHook should have beforeAddLiquidity"
        );
        require(
            proxyPermissions.beforeSwap,
            "ProxyHook should have beforeSwap"
        );
        require(
            proxyPermissions.beforeSwapReturnDelta,
            "ProxyHook should have beforeSwapReturnDelta"
        );
        require(
            !proxyPermissions.afterAddLiquidity,
            "ProxyHook should not have afterAddLiquidity"
        );
