...
contract VTSRingsTest is Test {
    ...
    contract VTSHarness is VTSManager {
        ...
        {
            PositionId id = PositionId.wrap(
                keccak256(abi.encodePacked(router, params.tickLower, params.tickUpper, params.salt))
            );
            c0 = commitmentMaxima[id][0];
            c1 = commitmentMaxima[id][1];
        }
        function _trackCommitment(address router, PoolId pid, ModifyLiquidityParams calldata params) external {
            VTSManager._trackCommitment(router, pid, params);
        }
    }
